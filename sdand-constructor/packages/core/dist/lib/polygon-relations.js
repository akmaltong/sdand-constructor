export function pointOnSegment(point, start, end, tolerance = 1e-6) {
...
export function pointInPolygon(point, polygon, options) {
    ...
    for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
        ...
        const intersects = current[1] > z !== previous[1] > z &&
...
export function polygonsIntersect(left, right) {
    for (let leftIndex = 0; leftIndex < left.length; leftIndex++) {
        ...
        for (let rightIndex = 0; rightIndex < right.length; rightIndex++) {
            if (segmentsIntersect(leftStart, leftEnd, right[rightIndex], right[(rightIndex + 1) % right.length])) {
                return true;
            }
        }
    }
...
export function polygonsOverlap(left, right) {
    return (polygonsIntersect(left, right) ||
