...
function pointDistanceToPolygonBoundary(point, polygon) {
    ...
    for (let index = 0; index < polygon.length; index += 1) {
        ...
        if (!(start && end))
            continue;
        minDistance = Math.min(minDistance, distanceToSegment(pointToTuple(point), pointToTuple(start), pointToTuple(end)));
    }
    return minDistance;
}
function wallBoundsRoom(wall, roomPolygon) {
    ...
    if (sampled.length === 0)
        return false;
    const candidates = sampled.length === 2
        ? [
            sampled[0],
            {
