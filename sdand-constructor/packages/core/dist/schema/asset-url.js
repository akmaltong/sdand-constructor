 ...
 */
const ALLOWED_SCHEMES = ['asset:', 'blob:', 'https:', 'data:image/', 'primitive:'];
...
function isAllowedPrimitiveUrl(url) {
...
function isAllowedAssetUrl(url) {
    ...
    if (url.startsWith('primitive:'))
        return isAllowedPrimitiveUrl(url);
    try {
...
export const AssetUrl = z.string().refine(isAllowedAssetUrl, {
    message: 'URL must be asset://, blob:, data:image/, primitive:box:<hex>, primitive:tex:<url>, /path, or https://. http://localhost allowed for dev.',
});
