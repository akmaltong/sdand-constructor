 ...
 */
export function getEffectiveDormerSurfaceMaterial(node, role) {
    const top = {
    ...
    const has = (spec) => spec.material !== undefined || typeof spec.materialPreset === 'string';
    if (role === 'top')
        return has(top) ? top : legacy;
    if (role === 'side')
        return has(side) ? side : has(wall) ? wall : legacy;
