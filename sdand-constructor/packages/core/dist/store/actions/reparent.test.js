...
globalThis.cancelAnimationFrame ??=
    () => { };
const ROOF_ID = 'roof_test';
const SEG_A_ID = 'rseg_a';
const SEG_B_ID = 'rseg_b';
const VENT_ID = 'bvent_v1';
function makeRoof() {
    return {
        id: ROOF_ID,
        type: 'roof',
...
function makeSegment(id, children = []) {
    return {
        id,
        type: 'roof-segment',
        parentId: ROOF_ID,
...
function makeVent(parentId) {
    return {
        id: VENT_ID,
        type: 'box-vent',
        parentId,
        roofSegmentId: parentId,
        object: 'node',
        visible: true,
        name: '',
        metadata: {},
...
function childrenOf(id) {
    const seg = useScene.getState().nodes[id];
    return (seg?.children ?? []);
}
function vent() {
    return useScene.getState().nodes[VENT_ID];
}
describe('node-actions reparent — repeated segment-hopping', () => {
    ...
    test('A→B→A→B: three hops leave children lists clean (regression: vent crash)', () => {
        ...
        useScene
            .getState()
            .updateNode(VENT_ID, { parentId: SEG_A_ID, roofSegmentId: SEG_A_ID });
        expect(childrenOf(SEG_A_ID)).toEqual([VENT_ID]);
        expect(childrenOf(SEG_B_ID)).toEqual([]);
        ...
        // on unmount.
        const totalListings = childrenOf(SEG_A_ID).filter((id) => id === VENT_ID).length +
    ...
    test.each([
        ...
        const hop = (to) => useScene.getState().updateNode(id, { parentId: to, roofSegmentId: to });
        hop(SEG_B_ID);
        hop(SEG_A_ID);
        hop(SEG_B_ID);
        expect(childrenOf(SEG_A_ID)).toEqual([]);
    ...
    test('A→B→A→B with redundant manual children edits (vent move-tool pattern, pre-fix)', () => {
        ...
        const hop = (from, to) => {
            ...
            const oldSeg = st.nodes[from];
            if (oldSeg) {
                st.updateNode(from, {
                    children: (oldSeg.children ?? []).filter((id) => id !== VENT_ID),
                });
            }
            const newSeg = st.nodes[to];
