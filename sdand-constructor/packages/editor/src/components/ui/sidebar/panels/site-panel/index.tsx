  ...
  return (
    ...
    >
      ...
      <div className="flex h-8 min-w-0 flex-1 cursor-pointer items-center gap-2 py-0 pl-[60px] text-muted-foreground group-hover/ref:text-foreground">
        ...
        <InlineRenameInput
          defaultName={refNode.type === 'scan' ? 'Площадка 3D' : 'Подложка'}
          isEditing={isEditing}
          nodeId={refNode.id}
          ...
          return (
            <div className="group/ref relative border-border/50 border-b" key="upload">
              ...
              >
                ...
                {uploading ? `Загрузка ${uploadingType}... ${progress}%` : 'Загрузить план / скан'}
              </button>
    ...
    <div className="relative flex flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        >
          ...
          <span className="truncate">Добавить уровень</span>
        </button>
        {levels.length === 0 && (
...
const ContentSection = memo(function ContentSection() {
  ...
  if (elementChildren.length === 0) {
    return <div className="px-3 py-4 text-muted-foreground text-sm">На этом уровне ничего нет</div>
  }
  return (
    ...
    >
      ...
      >
        <div className="flex h-full min-w-0 flex-1 cursor-pointer items-center gap-2 py-2 pl-3">
          ...
          />
          <span className="truncate font-medium text-sm">{building.name || 'Здание'}</span>
        </div>
        <Popover
        ...
        {siteNode && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              />
              <span className="font-medium text-sm">{siteNode.name || 'Проект'}</span>
            </div>
            <CameraPopover
              buttonClassName={cn(
                'transition-colors',
