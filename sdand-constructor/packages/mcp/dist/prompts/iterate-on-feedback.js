 ...
 */
export function buildIterateOnFeedbackPrompt(args) {
    return [PREAMBLE, '', '## User feedback', args.feedback.trim()].join('\n');
}
export function registerIterateOnFeedback(server, _bridge) {
    server.registerPrompt('iterate_on_feedback', {
        title: 'Iterate on a scene from user feedback',
        description: 'Produces a minimal-diff plan of apply_patch calls in response to user feedback on the current scene.',
        argsSchema: {
            feedback: z.string(),
        },
    }, async ({ feedback }) => ({
