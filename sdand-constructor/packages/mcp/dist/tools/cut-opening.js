...
export function registerCutOpening(server, bridge) {
    server.registerTool('cut_opening', {
        title: 'Cut opening',
        description: 'Cut a door or window opening into an existing wall. position is a parametric 0..1 offset along the wall centreline.',
        ...
        if (length < width) {
            throwMcpError(ErrorCode.InvalidParams, `Wall ${wallId} is ${length.toFixed(2)}m long, too short for a ${width.toFixed(2)}m opening`);
        }
        ...
        const base = {
            ...
            width,
            height,
            position: [wallLocalXFromT(wall, position, width), height / 2, 0],
        };
        ...
        const id = bridge.createNode(opening, wallId);
        await publishLiveSceneSnapshot(bridge, 'cut_opening');
        const payload = { openingId: id };
        return {
