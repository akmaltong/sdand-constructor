...
export function syncDerivedStairOpenings(operations) {
    const updates = syncAutoStairOpenings(operations.getNodes());
    if (updates.length === 0)
        return 0;
    operations.applyPatch(updates.map((update) => ({
        op: 'update',
        id: update.id,
        data: update.data,
    })));
 ...
 */
export async function publishLiveSceneSnapshot(operations, kind) {
    syncDerivedStairOpenings(operations);
    const active = operations.getActiveScene();
    if (!(active && operations.canAppendSceneEvents))
        return;
...
export async function appendLiveSceneEvent(operations, sceneId, version, kind, graph) {
