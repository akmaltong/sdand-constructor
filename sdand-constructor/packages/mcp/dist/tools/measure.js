...
export function registerMeasure(server, bridge) {
    server.registerTool('measure', {
        title: 'Measure',
        description: 'Measure distance (in meters) between two nodes, or the area of a polygon node when fromId === toId.',
        inputSchema: measureInput,
        outputSchema: measureOutput,
    }, async ({ fromId, toId }) => {
        const from = bridge.getNode(fromId);
        if (!from) {
            throwMcpError(ErrorCode.InvalidParams, `Node not found: ${fromId}`);
        }
        const to = bridge.getNode(toId);
        if (!to) {
        ...
        if (fromId === toId) {
            ...
            if (n.type === 'zone' || n.type === 'slab' || n.type === 'ceiling') {
                const area = shoelaceArea(n.polygon);
                const payload = {
                    distanceMeters: 0,
                    areaSqMeters: area,
                    units: 'meters',
            ...
            const payload = { distanceMeters: 0, units: 'meters' };
            return {
                content: [{ type: 'text', text: JSON.stringify(payload) }],
                structuredContent: payload,
            };
        }
        const fromCentre = getCentre(from);
        const toCentre = getCentre(to);
        if (!(fromCentre && toCentre)) {
            throwMcpError(ErrorCode.InvalidRequest, `Cannot derive centre for measurement between ${from.type} and ${to.type}`);
        }
