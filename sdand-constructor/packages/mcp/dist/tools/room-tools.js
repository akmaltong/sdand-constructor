import { CeilingNode, DoorNode, ItemNode, SlabNode, WallNode, WindowNode, ZoneNode, } from '@pascal-app/core/schema';
import { z } from 'zod';
import { findCatalogItem, searchCatalogItems } from './asset-catalog';
import { ErrorCode, throwMcpError } from './errors';
import { pointInBoundsWithPadding, polygonArea, polygonBounds, wallLength, wallLocalXFromT, } from './geometry';
...
function assertLevel(bridge, levelId) {
    ...
    if (typeof level.metadata === 'object' &&
        level.metadata !== null &&
        'role' in level.metadata &&
        level.metadata.role === 'roof') {
        throwMcpError(ErrorCode.InvalidParams, `Roof support level ${levelId} is not an occupied story; create rooms or furnishings on an occupied level instead`);
    }
    return level;
}
function assertWall(bridge, wallId) {
...
function inferRoomGeometry(bridge, levelId, polygon, zoneId) {
    if (levelId && polygon)
        return { levelId, polygon };
    if (!zoneId) {
        throwMcpError(ErrorCode.InvalidParams, 'Provide either levelId + polygon or zoneId so the room can be furnished');
    }
    const zone = bridge.getNode(zoneId);
    if (!zone)
...
function resolveWallT(toolName, t, position) {
...
function itemFootprint(asset, x, z, rotationDeg = 0) {
...
function buildRoomPlacements(roomType, polygon, doorWallIndex = 0) {
    ...
    const sidePos = (inset, lateral = 0) => [
        sideMidX + snX * inset + sax * lateral,
        sideMidZ + snZ * inset + saz * lateral,
    ];
    const placements = [];
    const area = polygonArea(polygon);
    const addBack = (assetId, inset, lateral = 0, rotationDeg = facingRot) => {
        const [x, z] = backPos(inset, lateral);
        placements.push({ assetId, x, z, rotationDeg });
    };
    const addSide = (assetId, inset, lateral = 0, rotationDeg = sideRot) => {
    ...
    switch (roomType) {
        case 'bedroom': {
            ...
            if (area >= 10)
                addSide('dresser', 0.55, sideAlongLen * 0.22);
            if (area >= 13)
                addSide('closet', 0.6, -sideAlongLen * 0.22);
            break;
        }
        case 'kitchen':
            addBack(alongLen >= 2.6 ? 'kitchen' : 'kitchen-counter', 0.55);
            if (alongLen >= 3.5)
                addBack('stove', 0.55, alongLen / 2 - 0.65);
        ...
        case 'bathroom':
            ...
            if (area >= 6.5)
                addSide('bathtub', 0.85);
            else
                placements.push({ assetId: 'shower-square', x: bounds.centerX, z: bounds.centerZ });
            break;
        case 'living': {
            addBack('sofa', 0.9);
        ...
        case 'entry':
        case 'hallway':
            if (Math.min(bounds.width, bounds.depth) >= 1.4)
                addSide('coat-rack', 0.35);
            break;
        case 'storage':
            addBack('closet', 0.6);
            break;
...
export function registerSearchAssets(server) {
    server.registerTool('search_assets', {
        title: 'Search assets',
        description: 'Search the built-in MCP item catalog by keyword. Call before place_item when you need a valid catalogItemId.',
...
export function registerCreateRoom(server, bridge) {
    server.registerTool('create_room', {
        title: 'Create room',
        description: 'Create a room on a level: zone, slab, ceiling, and one wall per polygon edge. Returns wallIds in polygon edge order.',
        ...
        const walls = points.map((start, index) => WallNode.parse({
...
export function registerAddDoor(server, bridge) {
    server.registerTool('add_door', {
        title: 'Add door',
        description: 'Add a door to an existing wall. t/position is 0..1 along the wall: 0 = start, 0.5 = center, 1 = end.',
        ...
        if (length < width) {
            throwMcpError(ErrorCode.InvalidParams, `Wall ${wallId} is ${length.toFixed(2)}m long, too short for a ${width.toFixed(2)}m door`);
        }
        ...
        const id = bridge.createNode(door, wallId);
...
export function registerAddWindow(server, bridge) {
    server.registerTool('add_window', {
        title: 'Add window',
        description: 'Add a window to an existing wall. t/position is 0..1 along the wall; sillHeight is the height from floor to window bottom.',
        ...
        const length = wallLength(wall);
        if (length < width) {
            throwMcpError(ErrorCode.InvalidParams, `Wall ${wallId} is ${length.toFixed(2)}m long, too short for a ${width.toFixed(2)}m window`);
        }
        ...
        const id = bridge.createNode(windowNode, wallId);
...
export function registerFurnishRoom(server, bridge) {
    server.registerTool('furnish_room', {
        title: 'Furnish room',
        description: 'Place realistic furniture for a room type using levelId + polygon, or infer both from zoneId. Parent floor items to the level so they render and validate.',
        inputSchema: furnishRoomInput,
        outputSchema: furnishRoomOutput,
    }, async ({ levelId, zoneId, roomType, polygon, doorWallIndex }) => {
        const room = inferRoomGeometry(bridge, levelId, polygon, zoneId);
        ...
        for (const placement of placements) {
            ...
            const padding = 0.05;
            if (!(pointInBoundsWithPadding(fp.minX, fp.minZ, bounds, -padding) &&
        ...
        if (items.length > 0) {
            bridge.applyPatch(items.map((item) => ({
                op: 'create',
                node: item,
                parentId: room.levelId,
            })));
...
export function registerRoomTools(server, bridge) {
