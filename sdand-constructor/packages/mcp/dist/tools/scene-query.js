import { z } from 'zod';
import { distance2D, pointInPolygon, polygonArea, polygonContainsPolygon, wallLength, } from './geometry';
...
function getDefaultLevelId(bridge, requested) {
    if (requested)
        return requested;
    const level = getLevels(bridge)[0];
    return level?.id ?? null;
}
function nodesOnLevel(bridge, levelId) {
    return Object.values(bridge.getNodes()).filter((node) => node.id !== levelId && bridge.resolveLevelId(node.id) === levelId);
}
function metadataRecord(node) {
...
function occupiedContentCount(counts) {
    return (counts.walls +
        counts.zones +
        counts.doors +
        counts.windows +
...
function classifyLevel(level, counts) {
    const metadataRole = metadataString(level, 'role');
    if (metadataRole === 'roof')
        return 'roof';
    if (metadataRole === 'support')
        return 'support';
    if (counts.roofs > 0 && occupiedContentCount(counts) === 0)
        return 'roof';
    return 'occupied';
}
function openingSummaries(bridge, wallId) {
    return bridge
        .getChildren(wallId)
...
function wallSummary(bridge, wall) {
    if (wall.type !== 'wall')
        return null;
    ...
    return {
        ...
        openings: openingSummaries(bridge, wall.id),
    };
}
...
function stairFootprintPolygons(bridge, stair) {
    ...
    const usableSegments = segments.length > 0
        ? segments
        : [
            {
                width: stair.width ?? 1,
...
function wallSamplePoints(wall) {
...
function targetLevelIdsForStair(bridge, stair) {
    const nodes = bridge.getNodes();
    const parentLevelId = bridge.resolveLevelId(stair.id);
...
function levelSummary(bridge, levelId) {
    ...
    const walls = nodes
        ...
        .filter((n) => !!n);
    const zones = nodes.map(zoneSummary).filter((n) => !!n);
    const items = nodes.map(itemSummary).filter((n) => !!n);
    const slabs = nodes
...
export function registerListLevels(server, bridge) {
    server.registerTool('list_levels', {
        title: 'List levels',
        description: 'List all levels in the current scene with ids, names, floor indices, and child counts.',
        inputSchema: {},
        outputSchema: listLevelsOutput,
    }, async () => {
        const activeScene = bridge.getActiveScene();
        const levels = getLevels(bridge).map((level) => {
            const summary = levelSummary(bridge, level.id);
            return {
                id: level.id,
                name: level.name,
                ...
                childCount: bridge.getChildren(level.id).length,
            };
        });
...
export function registerGetLevelSummary(server, bridge) {
    server.registerTool('get_level_summary', {
        title: 'Get level summary',
        description: 'Get a compact model-friendly summary of one level: counts plus walls, zones, slabs, ceilings, and items. Omit levelId to use the first level.',
        inputSchema: levelScopedInput,
        outputSchema: getLevelSummaryOutput,
    }, async ({ levelId }) => {
        const resolved = getDefaultLevelId(bridge, levelId);
        if (!resolved)
            throw new Error('No level exists in the scene');
        return textResult(levelSummary(bridge, resolved));
    });
}
export function registerGetWalls(server, bridge) {
    server.registerTool('get_walls', {
        title: 'Get walls',
        description: 'Get walls on a level with start/end coordinates, length, height, thickness, and child doors/windows. Omit levelId to use the first level.',
        inputSchema: levelScopedInput,
...
export function registerGetZones(server, bridge) {
    server.registerTool('get_zones', {
        title: 'Get zones',
        description: 'Get room/zone polygons on a level with names, colors, bounds, and approximate areas. Omit levelId to use the first level.',
        inputSchema: levelScopedInput,
...
export function registerVerifyScene(server, bridge) {
    server.registerTool('verify_scene', {
        title: 'Verify scene',
        description: 'High-level self-check after complex edits. Returns validation status, per-level room/content counts, empty levels, and practical layout issues.',
        inputSchema: {},
        ...
        const levels = getLevels(bridge).map((level) => {
            const summary = levelSummary(bridge, level.id);
            const totalContent = Object.values(summary.counts).reduce((sum, count) => sum + (typeof count === 'number' ? count : 0), 0);
            return {
                levelId: level.id,
        ...
        for (const level of levels) {
            ...
            if (level.role === 'roof') {
                if (level.content.roofs === 0) {
                    issues.push(`${level.levelName} is a roof support level but has no roof geometry; add or move roof geometry there rather than deleting the support level to satisfy story count`);
                }
                if (occupiedContent > 0) {
                    issues.push(`${level.levelName} is a roof support level but contains occupied-story content; move rooms, walls, stairs, slabs, ceilings, and items to an occupied story and keep the roof level for roof geometry only`);
                }
                if (level.referenceLevelId) {
                    ...
                    if (referenceLevel?.type === 'level' && level.floorIndex <= referenceLevel.level) {
                        issues.push(`${level.levelName} roof support level should be above its reference occupied level ${referenceLevel.name ?? referenceLevel.id}`);
                    }
                }
                continue;
            }
            ...
            if (level.content.roofs > 0 &&
                ...
                issues.push(`${level.levelName} mixes roof geometry with occupied-level content; place roofs on a dedicated roof level for solo/exploded level views`);
            }
        }
        ...
        if (hasMultipleOccupiedStories) {
            for (const level of getLevels(bridge)) {
                ...
                if (!summary?.isOccupiedStory)
                    continue;
                const expectedHeight = typeof level.metadata === 'object' &&
                    level.metadata !== null &&
                    'height' in level.metadata &&
                    typeof level.metadata.height === 'number'
                    ? level.metadata.height
                    : 3.2;
                for (const wall of nodesOnLevel(bridge, level.id).filter((node) => node.type === 'wall')) {
                    const wallHeight = wall.height ?? 2.5;
                    if (wallHeight > expectedHeight + 0.25) {
                        issues.push(`Wall ${wall.name ?? wall.id} on ${level.name ?? level.id} is ${wallHeight}m high; multi-story exterior walls should be split into level-owned story walls`);
                    }
                }
            }
        }
        for (const node of Object.values(bridge.getNodes())) {
            if (node.type === 'door' || node.type === 'window') {
                const parent = node.parentId ? bridge.getNode(node.parentId) : null;
                if (!parent || parent.type !== 'wall') {
                    issues.push(`${node.type} ${node.id} is not parented to a wall`);
                    continue;
                }
                if (node.wallId !== parent.id) {
                    issues.push(`${node.type} ${node.id} has wallId ${node.wallId ?? 'unset'} but is parented to wall ${parent.id}`);
                }
                ...
                if (bottom < -0.01 || top > wallHeight + 0.01) {
                    issues.push(`${node.type} ${node.id} vertical bounds [${bottom.toFixed(2)}, ${top.toFixed(2)}] exceed wall ${parent.id} height ${wallHeight.toFixed(2)}m`);
                }
            }
        }
        for (const stair of Object.values(bridge.getNodes()).filter((node) => node.type === 'stair')) {
            const sourceLevelId = bridge.resolveLevelId(stair.id);
            if (sourceLevelId) {
                const sourceLevel = bridge.getNode(sourceLevelId);
                const footprints = stairFootprintPolygons(bridge, stair);
                const sourceSlabs = nodesOnLevel(bridge, sourceLevelId).filter((node) => node.type === 'slab');
                if (sourceSlabs.length > 0) {
                    const outsideFootprints = footprints.filter((footprint) => !sourceSlabs.some((slab) => polygonContainsPolygon(slab.polygon, footprint)));
                    if (outsideFootprints.length > 0) {
                        issues.push(`Stair ${stair.name ?? stair.id} footprint extends outside source floor slab on ${sourceLevel?.name ?? sourceLevelId}`);
                    }
                }
                const obstructingWalls = nodesOnLevel(bridge, sourceLevelId)
                    .filter((node) => node.type === 'wall')
                    .filter((wall) => footprints.some((footprint) => wallSamplePoints(wall).some((point) => pointInPolygon(point, footprint, false))));
                for (const wall of obstructingWalls) {
                    issues.push(`Wall ${wall.name ?? wall.id} obstructs stair ${stair.name ?? stair.id} on ${sourceLevel?.name ?? sourceLevelId}`);
                }
            }
            if ((stair.slabOpeningMode ?? 'none') === 'destination') {
                ...
                if (targetLevelIds.length === 0) {
                    issues.push(`Stair ${stair.name ?? stair.id} requests a slab opening but has no target level`);
                }
                for (const targetLevelId of targetLevelIds) {
                    const targetLevel = bridge.getNode(targetLevelId);
                    const targetSlabs = nodesOnLevel(bridge, targetLevelId).filter((node) => node.type === 'slab');
                    if (targetSlabs.length === 0) {
                        issues.push(`Stair ${stair.name ?? stair.id} targets ${targetLevel?.name ?? targetLevelId} but it has no slab`);
                        continue;
                    }
                    const matchingHoles = targetSlabs.flatMap((slab) => (slab.holes ?? [])
                        .map((hole, index) => ({ slab, hole, index }))
                        .filter((entry) => holeBelongsToStair(entry.slab, entry.index, stair.id)));
                    if (matchingHoles.length === 0) {
                        issues.push(`Stair ${stair.name ?? stair.id} has no destination slab opening on ${targetLevel?.name ?? targetLevelId}`);
                        continue;
                    }
                    for (const { slab, hole } of matchingHoles) {
                        if (!polygonContainsPolygon(slab.polygon, hole)) {
                            issues.push(`Stair ${stair.name ?? stair.id} opening extends outside slab ${slab.name ?? slab.id}`);
                        }
                    }
                }
            }
...
export function registerSceneQueryTools(server, bridge) {
