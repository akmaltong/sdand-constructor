...
export const analyzeRoomPhotoOutput = {
    ...
    identifiedFixtures: z.array(z.object({
        type: z.string(),
        approximatePosition: z.tuple([z.number(), z.number()]).optional(),
    })),
    identifiedWindows: z.array(z.object({
...
export function registerAnalyzeRoomPhoto(server, _bridge) {
    server.registerTool('analyze_room_photo', {
        title: 'Analyze room photo',
        description: 'Defer to the MCP host (via sampling) to extract approximate dimensions, fixtures, and windows from a single-room photograph. Requires host support for sampling.',
