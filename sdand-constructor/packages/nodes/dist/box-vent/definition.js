import { BoxVentNode as BoxVentNodeSchema, } from '@pascal-app/core';
...
function boxVentWidthHandle(side) {
...
function boxVentRotateHandle() {
    return {
        ...
        decoration: {
            kind: 'ring',
            radius: (n) => Math.hypot(n.width / 2 + ROTATE_CORNER_OFFSET, n.depth / 2 + ROTATE_CORNER_OFFSET),
            y: (n) => getBodyMidY(n),
        },
    };
}
 ...
 * vent on click.
 */
export const boxVentDefinition = {
    kind: 'box-vent',
    schemaVersion: 1,
    schema: BoxVentNode,
    category: 'structure',
    surfaceRole: 'roof',
    defaults: () => {
        const stub = BoxVentNodeSchema.parse({ id: 'bvent_default', type: 'box-vent' });
    ...
    mcp: {
        description: 'A louvered box vent sitting on a roof segment. Style: standard / low-profile / dome. Width/depth/height/hoodOverhang parametric.',
    },
};
