 ...
 */
export function buildBoxVentGeometry(node) {
    if (node.style === 'box')
        return buildBoxShape(node);
    if (node.style === 'cap')
        return buildCapShape(node);
...
function buildBoxShape(node) {
    ...
    const cornerBevel = Math.max(0, Math.min(node.cornerBevel ?? 0.012, Math.min(baseW, baseD) / 2 - 0.001));
    const positions = [];
    const normals = [];
    const uvs = [];
...
function buildRoundedExtrusion(positions, normals, uvs, w, d, y0, y1, bevel) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (len < 1e-9)
            continue; // degenerate edge (zero-bevel duplicate corner points)
        const nx = ez / len;
        const nz = -ex / len;
        pushQuad(positions, normals, uvs, [a.x, y0, a.z], [b.x, y0, b.z], [b.x, y1, b.z], [a.x, y1, a.z], [nx, 0, nz]);
    }
...
function roundedRectProfile(w, d, bevel, segsPerCorner) {
    const hw = w / 2;
    const hd = d / 2;
...
function buildCapShape(node) {
    ...
    pushQuad(positions, normals, uvs, [hw, 0, -hd], [hw, 0, hd], [hw, bodyH, hd], [hw, bodyH, -hd], [1, 0, 0]);
    pushQuad(positions, normals, uvs, [-hw, 0, hd], [-hw, 0, -hd], [-hw, bodyH, -hd], [-hw, bodyH, hd], [-1, 0, 0]);
    pushQuad(positions, normals, uvs, [hw, 0, hd], [-hw, 0, hd], [-hw, bodyH, hd], [hw, bodyH, hd], [0, 0, 1]);
    pushQuad(positions, normals, uvs, [-hw, 0, -hd], [hw, 0, -hd], [hw, bodyH, -hd], [-hw, bodyH, -hd], [0, 0, -1]);
    pushQuad(positions, normals, uvs, [-hw, 0, -hd], [-hw, 0, hd], [hw, 0, hd], [hw, 0, -hd], [0, -1, 0]);
    // ── Body top (only when there's a visible gap to look through)
    if (capGap > 0) {
        pushQuad(positions, normals, uvs, [-hw, bodyH, hd], [-hw, bodyH, -hd], [hw, bodyH, -hd], [hw, bodyH, hd], [0, 1, 0]);
    }
    // ── Flange underside (the bit of the cap base that overhangs the body)
    if (overhang > 0 || capGap > 0) {
        pushQuad(positions, normals, uvs, [-bw, y0, -bd], [-bw, y0, bd], [bw, y0, bd], [bw, y0, -bd], [0, -1, 0]);
    }
    ...
    const dz = bd - td;
    // +X face
    pushQuad(positions, normals, uvs, [bw, y0, -bd], [bw, y0, bd], [tw, y1, td], [tw, y1, -td], [dx, capH, 0]);
    // -X face
    pushQuad(positions, normals, uvs, [-bw, y0, bd], [-bw, y0, -bd], [-tw, y1, -td], [-tw, y1, td], [-dx, capH, 0]);
    // +Z face
    pushQuad(positions, normals, uvs, [bw, y0, bd], [-bw, y0, bd], [-tw, y1, td], [tw, y1, td], [0, capH, dz]);
    // -Z face
    pushQuad(positions, normals, uvs, [-bw, y0, -bd], [bw, y0, -bd], [tw, y1, -td], [-tw, y1, -td], [0, capH, -dz]);
    ...
    pushQuad(positions, normals, uvs, [-tw, y1, td], [-tw, y1, -td], [tw, y1, -td], [tw, y1, td], [0, 1, 0]);
    return buildBufferGeometry(positions, normals, uvs);
}
...
function buildDomeStyleShape(node) {
...
function addBand(positions, normals, uvs, rA, rB, lng, hintFn) {
    for (let j = 0; j < lng; j++) {
        const a = rA[j];
        const b = rA[j + 1];
        const c = rB[j + 1];
...
function pushQuadOriented(positions, normals, uvs, a, b, c, d, hint) {
...
// ─── Helpers ─────────────────────────────────────────────────────────
function buildBufferGeometry(positions, normals, uvs) {
...
function pushQuad(positions, normals, uvs, a, b, c, d, n) {
...
function pushTri(positions, normals, uvs, a, b, c, n) {
    const nLen = Math.sqrt(n[0] * n[0] + n[1] * n[1] + n[2] * n[2]) || 1;
    const nx = n[0] / nLen;
    const ny = n[1] / nLen;
    const nz = n[2] / nLen;
 ...
 * source of truth.
 */
export function computeBoxVentSlopeTilt(segment, localZ) {
    if (!segment || segment.roofType === 'flat' || localZ === 0)
        return 0;
