...
import { emitter, sceneRegistry, useLiveTransforms, useScene, } from '@pascal-app/core';
import { CursorSphere, markToolCancelConsumed, triggerSFX, useEditor } from '@pascal-app/editor';
import { useViewer } from '@pascal-app/viewer';
import { useCallback, useEffect, useRef, useState } from 'react';
import * as THREE from 'three';
const Y_AXIS = new THREE.Vector3(0, 1, 0);
export function MoveBuildingContent({ node }) {
    const previousGridPosRef = useRef(null);
    const dragAnchorRef = useRef(null);
    // Stable refs so the effect never needs node in its dependency array
    const nodeIdRef = useRef(node.id);
    const originalPositionRef = useRef([...node.position]);
    const originalRotationRef = useRef(node.rotation[1] ?? 0);
    const pendingRotationRef = useRef(node.rotation[1] ?? 0);
    ...
    const centerOffsetLocalRef = useRef(new THREE.Vector3());
    ...
    useEffect(() => {
        ...
        const offsetAt = (rotationY) => offsetWork.copy(centerOffsetLocalRef.current).applyAxisAngle(Y_AXIS, rotationY);
        ...
        const publishLive = (posX, posZ, rotY) => {
        ...
        const onGridClick = (event) => {
            ...
            useViewer.getState().setSelection({ buildingId: nodeId });
            exitMoveMode();
    ...
    return (_jsx("group", { children: _jsx(CursorSphere, { position: cursorWorldPos, showTooltip: false }) }));
}
