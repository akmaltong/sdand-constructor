 ...
 */
export const CeilingBoundaryEditor = ({ ceilingId, }) => {
    ...
    if (!ceiling?.polygon || ceiling.polygon.length < 3)
        return null;
    return (_jsx(PolygonEditor, { allowEdgeMove: true, color: "#d4d4d4", levelId: resolveLevelId(ceiling, useScene.getState().nodes), minVertices: 3, onPolygonChange: handlePolygonChange, onPolygonPreview: handlePolygonPreview, polygon: ceiling.polygon, surfaceHeight: ceiling.height ?? 2.5 }));
};
