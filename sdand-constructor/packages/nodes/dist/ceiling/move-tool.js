...
import { collectAlignmentAnchors, emitter, polygonAnchors, resolveAlignment, sceneRegistry, useLiveTransforms, useScene, } from '@pascal-app/core';
...
function setMeshOffset(id, deltaX, deltaZ, height) {
    ...
    if (mesh)
        mesh.position.set(deltaX, height - 0.01, deltaZ);
}
/**
...
export const MoveCeilingTool = ({ node }) => {
    ...
    useEffect(() => {
        ...
        const applyPreview = (deltaX, deltaZ) => {
            ...
            setMeshOffset(ceilingId, deltaX, deltaZ, height);
        ...
        const clearPreview = () => {
            const mesh = sceneRegistry.nodes.get(ceilingId);
            if (mesh) {
                mesh.position.x = 0;
                mesh.position.z = 0;
    ...
    return (_jsx(CeilingMovePreview, { ceilingId: node.id, cursorLocalPos: cursorLocalPos, height: node.height ?? 2.5, originalHoles: originalHolesRef.current, originalPolygon: originalPolygonRef.current }));
};
/**
 ...
 */
function CeilingMovePreview({ ceilingId, cursorLocalPos, height, originalHoles, originalPolygon, }) {
    const live = useLiveTransforms((s) => s.get(ceilingId));
    const dx = live?.position[0] ?? 0;
    const dz = live?.position[2] ?? 0;
    const previewPolygon = useMemo(() => originalPolygon.map(([x, z]) => [x + dx, z + dz]), [originalPolygon, dx, dz]);
    const previewHoles = useMemo(() => originalHoles.map((hole) => hole.map(([x, z]) => [x + dx, z + dz])), [originalHoles, dx, dz]);
    const previewFillGeometry = useMemo(() => createCeilingPreviewGeometry(previewPolygon, previewHoles), [previewPolygon, previewHoles]);
    const previewOutlineGeometry = useMemo(() => createCeilingOutlineGeometry(previewPolygon), [previewPolygon]);
    return (_jsxs("group", { children: [_jsx("mesh", { geometry: previewFillGeometry, position: [0, height + 0.012, 0], children: _jsx("meshBasicMaterial", { color: "#f5f5f4", depthWrite: false, opacity: 0.3, side: DoubleSide, transparent: true }) }), _jsx("line", { geometry: previewOutlineGeometry, position: [0, height + 0.02, 0], children: _jsx("lineBasicMaterial", { color: "#ffffff", depthWrite: false, opacity: 0.95, transparent: true }) }), _jsx(CursorSphere, { position: cursorLocalPos, showTooltip: false })] }));
}
function createCeilingPreviewGeometry(polygon, holes) {
    if (polygon.length < 3)
...
function createCeilingOutlineGeometry(polygon) {
