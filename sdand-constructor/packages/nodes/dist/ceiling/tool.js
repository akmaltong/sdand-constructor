...
import { collectAlignmentAnchors, emitter, resolveAlignment, useScene, } from '@pascal-app/core';
import { CursorSphere, EDITOR_LAYER, markToolCancelConsumed, triggerSFX, useAlignmentGuides, useEditor, } from '@pascal-app/editor';
...
function calculateSnapPoint(lastPoint, currentPoint) {
...
function commitCeilingDrawing(levelId, points) {
...
export const CeilingTool = () => {
    const cursorRef = useRef(null);
    const gridCursorRef = useRef(null);
    const mainLineRef = useRef(null);
    const closingLineRef = useRef(null);
    const groundMainLineRef = useRef(null);
    const groundClosingLineRef = useRef(null);
    const verticalLineRef = useRef(null);
    const currentLevelId = useViewer((s) => s.selection.levelId);
    const setSelection = useViewer((s) => s.setSelection);
    const [points, setPoints] = useState([]);
    const [cursorPosition, setCursorPosition] = useState([0, 0]);
    const [snappedCursorPosition, setSnappedCursorPosition] = useState([0, 0]);
    const [levelY, setLevelY] = useState(0);
    const previousSnappedPointRef = useRef(null);
    ...
    const verticalGeo = useMemo(() => new BufferGeometry().setFromPoints([
        new Vector3(0, 0, 0),
        new Vector3(0, CEILING_HEIGHT - GRID_OFFSET, 0),
    ]), []);
    const gradientOpacityNode = useMemo(() => mix(0.6, 0.0, positionLocal.y.div(CEILING_HEIGHT - GRID_OFFSET).clamp()), []);
    useEffect(() => {
        if (!currentLevelId)
            return;
        ...
        // bypasses.
        const alignPoint = (fallback, raw, bypass) => {
        ...
        const onGridDoubleClick = (_event) => {
            if (!currentLevelId)
                return;
            if (points.length >= 3) {
                const ceilingId = commitCeilingDrawing(currentLevelId, points);
    ...
    useEffect(() => {
        ...
        const linePoints = points.map(([x, z]) => new Vector3(x, ceilingY, z));
        linePoints.push(new Vector3(snappedCursor[0], ceilingY, snappedCursor[1]));
        const gridY = levelY + GRID_OFFSET;
        const groundLinePoints = points.map(([x, z]) => new Vector3(x, gridY, z));
    ...
    return (_jsxs("group", { children: [_jsx(CursorSphere, { ref: cursorRef }), _jsxs("mesh", { layers: EDITOR_LAYER, ref: gridCursorRef, renderOrder: 2, rotation: [-Math.PI / 2, 0, 0], children: [_jsx("ringGeometry", { args: [0.15, 0.2, 32] }), _jsx("meshBasicMaterial", { color: "#818cf8", depthTest: false, depthWrite: true, opacity: 0.5, side: DoubleSide, transparent: true })] }), _jsx("line", { geometry: verticalGeo, layers: EDITOR_LAYER, ref: verticalLineRef, renderOrder: 1, children: _jsx("lineBasicNodeMaterial", { color: "#818cf8", depthTest: false, depthWrite: false, opacityNode: gradientOpacityNode, transparent: true }) }), previewShape && (_jsxs("mesh", { frustumCulled: false, layers: EDITOR_LAYER, position: [0, levelY + CEILING_HEIGHT, 0], rotation: [-Math.PI / 2, 0, 0], children: [_jsx("shapeGeometry", { args: [previewShape] }), _jsx("meshBasicMaterial", { color: "#818cf8", depthTest: false, opacity: 0.15, side: DoubleSide, transparent: true })] })), previewShape && (_jsxs("mesh", { frustumCulled: false, layers: EDITOR_LAYER, position: [0, levelY + GRID_OFFSET, 0], rotation: [-Math.PI / 2, 0, 0], children: [_jsx("shapeGeometry", { args: [previewShape] }), _jsx("meshBasicMaterial", { color: "#818cf8", depthTest: false, opacity: 0.1, side: DoubleSide, transparent: true })] })), _jsxs("line", { frustumCulled: false, layers: EDITOR_LAYER, 
                // @ts-expect-error
                ref: mainLineRef, renderOrder: 1, visible: false, children: [_jsx("bufferGeometry", {}), _jsx("lineBasicNodeMaterial", { color: "#818cf8", depthTest: false, depthWrite: false, linewidth: 3 })] }), _jsxs("line", { frustumCulled: false, layers: EDITOR_LAYER, 
                // @ts-expect-error
                ref: closingLineRef, renderOrder: 1, visible: false, children: [_jsx("bufferGeometry", {}), _jsx("lineBasicNodeMaterial", { color: "#818cf8", depthTest: false, depthWrite: false, linewidth: 2, opacity: 0.5, transparent: true })] }), _jsxs("line", { frustumCulled: false, layers: EDITOR_LAYER, 
                // @ts-expect-error
                ref: groundMainLineRef, renderOrder: 1, visible: false, children: [_jsx("bufferGeometry", {}), _jsx("lineBasicNodeMaterial", { color: "#818cf8", depthTest: false, depthWrite: false, linewidth: 3, opacity: 0.3, transparent: true })] }), _jsxs("line", { frustumCulled: false, layers: EDITOR_LAYER, 
                // @ts-expect-error
                ref: groundClosingLineRef, renderOrder: 1, visible: false, children: [_jsx("bufferGeometry", {}), _jsx("lineBasicNodeMaterial", { color: "#818cf8", depthTest: false, depthWrite: false, linewidth: 2, opacity: 0.15, transparent: true })] }), points.map(([x, z], index) => (_jsx(CursorSphere, { color: "#818cf8", position: [x, levelY + CEILING_HEIGHT + 0.01, z], showTooltip: false }, index)))] }));
};
