import { ChimneyNode as ChimneyNodeSchema, getActiveRoofHeight, } from '@pascal-app/core';
...
function resolveHostSegment(node, sceneApi) {
    if (!node.roofSegmentId)
        return undefined;
    return sceneApi.get(node.roofSegmentId);
}
...
// the eave.
function getBodyMidY(node, segment) {
...
function getBodyTopY(node, segment) {
...
function getCapBaseY(node, segment) {
...
function getCapTopY(node, segment) {
    if (!node.cap || node.capShape === 'none')
        return getBodyTopY(node, segment);
...
function chimneyWidthHandle(side) {
...
function chimneyHeightAboveRidgeHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
// follows the pointer.
function chimneyFlueHeightHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
// follows the pointer.
function chimneyCapThicknessHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
function chimneyCapOverhangHandle() {
    return {
        kind: 'radial-resize',
        axis: 'x',
        portal: 'grandparent',
...
const chimneyHandles = (node) => {
    const descriptors = [
        chimneyWidthHandle('right'),
        chimneyWidthHandle('left'),
    ];
    if (node.bodyShape !== 'round')
        descriptors.push(chimneyDepthHandle());
 ...
 */
export const chimneyDefinition = {
    kind: 'chimney',
    schemaVersion: 1,
    schema: ChimneyNode,
    category: 'structure',
    ...
    mcp: {
        description: 'A chimney on a roof segment. Square or round body; optional shoulder taper; sloped/flat/stepped cap; up to 4 protruding flues; optional cricket on the up-slope face.',
    },
};
