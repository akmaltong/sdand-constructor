 ...
 */
export function carveChimneyHoles(body, cap, flues, node, segment) {
    ...
    const subtractFrom = (base, yBot, yTop) => {
        if (specs.length === 0)
            return base;
    ...
    const wantPanels = node.panelStyle !== 'none' && !isRound && node.panelDepth > 0 && node.panelHeight > 0.01;
    if (wantPanels) {
        const panelCutters = buildPanelCutters(node, topY);
        if (panelCutters.length > 0) {
            newBody = subtractCutters(newBody, panelCutters);
            for (const cutter of panelCutters)
                cutter.geometry.dispose();
        }
    }
    ...
    if (flues && useFlueHoles) {
        const xs = flueXPositions(flueCount, node.width, flueDiameter, node.flueSpacing);
        const flueShape = node.flueShape ?? 'round';
        const cutters = xs.map((x) => buildCutter(node, { shape: flueShape, sizeX: flueInner, sizeZ: flueInner, xCenter: x }, capTopY - 0.02, capTopY + flueHeight + 0.02));
    ...
    if (newCap)
        partitionTopFaceGroups(newCap, capTopY - 0.005);
    if (newFlues)
        partitionTopFaceGroups(newFlues, capTopY + flueHeight - 0.005);
 ...
 */
export function partitionTopFaceGroups(geo, topYMin) {
    ...
    for (let i = 0; i < otherTris.length; i++)
        newArr[i] = otherTris[i];
    for (let i = 0; i < topTris.length; i++)
        newArr[otherTris.length + i] = topTris[i];
    geo.setIndex(new THREE.BufferAttribute(newArr, 1));
    geo.clearGroups();
    if (otherTris.length > 0)
        geo.addGroup(0, otherTris.length, 0);
    if (topTris.length > 0)
        geo.addGroup(otherTris.length, topTris.length, 1);
}
/**
 ...
 */
function buildPanelCutters(node, topY) {
    const w = node.width;
    const d = node.depth;
...
function buildCutter(node, spec, yBot, yTop) {
    const h = Math.max(0.02, yTop - yBot);
    const midY = (yTop + yBot) / 2;
    const geo = spec.shape === 'round'
...
function subtractCutters(base, cutters) {
    if (cutters.length === 0)
        return base;
    const indexed = mergeVertices(base, 1e-4);
    if (!indexed.getAttribute('normal'))
        indexed.computeVertexNormals();
    const ic = indexed.getIndex()?.count ?? 0;
    indexed.clearGroups();
    if (ic > 0)
        indexed.addGroup(0, ic, 0);
    ...
    try {
        for (const cutter of cutters) {
            const next = csgEvaluator.evaluate(current, cutter, SUBTRACTION);
            prepareBrushForCSG(next);
            if (current !== baseBrush)
        ...
        out.clearGroups();
        if (idx > 0)
            out.addGroup(0, idx, 0);
        else
            out.addGroup(0, out.getAttribute('position').count, 0);
