 ...
 */
export function resolveChimneyRole(materialIndex) {
    return materialIndex === 1 ? 'top' : 'body';
}
export function buildChimneyMaterialPatch(role, material, materialPreset) {
    if (role === 'top') {
        return { topMaterial: material, topMaterialPreset: materialPreset };
    }
    return { material, materialPreset };
}
export function getEffectiveChimneyMaterial(node, role) {
    if (role === 'top') {
 ...
 * 'body' role paints.
 */
function applyChimneyPreview(role, previewMaterial, root) {
    const restores = [];
 ...
 */
export const chimneyPaint = {
    resolveRole: ({ materialIndex }) => resolveChimneyRole(materialIndex),
    buildPatch: ({ role, material, materialPreset }) => buildChimneyMaterialPatch(role, material, materialPreset),
    applyPreview: ({ role, material, materialPreset, root }) => {
        const previewMaterial = buildPreviewMaterial(material, materialPreset);
        if (!previewMaterial)
            return null;
        return applyChimneyPreview(role, previewMaterial, root);
    },
    getEffectiveMaterial: ({ node, role }) => getEffectiveChimneyMaterial(node, role),
};
