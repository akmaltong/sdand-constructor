...
import { createMaterial, createMaterialFromPresetRef, createSurfaceRoleMaterial, getRoofSegmentBrushes, useNodeEvents, useViewer, } from '@pascal-app/viewer';
 ...
 */
const ChimneyRenderer = ({ node: storeNode }) => {
    ...
    const overrides = useLiveNodeOverrides((state) => state.get(storeNode.id));
    const node = useMemo(() => (overrides ? { ...storeNode, ...overrides } : storeNode), [storeNode, overrides]);
    ...
    const segmentBrushes = useMemo(() => (segment ? getRoofSegmentBrushes(segment) : null), [
        segment?.roofType,
        segment?.width,
        segment?.depth,
        segment?.wallHeight,
    ...
    const fallbackBodyMaterial = useMemo(() => new THREE.MeshStandardMaterial({
        color: 0xb8_88_72,
        roughness: 0.85,
        metalness: 0,
    }), []);
    const fallbackTopMaterial = useMemo(() => new THREE.MeshStandardMaterial({
        color: 0xa0_a0_a0,
        roughness: 0.75,
        metalness: 0,
    }), []);
    ...
    const capSurfaceMaterial = useMemo(() => {
        ...
        if (node.topMaterial)
            return createMaterial(node.topMaterial, shading);
    ...
    // renders.
    const surfaceArray = useMemo(() => [surfaceMaterial, capSurfaceMaterial], [surfaceMaterial, capSurfaceMaterial]);
    if (!segment || !geo)
        return null;
    ...
    return (_jsx("group", { position: segment.position, "rotation-y": segment.rotation, visible: node.visible, ...handlers, children: _jsxs("group", { position: [node.position[0] ?? 0, 0, node.position[2] ?? 0], ref: ref, "rotation-y": node.rotation ?? 0, children: [_jsx("mesh", { castShadow: true, geometry: trimmedBody ?? geo.body, material: surfaceArray, name: "chimney-body", receiveShadow: true }), geo.cap && (_jsx("mesh", { castShadow: true, geometry: geo.cap, material: surfaceArray, name: "chimney-cap", receiveShadow: true })), geo.flues && (_jsx("mesh", { castShadow: true, geometry: geo.flues, material: surfaceArray, name: "chimney-flues", receiveShadow: true })), geo.cricket && (_jsx("mesh", { castShadow: true, geometry: geo.cricket, material: surfaceMaterial, name: "chimney-cricket", receiveShadow: true })), geo.bands && (_jsx("mesh", { castShadow: true, geometry: geo.bands, material: surfaceMaterial, name: "chimney-bands", receiveShadow: true }))] }) }));
};
