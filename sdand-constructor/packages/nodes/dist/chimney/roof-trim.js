 ...
 */
export function trimChimneyBodyAgainstRoof(body, segment, node, segBrushes) {
    ...
    if (hasRotation)
        indexed.rotateY(node.rotation);
    indexed.translate(posX, 0, posZ);
    const indexCount = indexed.getIndex()?.count ?? 0;
    indexed.clearGroups();
    if (indexCount > 0)
        indexed.addGroup(0, indexCount, 0);
    indexed.computeBoundsTree?.({ maxLeafSize: 10 });
    ...
    try {
        ...
        const step1 = csgEvaluator.evaluate(chimneyBrush, wallBrush, SUBTRACTION);
        prepareBrushForCSG(step1);
        const step2 = csgEvaluator.evaluate(step1, shinSlab, SUBTRACTION);
