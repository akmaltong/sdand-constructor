import { ColumnNode as ColumnNodeSchema, } from '@pascal-app/core';
...
function columnAxisHandle(axis) {
    return {
        kind: 'linear-resize',
        axis,
        anchor: 'center',
...
function columnBraceBottomSpreadHandle() {
    return {
        kind: 'linear-resize',
        axis: 'x',
        anchor: 'center',
        min: MIN_BRACE_BOTTOM_SPREAD,
        currentValue: (n) => Math.max(MIN_BRACE_BOTTOM_SPREAD, n.braceBottomSpread ?? Math.max(n.width * 3, 1.2)),
        apply: (_n, newValue) => ({ braceBottomSpread: newValue }),
        placement: {
            position: (n) => {
                const spread = Math.max(MIN_BRACE_BOTTOM_SPREAD, n.braceBottomSpread ?? Math.max(n.width * 3, 1.2));
...
function columnBraceTopSpreadHandle() {
    return {
        kind: 'linear-resize',
        axis: 'x',
        anchor: 'center',
...
function columnBraceHandle(axis) {
    return {
        kind: 'linear-resize',
        axis,
        anchor: 'center',
...
const STYLES_WITH_BOTTOM_SPREAD = new Set(['a-frame']);
const STYLES_WITH_TOP_SPREAD = new Set([
    'a-frame',
    'y-frame',
    'v-frame',
]);
...
function columnFootprintHalf(n) {
    ...
    return {
        halfX: Math.max(n.width, n.braceWidth ?? 0, n.braceBottomSpread ?? 0, n.braceTopSpread ?? 0) / 2,
...
function columnHandles(node) {
    ...
    const handles = [columnHeightHandle()];
...
export const columnDefinition = {
    ...
    surfaceRole: 'wall',
    defaults: () => {
        const stub = ColumnNodeSchema.parse({ id: 'column_default', type: 'column' });
    ...
    capabilities: {
        ...
        floorPlaced: {
            footprint: (node) => {
                const column = node;
                return {
                    dimensions: [column.width, column.height, column.depth],
