...
const ROUND_CROSS_SECTIONS = new Set([
    'round',
    'octagonal',
    'sixteen-sided',
]);
 ...
 */
export function buildColumnFloorplan(node, ctx) {
    ...
    const points = polygon.map((p) => [p.x, p.y]);
    const children = [
        {
            kind: 'polygon',
            points,
    ...
    if (isSelected) {
        ...
        const emitArrowAlong = (dim, localDirection, localOffsetDistance) => {
            ...
            const localPos = localDirection === 'x' ? [localOffsetDistance, 0] : [0, localOffsetDistance];
            ...
            const outwardLocal = localDirection === 'x' ? [1, 0] : [0, 1];
            ...
            children.push({
                ...
                payload: { dim, planAxis: [planAxisX, planAxisY] },
            });
        };
        if (node.supportStyle !== 'vertical') {
            ...
            const halfBraceX = Math.max(node.width, node.braceWidth ?? 0, node.braceBottomSpread ?? 0, node.braceTopSpread ?? 0) / 2;
 ...
 */
function columnPlanHalfExtents(column) {
    if (column.supportStyle !== 'vertical') {
        return {
            halfX: Math.max(column.width, column.braceWidth ?? 0, column.braceBottomSpread ?? 0, column.braceTopSpread ?? 0) / 2,
...
function rotatePlanVector(x, y, rotation) {
    const c = Math.cos(rotation);
    const s = Math.sin(rotation);
    return [x * c - y * s, x * s + y * c];
}
function getRotatedRectanglePolygon(center, width, depth, rotation) {
    const halfW = width / 2;
    const halfD = depth / 2;
    const corners = [
        [-halfW, -halfD],
...
function getColumnPlanFootprint(column) {
    ...
    if (column.supportStyle === 'a-frame' ||
        ...
        const width = Math.max(column.supportStyle === 'a-frame' ||
            column.supportStyle === 'x-brace' ||
            column.supportStyle === 'k-brace' ||
            column.supportStyle === 'single-strut' ||
            column.supportStyle === 'tripod' ||
            ...
            (column.supportStyle === 'y-frame' ||
                ...
                column.supportStyle === 'box-frame'
                ? 1
                : 0), (column.braceWidth ?? column.width) * 2);
        const depth = Math.max(column.supportStyle === 'tripod' ||
            column.supportStyle === 'trestle' ||
            column.supportStyle === 'box-frame'
            ? (column.braceTopSpread ?? 1)
            : 0, column.braceDepth ?? column.depth, 0.08);
    ...
    const isRound = column.crossSection === 'round' ||
        column.crossSection === 'octagonal' ||
        column.crossSection === 'sixteen-sided';
    const shaftWidth = isRound ? column.radius * 2 : column.width;
    const shaftDepth = isRound ? column.radius * 2 : column.depth;
    const width = Math.max(shaftWidth, column.width * column.baseWidthScale, column.width * column.capitalWidthScale);
    const depth = Math.max(shaftDepth, column.depth * column.baseDepthScale, column.depth * column.capitalDepthScale);
    if (column.crossSection === 'square' || column.crossSection === 'rectangular') {
        return getRotatedRectanglePolygon(center, width, depth, -column.rotation);
    }
    const segmentCount = column.crossSection === 'octagonal' ? 8 : column.crossSection === 'sixteen-sided' ? 16 : 32;
