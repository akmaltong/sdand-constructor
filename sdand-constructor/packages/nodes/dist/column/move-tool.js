...
import { ColumnNode as ColumnNodeSchema, collectAlignmentAnchors, emitter, movingFootprintAnchors, resolveAlignment, sceneRegistry, useLiveTransforms, useScene, } from '@pascal-app/core';
import { CursorSphere, commitFreshPlacementSubtree, DragBoundingBox, getFloorStackPreviewPosition, markToolCancelConsumed, resolvePlanarCursorPosition, stripPlacementMetadataFlags, triggerSFX, useAlignmentGuides, useEditor, useFreshPlacementVisibility, } from '@pascal-app/editor';
...
function MoveColumnTool({ node }) {
    const [previewPosition, setPreviewPosition] = useState(node.position);
    const [previewRotation, setPreviewRotation] = useState(node.rotation);
    const { isFreshPlacement, previewVisible, revealFreshPlacement, useAbsoluteCursorPlacement } = useFreshPlacementVisibility({ node });
    ...
    useEffect(() => {
        ...
        const getVisualPosition = (position, rotation = rotationY) => getFloorStackPreviewPosition({
            node,
            position,
            rotation,
        ...
        const onGridClick = (event) => {
            ...
            if (nodeId && useScene.getState().nodes[nodeId]) {
                ...
                if (isNew) {
                    const finalId = commitFreshPlacementSubtree(nodeId, data);
                    if (finalId) {
                        committed = true;
                        committedId = finalId;
                    }
                ...
                committed = true;
                useScene.temporal.getState().resume();
                useScene.getState().createNode(column, node.parentId);
            }
    ...
    if (!previewVisible)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx(CursorSphere, { color: "#a78bfa", height: node.height, position: previewPosition }), _jsx(DragBoundingBox, { fallbackSize: [node.width, node.height, node.depth], nodeId: node.id, position: previewPosition, rotationY: previewRotation })] }));
}
