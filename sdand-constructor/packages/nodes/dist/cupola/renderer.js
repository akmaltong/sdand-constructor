 ...
 */
const CupolaRenderer = ({ node: storeNode }) => {
    ...
    const segment = useScene((state) => node.roofSegmentId
        ...
        : undefined);
    const geometry = useMemo(() => buildCupolaGeometry(node), [node.width, node.depth, node.height, node.roofStyle, node.finial]);
    ...
    return (_jsx("group", { position: segPos, "rotation-y": segRotY, children: _jsx("group", { position: [node.position[0] ?? 0, node.position[1] ?? 0, node.position[2] ?? 0], quaternion: composedQuat, ref: ref, visible: node.visible, children: _jsx("mesh", { castShadow: true, geometry: geometry, material: material, name: "cupola-surface", receiveShadow: true, ...handlers }) }) }));
};
