...
function readWallLength(door, scene) {
    if (!door.wallId)
        return Number.POSITIVE_INFINITY;
    const wall = scene.get(door.wallId);
    if (!wall)
        return Number.POSITIVE_INFINITY;
    return Math.hypot(wall.end[0] - wall.start[0], wall.end[1] - wall.start[1]);
}
function readWallHeight(door, scene) {
    if (!door.wallId)
        return Number.POSITIVE_INFINITY;
    const wall = scene.get(door.wallId);
...
export const doorDefinition = {
    ...
    defaults: () => {
        const stub = DoorNode.parse({ id: 'door_default', type: 'door' });
