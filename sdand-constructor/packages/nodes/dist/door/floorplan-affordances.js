 ...
 */
export const doorWidthAffordance = {
    start({ node, payload, nodes, initialPlanPoint }) {
        const { side } = payload;
        const doorId = node.id;
        ...
        const anchorX = side === 'end' ? initialDoorX - initialWidth / 2 : initialDoorX + initialWidth / 2;
        // Wall axis (level-local) for projecting pointer movement to a
        // wall-local X delta.
        const wallStart = wall ? wall.start : [0, 0];
        const wallEnd = wall ? wall.end : [1, 0];
        ...
        const projectToWallLocalX = (planPoint) => {
        ...
        return {
            ...
            apply({ planPoint }) {
                ...
                const newWidth = Math.min(Math.max(MIN_DOOR_WIDTH, initialWidth + delta), Math.max(MIN_DOOR_WIDTH, maxWidth));
