 ...
 */
export function buildDoorFloorplan(node, ctx) {
    const wall = ctx.parent;
    ...
    const drawSwingLeaf = (hX, hZ, closedX, closedZ, signedAngle) => {
    ...
    if (isOpening) {
        ...
        const panelRect = (centerAlong, perpSign) => {
        ...
        const pushPanel = (points) => children.push({
            kind: 'polygon',
            points,
            fill: fillColor,
            stroke: accentColor,
        ...
        const rectPoints = (centerAlong, halfLen, halfThick) => {
        ...
        const frontRect = (centerAlong, halfLen) => {
        ...
        if (isDoubleLeaf) {
            ...
            const hingeTangentSign = hingesSide === 'left' ? 1 : -1;
            drawSwingLeaf(cx - dirX * halfWidth * hingeTangentSign, cz - dirZ * halfWidth * hingeTangentSign, dirX * width * hingeTangentSign, dirZ * width * hingeTangentSign, swingAngle * swingSign * hingeTangentSign);
        }
    }
 ...
 */
function isOpeningPlanFlipped(rotation) {
    const normalized = ((((rotation[1] % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2)) + 1e-6) % (Math.PI * 2);
