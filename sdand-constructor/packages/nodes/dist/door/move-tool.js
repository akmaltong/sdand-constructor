...
import { collectAlignmentAnchors, DoorNode, emitter, isCurvedWall, sceneRegistry, spatialGridManager, useLiveTransforms, useScene, } from '@pascal-app/core';
import { calculateCursorRotation, calculateItemRotation, EDITOR_LAYER, getSideFromNormal, isValidWallSideFace, stripPlacementMetadataFlags, triggerSFX, useAlignmentGuides, useEditor, } from '@pascal-app/editor';
...
const MoveDoorTool = ({ node: movingDoorNode }) => {
    ...
    useEffect(() => {
        ...
        const meta = typeof movingDoorNode.metadata === 'object' && movingDoorNode.metadata !== null
            ? movingDoorNode.metadata
            : {};
        const isNew = !!meta.isNew;
        const original = {
            position: [...movingDoorNode.position],
            rotation: [...movingDoorNode.rotation],
        ...
        let currentWallId = movingDoorNode.parentId;
        let dragAnchor = null;
        let lastTarget = null;
        ...
        const alignmentCandidates = collectAlignmentAnchors(useScene.getState().nodes, movingDoorNode.id);
        ...
        const getPlacementOrientation = (event) => {
            ...
            return {
                ...
                cursorRotation: calculateCursorRotation(event.normal, event.node.start, event.node.end) + rotationOffset,
            };
        };
        const resolveMoveTarget = (event) => {
            ...
            const { clampedX, clampedY } = clampToWall(event.node, localX, movingDoorNode.width, movingDoorNode.height);
            const valid = !hasWallChildOverlap(event.node.id, clampedX, clampedY, movingDoorNode.width, movingDoorNode.height, movingDoorNode.id);
            return {
                wallNode: event.node,
                wallId: event.node.id,
                side,
        ...
        const applyPreview = (target) => {
            if (currentWallId !== target.wallId) {
                ...
                const doorMesh = sceneRegistry.nodes.get(movingDoorNode.id);
                if (doorMesh) {
        ...
        const onWallClick = (event) => {
            ...
            if (isNew) {
                ...
                const cloned = structuredClone(movingDoorNode);
                delete cloned.id;
        ...
        return () => {
            const current = useScene.getState().nodes[movingDoorNode.id];
    ...
    const edgesGeo = useMemo(() => {
        const boxGeo = new BoxGeometry(movingDoorNode.width, movingDoorNode.height, movingDoorNode.frameDepth ?? 0.07);
