import { getActiveRoofHeight, getPitchFromActiveRoofHeight, ROOF_SHAPE_DEFAULTS, } from '@pascal-app/core';
import { ADDITION, Brush, computeGeometryBoundsTree, csgEvaluator, csgGeometry, csgMaterials, getRoofSegmentBrushes, mapRoofGroupMaterialIndex, prepareBrushForCSG, remapRoofShellFaces, roofCsgDummyMats, SUBTRACTION, } from '@pascal-app/viewer';
...
function dormerSkirtHeight(dormer) {
 ...
 */
export function buildDormerFallbackGeometry(dormer) {
...
export function createDormerArchShape(w, h, archHeight) {
    const hw = w / 2;
    const hh = h / 2;
...
export function normalizeDormerCornerRadii(radii, w, h) {
    const r = radii.map((v) => Math.max(v, 0));
    const scale = Math.min(1, Math.max(w, 0) / Math.max(r[0] + r[1], 1e-6), Math.max(w, 0) / Math.max(r[3] + r[2], 1e-6), Math.max(h, 0) / Math.max(r[0] + r[3], 1e-6), Math.max(h, 0) / Math.max(r[1] + r[2], 1e-6));
    if (scale >= 1)
        return r;
    return r.map((v) => v * scale);
}
export function createDormerRoundedShape(w, h, radii) {
    const hw = w / 2;
    const hh = h / 2;
    ...
    if (br > 0)
        shape.absarc(hw - br, -hh + br, br, -Math.PI / 2, 0, false);
    else
        shape.lineTo(hw, -hh);
    shape.lineTo(hw, hh - tr);
    if (tr > 0)
        shape.absarc(hw - tr, hh - tr, tr, 0, Math.PI / 2, false);
    else
        shape.lineTo(hw, hh);
    shape.lineTo(-hw + tl, hh);
    if (tl > 0)
        shape.absarc(-hw + tl, hh - tl, tl, Math.PI / 2, Math.PI, false);
    else
        shape.lineTo(-hw, hh);
    shape.lineTo(-hw, -hh + bl);
    if (bl > 0)
        shape.absarc(-hw + bl, -hh + bl, bl, Math.PI, (3 * Math.PI) / 2, false);
    else
        shape.lineTo(-hw, -hh);
    shape.closePath();
    return shape;
}
function resolveDormerRadii(dormer, w, h) {
    return normalizeDormerCornerRadii(dormer.windowCornerRadii, w, h);
}
function createDormerWindowCutGeometry(dormer, w, h, depth) {
 ...
 */
export function getDormerExposedFaces(dormer, hostSegment) {
 ...
 */
export function getDormerSkirtWindowDims(dormer) {
    const skirtH = dormerSkirtHeight(dormer);
 ...
 */
export function generateDormerGeometry(dormer, hostSegment) {
    ...
    let dormerSolid = null;
    let hostSolid = null;
    try {
        const hollowWall = csgEvaluator.evaluate(dormerBrushes.wallBrush, dormerBrushes.innerBrush, SUBTRACTION);
        const shinDeck = csgEvaluator.evaluate(dormerBrushes.shinSlab, dormerBrushes.deckSlab, ADDITION);
        dormerSolid = csgEvaluator.evaluate(shinDeck, hollowWall, ADDITION);
        hollowWall.geometry.dispose();
        shinDeck.geometry.dispose();
        const bakeMatrix = new THREE.Matrix4().compose(new THREE.Vector3(0, -skirt, 0), new THREE.Quaternion().setFromAxisAngle(_yAxis, yawBake), _scale);
        ...
        if (hostBrushes) {
            const wallPlusDeck = csgEvaluator.evaluate(hostBrushes.wallBrush, hostBrushes.deckSlab, ADDITION);
            hostSolid = csgEvaluator.evaluate(wallPlusDeck, hostBrushes.shinSlab, ADDITION);
            ...
            const fullTrim = csgEvaluator.evaluate(hostSolid, groundBrush, ADDITION);
            ...
            const segToMesh = new THREE.Matrix4()
                .compose(new THREE.Vector3(dormer.position[0] ?? 0, dormer.position[1] ?? 0, dormer.position[2] ?? 0), new THREE.Quaternion().setFromAxisAngle(_yAxis, dormer.rotation), _scale)
                .invert();
            csgGeometry(hostSolid).applyMatrix4(segToMesh);
            prepareBrushForCSG(hostSolid);
            const trimmed = csgEvaluator.evaluate(dormerSolid, hostSolid, SUBTRACTION);
        ...
        const cutDepth = 0.4;
        const cutFace = (zSign) => {
            const cutGeo = createDormerWindowCutGeometry(dormer, skirtWin.width, skirtWin.height, cutDepth);
            ...
            const result = csgEvaluator.evaluate(dormerSolid, brush, SUBTRACTION);
            dormerSolid.geometry.dispose();
 ...
 */
export function buildDormerCutShape(roofType, innerW, innerD, skirt, wallH, roofH) {
    const hw = innerW / 2;
    const hd = innerD / 2;
    if (roofType === 'flat') {
    ...
    if (roofType === 'hip' || roofType === 'dutch' || roofType === 'mansard') {
        ...
        const ridgeA = longerIsZ
            ? [0, peakY, -ridgeHalfLen]
            : [-ridgeHalfLen, peakY, 0];
 ...
 */
export function buildDormerRoofCut(dormer) {
    ...
    if (dormer.width < 0.01 || dormer.depth < 0.01)
        return null;
    const skirt = dormerSkirtHeight(dormer);
 ...
 */
function splitDormerGableMaterial(geometry, wallHeight, gableMatIndex) {
    ...
    if (triangleCount === 0)
        return;
    const triangleMats = new Array(triangleCount).fill(0);
    ...
    let groupStart = 0;
    let curMat = triangleMats[sortedTri[0]];
    for (let i = 1; i < sortedTri.length; i++) {
        const mat = triangleMats[sortedTri[i]];
        if (mat !== curMat) {
