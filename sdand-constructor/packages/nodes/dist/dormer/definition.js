import { DormerNode as DormerNodeSchema, } from '@pascal-app/core';
...
function dormerWidthHandle(side) {
...
function dormerWallHeightHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
function dormerWallSkirtHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'max',
...
function dormerRoofHeightHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
// not useful there.
function getExposedFaceZSign(n, sceneApi) {
    if (!n.roofSegmentId)
        return 1;
    const segment = sceneApi.get(n.roofSegmentId);
    if (!segment)
        return 1;
...
function dormerWindowWidthHandle(side) {
    const sign = side === 'right' ? 1 : -1;
    return {
        kind: 'linear-resize',
        axis: 'x',
...
function dormerWindowHeightHandle(side) {
...
export const dormerDefinition = {
    ...
    capabilities: {
        ...
        roofAccessory: {
            buildCut: (node, _hostSegment) => buildDormerRoofCut(node),
        },
    ...
    mcp: {
        description: 'A dormer on a roof segment. Box body + gable roof + inlined window opening. Geometry beyond the stub silhouette coming later.',
    },
};
