 ...
 */
export function DormerPositionSection({ node, segment, roof, selectedId, previewProp, commitProp, }) {
    ...
    const worldXform = useMemo(() => {
        ...
        if (roof) {
            ...
            if (Number.isFinite(lo_x))
                bounds = { minX: lo_x, maxX: hi_x, minZ: lo_z, maxZ: hi_z };
        }
    ...
    return (_jsxs(PanelSection, { title: "Position", children: [_jsx(SliderControl, { label: "X", max: Math.round(worldMaxX * 10) / 10, min: Math.round(worldMinX * 10) / 10, onChange: (newWorldX) => {
                    if (!segment)
                        return;
                ...
                }, onCommit: (degrees) => commitWorldRotation((degrees * Math.PI) / 180), precision: 0, restoreOnCommit: false, step: 1, unit: "\u00B0", value: Math.round((worldRotation_now * 180) / Math.PI) })] }));
}
