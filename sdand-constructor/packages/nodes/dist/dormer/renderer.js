...
import { getEffectiveDormerSurfaceMaterial, useLiveNodeOverrides, useRegistry, useScene, } from '@pascal-app/core';
...
import { buildDormerFallbackGeometry, DORMER_GABLE_MATERIAL_INDEX, generateDormerGeometry, } from './csg-geometry';
import DormerWindowAssembly from './window-assembly';
const DormerRenderer = ({ node: storeNode }) => {
    ...
    const glassMat = useMemo(() => createSurfaceRoleMaterial('glazing', colorPreset, undefined, sceneTheme), [colorPreset, sceneTheme]);
    const geometry = useMemo(() => {
        if (!segment)
            return null;
        if (isLiveDrag)
            return buildDormerFallbackGeometry(node);
    ...
    return (_jsx("group", { position: segment.position, "rotation-y": segment.rotation ?? 0, visible: node.visible, ...handlers, children: _jsxs("group", { position: [node.position[0] ?? 0, node.position[1] ?? 0, node.position[2] ?? 0], ref: ref, "rotation-y": node.rotation ?? 0, children: [_jsx("mesh", { castShadow: true, geometry: geometry, material: material, name: "dormer-body", receiveShadow: true }), _jsx(DormerWindowAssembly, { frameMaterial: frameSideMat, glassMaterial: glassMat, node: node, segment: segment })] }) }));
};
