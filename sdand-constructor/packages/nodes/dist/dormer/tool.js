...
import { DormerNode, useScene } from '@pascal-app/core';
 ...
 */
function nextDormerNumber(nodes) {
    const used = new Set();
...
const DormerTool = () => {
    ...
    const previewNode = useMemo(() => DormerNode.parse({
    ...
    const { activeBuildingId, segmentXform, hitLocal, ghostRotation } = useDormerPlacement({
        onCommit: (hit, rotation) => {
            ...
            state.createNode(dormer, hit.segment.id);
