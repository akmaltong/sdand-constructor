...
export function useDormerPlacement(opts) {
    ...
    useEffect(() => {
        ...
        const roofDrag = relativeStartRef.current
            ...
            : null;
        let lastRelativeHit = null;
        const resolvePlacementHit = (event) => {
            if (roofDrag)
                return roofDrag.resolve(event);
            return resolveRoofSegmentHit(event.node, event.position[0], event.position[1], event.position[2]);
        };
