...
export function buildDormerWindowGeometries(winW, winH, ft, fd, cols, rows, dt, shape = 'rectangle', archHeight = 0.35, cornerRadii = [0.15, 0.15, 0.15, 0.15]) {
    ...
    if (shape === 'arch' || shape === 'rounded') {
        const insetRadii = cornerRadii.map((r) => Math.max(r - safeFt, 0));
        const outerShape = shape === 'arch'
            ? createDormerArchShape(winW, winH, archHeight)
            : createDormerRoundedShape(winW, winH, cornerRadii);
        const innerHole = shape === 'arch'
