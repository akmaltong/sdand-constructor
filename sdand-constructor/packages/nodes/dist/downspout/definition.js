import { DownspoutNode as DownspoutNodeSchema, useLiveNodeOverrides, useScene, } from '@pascal-app/core';
import { downspoutParametrics } from './parametrics';
import { computeDownspoutPath, downspoutPipeDims, effectiveWallJog, resolveDownspoutRouting, } from './routing';
 ...
 */
function downspoutLengthHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'max',
        ...
        placement: {
            position: (n, scene) => {
                ...
                const path = computeDownspoutPath(n.length, effectiveWallJog(n, routing), (n.terminal ?? 'splash') !== 'straight');
...
function readOutletOffset(n) {
    if (!n.gutterId)
        return 0;
    const id = n.gutterId;
    const override = useLiveNodeOverrides.getState().get(id);
    const gutter = useScene.getState().nodes[id];
    const outlets = override?.outlets ?? gutter?.outlets ?? [];
 ...
 */
function downspoutMoveHandle(side) {
    ...
    return {
        ...
        anchor: 'min',
        cursor: 'ew-resize',
        overrideTarget: (n) => (n.gutterId ? n.gutterId : undefined),
        currentValue: (n) => readOutletOffset(n),
        apply: (n, newOffset, scene) => {
            const gutter = n.gutterId ? scene.get(n.gutterId) : undefined;
            if (!gutter)
                return {};
            const outlets = (gutter.outlets ?? []).map((o) => o.id === n.outletId ? { ...o, offset: newOffset } : o);
            // Patch targets the GUTTER (overrideTarget), not the downspout.
            return { outlets };
        },
        min: (n, scene) => -moveBound(n, n.gutterId ? scene.get(n.gutterId) : undefined),
        max: (n, scene) => moveBound(n, n.gutterId ? scene.get(n.gutterId) : undefined),
        placement: {
 ...
 */
export const downspoutDefinition = {
    kind: 'downspout',
    schemaVersion: 1,
    ...
    mcp: {
        description: 'A downspout — drop pipe from a gutter outlet that elbows back to the wall, runs down the wall face, and kicks out at the bottom. length / diameter / standoff parametric.',
    },
};
