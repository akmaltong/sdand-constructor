...
import { computeDownspoutPath, downspoutPipeDims, effectiveWallJog, } from './routing';
/**
...
export function buildDownspoutGeometry(node, routing) {
    ...
    const pathData = computeDownspoutPath(node.length, effectiveWallJog(node, routing), terminal !== 'straight');
    ...
    for (let i = 0; i < path.length - 1; i++) {
        ...
        pieces.push(i === lastLeg
            ? ringTube(path[i], path[i + 1], dims)
            : segmentBetween(path[i], path[i + 1], dims));
        if (i > 0)
            pieces.push(jointAt(path[i], path[i - 1], path[i + 1], dims));
    }
    if ((node.strapStyle ?? 'band') !== 'none') {
        for (const strap of buildStraps(pathData, dims, node.strapSpacing ?? 1.8))
            pieces.push(strap);
    }
    if (terminal === 'splash') {
 ...
 */
function segmentBetween(a, b, dims) {
    const dir = new THREE.Vector3().subVectors(b, a);
    const len = dir.length();
    const geo = dims.shape === 'round'
 ...
 */
function jointAt(p, prev, next, dims) {
    if (dims.shape === 'round') {
    ...
    if (bis.lengthSq() < 1e-8)
        bis.copy(dirOut); // straight-through; degenerate
    bis.normalize();
