import { ElevatorNode as ElevatorNodeSchema, getElevatorCabDepth, getElevatorCabWidth, getElevatorShaftDepth, getElevatorShaftWallThickness, getElevatorShaftWidth, resolveElevatorLevels, } from '@pascal-app/core';
...
function elevatorAxisHandle(axis) {
    return {
        kind: 'linear-resize',
        axis,
        anchor: 'center',
...
// are configured yet.
function elevatorCabHeightHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
function elevatorOuterHalfExtents(n) {
    const cabWidth = getElevatorCabWidth(n);
    const cabDepth = getElevatorCabDepth(n);
    const wallThickness = getElevatorShaftWallThickness(n);
    return {
 ...
 * hardcoded switch.
 */
export const elevatorDefinition = {
    kind: 'elevator',
    schemaVersion: 1,
    schema: ElevatorNode,
    category: 'structure',
    surfaceRole: 'joinery',
    defaults: () => {
        const stub = ElevatorNodeSchema.parse({ id: 'elevator_default', type: 'elevator' });
