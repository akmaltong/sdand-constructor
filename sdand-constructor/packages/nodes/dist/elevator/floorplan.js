import { resolveElevatorServiceLevelIds, useInteractive, useLiveNodeOverrides, } from '@pascal-app/core';
/**
...
export function buildElevatorFloorplan(node, ctx) {
    ...
    if (STAGE_LEVEL_FILTER_HIDE && parentLevelId) {
        ...
        if (!serviceLevelIds.includes(parentLevelId)) {
            return null;
        }
    }
    ...
    const isQueuedLevel = parentLevelId
        ? (runtime?.queue.includes(parentLevelId) ?? false)
        : false;
    ...
    if (isSelected && parentLevelId) {
        ...
        if (serviceLevelIds.length > 1) {
            ...
            serviceLevelIds.forEach((levelId, index) => {
                ...
                // public return type.
                const isQueued = runtime?.queue.includes(levelId) ?? false;
...
function collectAllNodes(ctx) {
    ...
    if (level) {
        ...
        const building = level.parentId
            ? ctx.resolve(level.parentId)
            : undefined;
        if (building) {
            out[building.id] = building;
            const childIds = building.children;
            if (Array.isArray(childIds)) {
                for (const cid of childIds) {
                    const child = ctx.resolve(cid);
                    if (child)
                        out[child.id] = child;
                }
            }
