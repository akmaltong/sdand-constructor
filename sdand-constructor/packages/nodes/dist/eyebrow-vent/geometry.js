 ...
 */
export function buildEyebrowVentGeometry(node) {
...
function addScoop(p, n, uv, w, d, h, yB, slats) {
    const a = w / 2;
    const b = h;
    const zF = d / 2;
    const NZ = 20;
...
function addHalfRound(p, n, uv, w, d, h, yB, slats) {
    const a = w / 2;
...
function addSlantBox(p, n, uv, w, d, h, yB, slats, backRatio) {
    const hw = w / 2;
    const zF = d / 2;
    const zB = -d / 2;
    ...
    pushQuad(p, n, uv, [oL, oB, screenZ], [oR, oB, screenZ], [oR, oT, screenZ], [oL, oT, screenZ], [0, 0, 1]);
...
function addArchLouvers(p, n, uv, a, b, yB, z, slats) {
    if (slats <= 0)
        return;
    ...
    for (let k = 1; k <= slats; k++) {
        ...
        if (halfX < 1e-3)
            continue;
        addSlab(p, n, uv, [-halfX, y, z], [halfX, y, z], [halfX, y - drop, z - back], [-halfX, y - drop, z - back], thick);
    }
}
// Angled horizontal slats across a rectangular opening (±halfX, yB..yTop) set
// just inside the face at `z`.
function addRectLouvers(p, n, uv, halfX, yB, yTop, z, slats) {
    if (slats <= 0)
        return;
    const span = yTop - yB;
...
function addSlab(p, n, uv, a, b, c, d, t) {
    ...
    ny = (ny / len) * (t / 2);
    nz = (nz / len) * (t / 2);
    const up = (q) => [q[0] + nx, q[1] + ny, q[2] + nz];
    const dn = (q) => [q[0] - nx, q[1] - ny, q[2] - nz];
    const aT = up(a);
    const bT = up(b);
    const cT = up(c);
    const dT = up(d);
...
function halfRing(a, b, yB, z, scale, steps) {
    const row = [];
...
// ─── Primitives ───────────────────────────────────────────────────────────
function addBand(p, n, uv, rA, rB, lng, hintFn) {
    for (let j = 0; j < lng; j++) {
        const a = rA[j];
        const b = rA[j + 1];
        const c = rB[j + 1];
...
function doubleSide(p, n, uv) {
    const triCount = Math.floor(p.length / 9);
    for (let t = 0; t < triCount; t++) {
        const o = t * 9;
        const u = t * 6;
        // verts v0, v2, v1 (reverse the last two to flip the face).
        p.push(p[o], p[o + 1], p[o + 2], p[o + 6], p[o + 7], p[o + 8], p[o + 3], p[o + 4], p[o + 5]);
        n.push(-n[o], -n[o + 1], -n[o + 2], -n[o + 6], -n[o + 7], -n[o + 8], -n[o + 3], -n[o + 4], -n[o + 5]);
...
function pushTri(positions, normals, uvs, a, b, c, hint) {
