 ...
 */
const EyebrowVentRenderer = ({ node: storeNode }) => {
    ...
    return (_jsx("group", { position: segPos, "rotation-y": segRotY, children: _jsx("group", { position: [node.position[0] ?? 0, node.position[1] ?? 0, node.position[2] ?? 0], quaternion: composedQuat, ref: ref, visible: node.visible, children: _jsx("mesh", { castShadow: true, geometry: geometry, material: material, name: "eyebrow-vent-surface", receiveShadow: true, ...handlers }) }) }));
};
