...
import { EyebrowVentNode, emitter, sceneRegistry, useScene, } from '@pascal-app/core';
...
const EyebrowVentTool = () => {
    ...
    const previewNode = useMemo(() => EyebrowVentNode.parse({
    ...
    if (!activeBuildingId || !previewPos || !previewSurfaceQuat)
        return null;
    return (_jsx("group", { position: previewPos, children: _jsx("group", { "rotation-y": previewYaw, children: _jsx("group", { quaternion: previewSurfaceQuat, children: _jsx(EyebrowVentPreview, { node: previewNode }) }) }) }));
};
