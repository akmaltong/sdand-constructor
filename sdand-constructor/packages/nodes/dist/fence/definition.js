...
function fenceSideMoveHandle(side) {
    ...
    return {
        ...
        placement: {
            position: (n) => {
                ...
                const offset = Math.max((n.thickness ?? 0.1) / 2 + SIDE_HANDLE_OFFSET, SIDE_HANDLE_MIN_OFFSET);
...
function fenceCornerPicker(endpoint) {
    return {
        kind: 'tap-action',
        shape: 'corner-picker',
        cursor: 'move',
