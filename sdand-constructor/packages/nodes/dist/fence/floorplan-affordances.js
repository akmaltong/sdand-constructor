...
import { alignFloorplanDraftPoint, getSegmentGridStep, isSegmentLongEnough, snapBuildingLocalToWorldGrid, snapFenceDraftPoint, snapScalarToGrid, useAlignmentGuides, WALL_FINE_GRID_STEP, WALL_GRID_STEP, } from '@pascal-app/editor';
/**
...
function collectLevel(nodes, parentId) {
    const walls = [];
    const fences = [];
...
function collectLinkedFences(fences, draggedFenceId, linkedPoint) {
    const out = [];
    for (const fence of fences) {
        if (fence.id === draggedFenceId)
            continue;
 ...
 */
export const fenceCurveAffordance = {
    start({ node }) {
...
export const fenceMoveEndpointAffordance = {
    start({ node, payload, nodes }) {
        ...
        const affectedIds = [node.id, ...linkedOriginals.map((l) => l.id)];
        return {
            affectedIds,
            apply({ planPoint, modifiers }) {
            ...
            canCommit() {
                ...
                const finalFence = useScene.getState().nodes[node.id];
                return (!!finalFence &&
