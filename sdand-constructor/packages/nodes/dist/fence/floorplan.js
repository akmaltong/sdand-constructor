import { getWallCurveFrameAt, getWallCurveLength, getWallMidpointHandlePoint, isCurvedWall, sampleWallCenterline, } from '@pascal-app/core';
/**
...
function getFloorplanFenceLength(fence) {
 ...
 */
function getFloorplanFenceMarkerTs(fence) {
    const length = getFloorplanFenceLength(fence);
    if (length <= 0.24)
        return [0.5];
    const spacing = clamp(fence.style === 'privacy' ? fence.postSpacing * 0.72 : fence.postSpacing, 0.34, 1.5);
    const inset = clamp(Math.max(fence.postSize * 1.25, fence.edgeInset * 10), 0.18, Math.min(0.48, length * 0.22));
    ...
    if (markerCount === 1)
        return [0.5];
    return Array.from({ length: markerCount }, (_, index) => clamp((inset + (usableLength * index) / (markerCount - 1)) / length, 0.08, 0.92));
}
function buildCenterlinePathD(points) {
    if (points.length < 2)
        return '';
    const first = points[0];
    return [`M ${first.x} ${first.y}`, ...points.slice(1).map((p) => `L ${p.x} ${p.y}`)].join(' ');
}
function buildMarker(fence, point, angleRadians, accentColor, surfaceColor, isActive) {
...
export function buildFenceFloorplan(node, ctx) {
    ...
    const accentStroke = isActive && palette
        ? palette.selectedStroke
        : isHovered && palette
            ? palette.wallHoverStroke
            : '#111827';
    const glowStroke = isActive && palette
        ? palette.selectedStroke
        : isHovered && palette
            ? palette.wallHoverStroke
            : accentStroke;
    ...
    const visibleMarkers = (node.showInfill ?? true)
        ? markerFrames
    ...
    for (const marker of visibleMarkers) {
        children.push(buildMarker(node, marker.point, marker.angle, accentStroke, markerSurface, showInteractiveChrome));
    }
    ...
    if (isSelected) {
        children.push({
            ...
            payload: { fenceId: node.id, endpoint: 'start' },
        });
        children.push({
            kind: 'endpoint-handle',
            ...
            state: 'idle',
            affordance: 'move-endpoint',
            payload: { fenceId: node.id, endpoint: 'end' },
        });
