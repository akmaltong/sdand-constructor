...
import { getClampedWallCurveOffset, getMaxWallCurveOffset, getWallCurveLength, normalizeWallCurveOffset, } from '@pascal-app/core';
 ...
 */
export function FenceLengthEditor({ node, onUpdate, }) {
    ...
    return (_jsx(SliderControl, { label: "Length", max: 50, min: 0.1, onChange: handleChange, precision: 2, step: 0.01, unit: "m", value: length }));
}
export function FenceCurveEditor({ node, onUpdate, }) {
    const curveOffset = getClampedWallCurveOffset(node);
    const maxCurveOffset = getMaxWallCurveOffset(node);
    return (_jsx(SliderControl, { label: "Curve", max: Math.max(0.01, maxCurveOffset), min: -Math.max(0.01, maxCurveOffset), onChange: (value) => onUpdate({ curveOffset: normalizeWallCurveOffset(node, value) }), precision: 2, step: 0.1, unit: "m", value: Math.round(curveOffset * 100) / 100 }));
}
