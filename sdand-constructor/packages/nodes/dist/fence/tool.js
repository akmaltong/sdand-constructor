...
import { CursorSphere, createFenceOnCurrentLevel, EDITOR_LAYER, formatAngleRadians, getAngleArcToSegmentReference, getAngleToSegmentReference, getSegmentAngleReferenceAtPoint, markToolCancelConsumed, snapFenceDraftPoint, triggerSFX, useAlignmentGuides, useEditor, WALL_FINE_GRID_STEP, } from '@pascal-app/editor';
...
function toFencePlanPoint(point) {
    return [point.x, point.y];
}
function toMiterWall(segment) {
    return {
        object: 'node',
        id: segment.id,
        type: 'wall',
...
function buildDraftFenceSegment(start, end, thickness) {
    return {
        id: 'fence_draft',
        start,
        end,
...
function getSegmentEndpointKind(point, segment) {
    if (pointMatches(point, segment.start))
        return 'start';
    if (pointMatches(point, segment.end))
        return 'end';
    return null;
}
function getFenceFaceAngleCandidates(point, segment, miterData) {
...
function getDraftAngleLabels(start, end, segments, baseY, previewHeight, previewThickness) {
    ...
    for (const endpoint of endpoints) {
        const connectedSegment = segments.find((segment) => Boolean(getSegmentAngleReferenceAtPoint(endpoint.point, segment)));
        if (!connectedSegment)
            continue;
        ...
        const connectedFaceCandidates = getFenceFaceAngleCandidates(endpoint.point, connectedSegment, miterData);
...
function getDraftMeasurementState(start, end, segments, unit, baseY, previewHeight, previewThickness) {
...
function getReferenceSegments(walls, fences) {
    return [
        ...walls.map((wall) => ({
            id: wall.id,
            start: wall.start,
...
function updateFencePreview(mesh, start, end, previewHeight, previewThickness) {
...
function getCurrentLevelElements() {
    ...
    if (!levelNode || levelNode.type !== 'level')
        return { walls: [], fences: [] };
    const children = levelNode.children.map((childId) => nodes[childId]);
    return {
        walls: children.filter((n) => n?.type === 'wall'),
        fences: children.filter((n) => n?.type === 'fence'),
    };
}
export const FenceTool = () => {
    ...
    const previewHeight = typeof fenceDefaults?.height === 'number' ? fenceDefaults.height : FENCE_PREVIEW_HEIGHT;
    const previewThickness = typeof fenceDefaults?.thickness === 'number' ? fenceDefaults.thickness : FENCE_PREVIEW_THICKNESS;
    ...
    useEffect(() => {
        ...
        const onGridMove = (event) => {
            if (!(cursorRef.current && previewRef.current))
                return;
            ...
            if (buildingState.current === 1) {
                const snappedLocal = alignPoint(snapFenceDraftPoint({ point: localPoint, walls, fences, step }), bypassAlign);
                endingPoint.current.set(snappedLocal[0], event.localPosition[1], snappedLocal[1]);
                cursorRef.current.position.copy(endingPoint.current);
                const currentFenceEnd = [snappedLocal[0], snappedLocal[1]];
                if (previousFenceEnd &&
                    (currentFenceEnd[0] !== previousFenceEnd[0] || currentFenceEnd[1] !== previousFenceEnd[1])) {
                    triggerSFX('sfx:grid-snap');
                }
                previousFenceEnd = currentFenceEnd;
                updateFencePreview(previewRef.current, startingPoint.current, endingPoint.current, previewHeightRef.current, previewThicknessRef.current);
                setDraftMeasurement(getDraftMeasurementState([startingPoint.current.x, startingPoint.current.z], snappedLocal, getReferenceSegments(walls, fences), unit, startingPoint.current.y, previewHeightRef.current, previewThicknessRef.current));
            }
            else {
                const snappedPoint = alignPoint(snapFenceDraftPoint({ point: localPoint, walls, fences, step }), bypassAlign);
        ...
        const onGridClick = (event) => {
            ...
            if (buildingState.current === 0) {
                const snappedStart = alignPoint(snapFenceDraftPoint({ point: localClick, walls, fences, step: clickStep }), bypassAlign);
                ...
                const snappedEnd = alignPoint(snapFenceDraftPoint({ point: localClick, walls, fences, step: clickStep }), bypassAlign);
                const dx = snappedEnd[0] - startingPoint.current.x;
                const dz = snappedEnd[1] - startingPoint.current.z;
                if (dx * dx + dz * dz < 0.01 * 0.01)
                    return;
                const createdFence = createFenceOnCurrentLevel([startingPoint.current.x, startingPoint.current.z], snappedEnd);
                if (!createdFence)
                    return;
