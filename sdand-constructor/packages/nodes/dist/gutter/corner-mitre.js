export const NO_MITRES = { left: 0, right: 0 };
 ...
 */
export function gutterEndpointsInFrame(g, segment) {
...
export function planDistSq(a, b) {
    const dx = a[0] - b[0];
    const dz = a[2] - b[2];
    return dx * dx + dz * dz;
}
function mitreBetween(a, b) {
...
function axisIntersectionXZ(aPos, aDir, bPos, bDir) {
    const cross = aDir[0] * bDir[1] - aDir[1] * bDir[0];
    if (Math.abs(cross) < AXIS_PARALLEL_EPSILON)
        return null;
 ...
 */
export function computeGutterMitres(subject, subjectSegment, siblings) {
    ...
    for (const sib of siblings) {
        ...
        const corner = axisIntersectionXZ(subj.minus.pos, subj.minus.awayDir, other.minus.pos, other.minus.awayDir);
        if (!corner)
            continue;
        ...
        if (!otherPlusAtCorner && !otherMinusAtCorner)
            continue;
