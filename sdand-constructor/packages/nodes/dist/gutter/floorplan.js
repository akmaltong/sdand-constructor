 ...
 */
export function buildGutterFloorplan(node, ctx) {
    const segment = ctx.parent;
    if (!segment || segment.type !== 'roof-segment')
        return null;
    const roofId = segment.parentId;
    const roof = roofId ? ctx.resolve(roofId) : undefined;
    ...
    for (const segId of roof.children ?? []) {
        const sib = ctx.resolve(segId);
        if (!sib || sib.type !== 'roof-segment')
            continue;
        for (const gid of sib.children ?? []) {
            const g = ctx.resolve(gid);
    ...
    if (node.hangerStyle === 'strap') {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (x <= backLeftX + 0.02 || x >= backRightX - 0.02)
                continue;
    ...
    if (outlets.length > 0) {
        const floorZ = Math.min(Math.max(profileFloorMidZ(node.profile, width), width * 0.25), width * 0.85);
