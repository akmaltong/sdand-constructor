...
import { NO_MITRES } from './corner-mitre';
import { OUTLET_STUB_LENGTH, OUTLET_WALL_THICKNESS, outletDims, outletShapeForProfile, profileFloorMidZ, } from './profile-geometry';
/**
 ...
 */
export function buildGutterGeometry(node, mitres = NO_MITRES) {
    ...
    const pieces = [];
    let channel = new THREE.ExtrudeGeometry(channelCross, {
        depth: channelLen,
        bevelEnabled: false,
        curveSegments: 16,
        steps: 1,
    ...
    if (placements.length > 0) {
        ...
        for (const p of placements) {
            ...
            const next = csgEvaluator.evaluate(workingBrush, drillBrush, SUBTRACTION);
            // Free the previous step's intermediate result (but not `merged`,
            // which is disposed once below).
            if (workingBrush.geometry !== merged)
                workingBrush.geometry.dispose();
            workingBrush = next;
            drill.dispose();
        }
...
function stripEndCaps(geo, channelLen, removeRight, removeLeft) {
    ...
    for (let t = 0; t < triCount; t++) {
        ...
        const allLeft = Math.abs(za - channelLen) < eps &&
            Math.abs(zb - channelLen) < eps &&
            Math.abs(zc - channelLen) < eps;
        if ((removeRight && allRight) || (removeLeft && allLeft))
            continue;
        for (let k = 0; k < 3; k++) {
            const idx = a + k;
            keepPos.push(pos.getX(idx), pos.getY(idx), pos.getZ(idx));
            if (keepUv && uv)
                keepUv.push(uv.getX(idx), uv.getY(idx));
        }
    }
    if (src !== geo)
        src.dispose();
...
function buildKStyleCross(size, t) {
    ...
    shape.bezierCurveTo(wTop - size * 0.15 - t, ogeeY * 0.4, wBot + size * 0.15 - t, ogeeY, wBot - t, -size + t);
...
// closes the loop.
function buildHalfRoundCross(size, t) {
...
function buildKStyleOuterOnly(size) {
    const wBot = size * 0.8;
    const wTop = size * 0.95;
...
function buildHalfRoundOuterOnly(size) {
    const r = size;
    const segs = 24;
    const shape = new THREE.Shape();
    shape.moveTo(0, 0);
...
function buildBoxOuterOnly(size) {
    const w = size;
    const shape = new THREE.Shape();
    shape.moveTo(0, 0);
    shape.lineTo(0, -size);
...
function profileRimWidth(profile, size) {
    if (profile === 'half-round')
        return 2 * size;
    if (profile === 'box')
        return size;
    return size * 0.95; // k-style wTop
}
function buildHangers(node, len, size, capLeftLen, capRightLen, mitres) {
    ...
    const concaveInset = (mitre) => (mitre < 0 ? strapDepth : 0);
 ...
 */
function resolveOutletPlacements(node, len, size, capLeftLen, capRightLen) {
...
function outletSolid(dims, h) {
    if (dims.shape === 'round') {
        return new THREE.CylinderGeometry(dims.halfX, dims.halfX, h, OUTLET_RADIAL_SEGMENTS).toNonIndexed();
    }
 ...
 */
function buildOutletStub(p, size) {
 ...
 */
function buildOutletFunnel(p, size) {
    const centerY = -size - OUTLET_FLARE_HEIGHT / 2;
    let funnel;
    if (p.shape === 'round') {
        funnel = new THREE.CylinderGeometry(p.outer.halfX * OUTLET_FLARE_SCALE, p.outer.halfX, OUTLET_FLARE_HEIGHT, OUTLET_RADIAL_SEGMENTS).toNonIndexed();
    }
    else {
        funnel = new THREE.BoxGeometry(2 * p.outer.halfX * OUTLET_FLARE_SCALE, OUTLET_FLARE_HEIGHT, 2 * p.outer.halfZ * OUTLET_FLARE_SCALE).toNonIndexed();
    }
 ...
 */
function buildOutletDrill(p, size) {
