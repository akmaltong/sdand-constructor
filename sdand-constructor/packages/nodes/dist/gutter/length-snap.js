 ...
 */
export function snapLengthToCorner(initial, proposedLength, sign, anchorX, anchorZ, armX, armZ, minLength, sceneApi) {
    const segmentId = initial.roofSegmentId;
    if (!segmentId)
        return { length: proposedLength };
    const seg = sceneApi.get(segmentId);
    if (!seg)
        return { length: proposedLength };
    ...
    for (const sid of roofChildren ?? []) {
        const s = sceneApi.get(sid);
        if (!s || s.type !== 'roof-segment')
            continue;
        const tf = segmentTransform(s);
        for (const gid of s.children ?? []) {
            const g = sceneApi.get(gid);
            if (g?.type === 'gutter' && g.id !== initial.id) {
                candidates.push({ gutter: g, tf });
            }
        }
    }
    ...
    for (const { gutter: mateG, tf } of candidates) {
        ...
        if (Math.min(dPlusT, dMinusT) > CORNER_MATE_REACH_SQ)
            continue;
