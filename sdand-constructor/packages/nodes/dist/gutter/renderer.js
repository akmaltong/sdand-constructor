...
import { computeGutterMitres, NO_MITRES } from './corner-mitre';
 ...
 * frame.
 */
const GutterRenderer = ({ node: storeNode }) => {
    ...
    const node = overrides ? { ...storeNode, ...overrides } : storeNode;
    ...
    const segmentOverrides = useLiveNodeOverrides((s) => node.roofSegmentId
        ? s.get(node.roofSegmentId)
        : undefined);
    ...
    const mitreNodes = useScene(useShallow((state) => {
        const segmentId = node.roofSegmentId;
        const seg = segmentId ? state.nodes[segmentId] : undefined;
        const roofId = seg?.parentId;
        const roof = roofId
            ? state.nodes[roofId]
            : undefined;
        if (!roof)
        ...
        for (const sid of roof.children ?? []) {
            ...
            if (s?.type !== 'roof-segment')
                continue;
            out.push(s);
            for (const gid of s.children ?? []) {
                const g = state.nodes[gid];
                if (g?.type === 'gutter' && g.id !== storeNode.id)
                    out.push(g);
            }
        }
        return out;
    ...
    const { mitres, sharedEaveY } = useMemo(() => {
        if (!effectiveSegment)
            return { mitres: NO_MITRES, sharedEaveY: undefined };
        const segById = new Map();
        for (const n of mitreNodes) {
            if (n.type === 'roof-segment')
                segById.set(n.id, n);
        }
        const siblings = [];
        for (const n of mitreNodes) {
            ...
            if (seg)
                siblings.push({ gutter: g, segment: seg });
        }
        return {
            ...
            // needs at runtime.
            sharedEaveY: computeSharedEaveY(node, effectiveSegment, siblings),
        };
    }, [
        node.position[0],
        node.position[1],
    ...
    const geometry = useMemo(() => buildGutterGeometry(node, mitres), [
        node.length,
        node.size,
        node.thickness,
        node.profile,
    ...
    return (_jsx("group", { position: segPos, "rotation-y": segRotY, children: _jsx("group", { position: [node.position[0] ?? 0, liveEaveY, node.position[2] ?? 0], ref: ref, "rotation-y": node.rotation ?? 0, visible: node.visible, children: _jsx("mesh", { castShadow: true, geometry: geometry, material: material, name: "gutter-surface", receiveShadow: true, ...handlers }) }) }));
};
