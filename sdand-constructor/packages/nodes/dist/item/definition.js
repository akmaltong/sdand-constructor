...
export const itemDefinition = {
    ...
    capabilities: {
        ...
        floorPlaced: {
            ...
            applies: (node) => !node.asset.attachTo,
        },
        ...
        ceilingCut: {
            buildCeilingHole(rawNode) {
                const node = rawNode;
                if (!node.asset.recessed || node.asset.attachTo !== 'ceiling')
                    return null;
    ...
    toolHints: [
        { key: 'Left click', label: 'Поставить' },
        { key: 'R', label: 'Повернуть против часовой' },
        { key: 'T', label: 'Повернуть по часовой' },
        { key: 'Shift', label: 'Свободное размещение' },
    ...
    mcp: {
        description: 'A catalog-backed item with asset reference, transforms, and optional attachTo for wall/ceiling mounting.',
    },
};
