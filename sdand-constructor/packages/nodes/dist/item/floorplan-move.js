import { collectAlignmentAnchors, getScaledDimensions, movingFootprintAnchors, useScene, } from '@pascal-app/core';
import { applyFloorplanAlignment, useEditor } from '@pascal-app/editor';
import { createFloorplanCursorResolver } from '../shared/floorplan-cursor';
import { findClosestWallInPlan, snapLocalXToNeighbors } from '../shared/wall-attach-target';
function rotateVec(x, z, rotationY) {
    const c = Math.cos(rotationY);
    const s = Math.sin(rotationY);
    return [x * c + z * s, -x * s + z * c];
}
function resolveItemPlanTransform(item, nodes, cache = new Map()) {
    ...
    const parent = item.parentId ? nodes[item.parentId] : null;
    if (parent?.type === 'wall') {
        const wallRotation = -Math.atan2(parent.end[1] - parent.start[1], parent.end[0] - parent.start[0]);
        const wallLocalZ = item.asset.attachTo === 'wall-side'
        ...
        const parentTransform = resolveItemPlanTransform(parent, nodes, cache);
        const [offsetX, offsetZ] = rotateVec(item.position[0], item.position[2], parentTransform.rotation);
        result = {
...
function resolveItemPlanPoint(item, nodes, cache = new Map()) {
    return resolveItemPlanTransform(item, nodes, cache).point;
}
function createPlanarMovePointResolver(originalPlanPoint, node) {
    const resolveCursor = createFloorplanCursorResolver({
        original: originalPlanPoint,
        metadata: node.metadata,
    });
    return (planPoint, shiftKey) => {
        const snap = (value) => {
            if (shiftKey)
                return value;
        ...
        return resolveCursor(planPoint, { snap });
    };
}
export const itemFloorplanMoveTarget = ({ node, nodes }) => {
...
function buildWallItemSession(node, startLevelId) {
    ...
    return {
        ...
        apply({ planPoint, modifiers }) {
            ...
            const step = useEditor.getState().gridSnapStep;
            const snappedLocalX = neighborX ?? (modifiers.shiftKey ? hit.localX : Math.round(hit.localX / step) * step);
            const halfW = width / 2;
 ...
 * user holds Shift.
 */
function buildFloorItemSession(node, startLevelId, nodes) {
    ...
    return {
        ...
        apply({ planPoint, modifiers }) {
            ...
            const nextPosition = [snapped[0], sourceY, snapped[1]];
            useScene.getState().updateNodes([
                {
                    id: node.id,
                    data: {
 ...
 */
function buildSurfaceItemSession(node, startLevelId, targetKind) {
    const resolvePlanPoint = createPlanarMovePointResolver(resolveItemPlanPoint(node, useScene.getState().nodes), node);
    return {
        affectedIds: [node.id],
        apply({ planPoint, modifiers }) {
            const nodes = useScene.getState().nodes;
 ...
 */
function findContainingSurface(point, nodes, parentLevelId, targetKind) {
    if (!parentLevelId)
        return null;
