import { getScaledDimensions, useLiveTransforms, } from '@pascal-app/core';
...
function resolveItemTransform(item, ctx, cache = new Map()) {
    ...
    if (parentNode?.type === 'wall') {
        ...
        const parentT = resolveItemTransform(parentNode, ctx, cache);
        if (parentT) {
        ...
        const shelf = parentNode;
        const live = useLiveTransforms.getState().get(shelf.id);
...
export function buildItemFloorplan(node, ctx) {
