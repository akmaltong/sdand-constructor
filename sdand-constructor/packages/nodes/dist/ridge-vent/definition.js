import { RidgeVentNode as RidgeVentNodeSchema, } from '@pascal-app/core';
...
// shorten gesture.
function ridgeVentLengthHandle(side) {
...
function ridgeVentHeightHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
function ridgeVentRotateHandle() {
    return {
        kind: 'arc-resize',
        axis: 'angular',
        shape: 'rotate',
        ...
        decoration: {
            kind: 'ring',
            radius: (n) => Math.hypot(n.length / 2 + ROTATE_CORNER_OFFSET, n.width / 2 + ROTATE_CORNER_OFFSET),
            y: (n) => getBodyMidY(n),
        },
    };
}
 ...
 */
export const ridgeVentDefinition = {
    kind: 'ridge-vent',
    schemaVersion: 1,
    ...
    mcp: {
        description: 'A ridge vent — three styles (standard curved cap / shingled / metal), optional end caps, length / width / height parametric.',
    },
};
