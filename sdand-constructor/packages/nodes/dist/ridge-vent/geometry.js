 ...
 */
export function buildRidgeVentGeometry(node) {
...
// Smooth rounded arch.
function standardTop(halfW, h, t) {
    const pts = [];
...
function shingledTop(halfW, h, t) {
...
// ─── Band assembly ───────────────────────────────────────────────────────
function buildBand(positions, normals, uvs, top, t, halfLen, withCaps) {
    const n = top.length;
    // Underside: the same profile dropped straight down by `t` (eaves → y 0).
    const inner = top.map(([z, y]) => [z, y - t]);
    // Top surface + underside, swept along the ridge length.
    for (let i = 0; i < n - 1; i++) {
        const [z0, y0] = top[i];
        const [z1, y1] = top[i + 1];
        pushQuad(positions, normals, uvs, [-halfLen, y0, z0], [halfLen, y0, z0], [halfLen, y1, z1], [-halfLen, y1, z1], [0, 1, 0]);
        const [iz0, iy0] = inner[i];
        const [iz1, iy1] = inner[i + 1];
        pushQuad(positions, normals, uvs, [-halfLen, iy0, iz0], [halfLen, iy0, iz0], [halfLen, iy1, iz1], [-halfLen, iy1, iz1], [0, -1, 0]);
    }
    ...
    for (const idx of [0, n - 1]) {
        ...
        const hint = [0, 0, z < 0 ? -1 : 1];
        pushQuad(positions, normals, uvs, [-halfLen, yInner, z], [halfLen, yInner, z], [halfLen, yTop, z], [-halfLen, yTop, z], hint);
    }
    ...
    if (withCaps) {
        for (const sign of [-1, 1]) {
            ...
            for (let i = 0; i < n - 1; i++) {
                ...
                const [iz0, iy0] = inner[i];
                const [iz1, iy1] = inner[i + 1];
                pushQuad(positions, normals, uvs, [x, y0, z0], [x, y1, z1], [x, iy1, iz1], [x, iy0, iz0], hint);
            }
        }
    }
}
...
function addShingledTabs(positions, normals, uvs, halfLen, top, h) {
    ...
    for (let tab = 1; tab < numTabs; tab++) {
        ...
        for (let i = 0; i < top.length - 1; i++) {
            ...
            pushQuad(positions, normals, uvs, [x, r0y, r0z], [x, r1y, r1z], [x, y1, z1], [x, y0, z0], [1, 0, 0]);
            pushQuad(positions, normals, uvs, [x - ridgeD, r0y, r0z], [x - ridgeD, r1y, r1z], [x - ridgeD, y1, z1], [x - ridgeD, y0, z0], [-1, 0, 0]);
        }
    }
}
// ─── Geometry plumbing ───────────────────────────────────────────────────
...
function pushQuad(positions, normals, uvs, a, b, c, d, hint) {
