 ...
 */
const RidgeVentRenderer = ({ node: storeNode }) => {
    ...
    const node = overrides
        ? { ...storeNode, ...overrides }
        : storeNode;
    const segmentStore = useScene((state) => node.roofSegmentId
    ...
    return (_jsx("group", { position: segPos, "rotation-y": segRotY, children: _jsx("group", { position: [ridgeX, ridgeY, ridgeZ], ref: ref, "rotation-y": node.rotation ?? 0, visible: node.visible, children: _jsx("mesh", { castShadow: true, geometry: geometry, material: material, name: "ridge-vent-surface", receiveShadow: true, ...handlers }) }) }));
};
