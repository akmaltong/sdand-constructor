import { snapScalar, useScene, } from '@pascal-app/core';
...
function resolveSegmentFrame(segment, nodes) {
    const roofId = segment.parentId;
    const roof = roofId ? nodes[roofId] : undefined;
 ...
 */
export const roofSegmentResizeAffordance = {
    start({ node, payload, nodes, initialPlanPoint, gridSnapStep }) {
        const { axis, side } = payload;
 ...
 */
export const roofSegmentRotateAffordance = {
...
export const roofSegmentMoveTarget = ({ node, nodes }) => {
    ...
    void roofRot;
    let lastLocal = [node.position[0], node.position[1], node.position[2]];
    return {
        affectedIds: [segmentId],
        apply({ planPoint, modifiers }) {
            const step = getSegmentGridStep();
            const snap = (value) => (modifiers.shiftKey ? value : snapScalar(value, step));
