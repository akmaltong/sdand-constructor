...
import { getEffectiveRoofSurfaceMaterial, getEffectiveSegmentSurfaceMaterial, useRegistry, useScene, } from '@pascal-app/core';
import { createMaterial, createMaterialFromPresetRef, getRoofMaterialArray, useNodeEvents, useViewer, } from '@pascal-app/viewer';
...
export const RoofSegmentRenderer = ({ node }) => {
    ...
    const customMaterial = useMemo(() => {
        ...
        const slot = (i) => themedArray?.[i] ?? new THREE.MeshStandardMaterial();
        return [edge ?? slot(0), wall ?? slot(1), wall ?? slot(2), top ?? slot(3)];
    }, [
        node.material,
        node.materialPreset,
        node.topMaterial,
