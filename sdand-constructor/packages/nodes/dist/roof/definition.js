import { RoofNode as RoofNodeSchema, } from '@pascal-app/core';
...
function getRoofFootprintBounds(node, sceneApi) {
    let bounds = null;
    for (const childId of node.children ?? []) {
        const segment = sceneApi.get(childId);
...
const roofHandles = [roofMoveHandle()];
/**
...
export const roofDefinition = {
    ...
    surfaceRole: 'roof',
    defaults: () => {
        const stub = RoofNodeSchema.parse({ id: 'roof_default', type: 'roof' });
