...
function rayHitT(vx, vz, dx, dz, ax, az, bx, bz) {
    const ex = bx - ax;
    const ez = bz - az;
...
function buildSegPlan(roof, seg) {
 ...
 * segment hit-targets.
 */
export function buildRoofFloorplan(node, ctx) {
    const segments = ctx.children.filter((c) => c.type === 'roof-segment');
    if (segments.length === 0)
        return null;
    const plans = segments.map((s) => buildSegPlan(node, s));
    const rings = unionPolygons(plans.map((p) => p.footprint));
    if (rings.length === 0)
        return null;
    ...
    const allRidges = plans.flatMap((p) => p.ridges);
    const valleys = [];
    ...
    const clipEnd = (pt, other, ownIndex) => {
        let inOther = false;
        for (let i = 0; i < footprints.length; i++) {
            if (i === ownIndex)
                continue;
            if (pointInPolygon(pt[0], pt[1], footprints[i])) {
                inOther = true;
                break;
            }
        }
        ...
        if (!Number.isFinite(bestT))
            return pt; // overshoots but no valley to stop at
        return [pt[0] + (other[0] - pt[0]) * bestT, pt[1] + (other[1] - pt[1]) * bestT];
    };
    const clipPush = (line, width, ownIndex) => {
    ...
    plans.forEach((p, idx) => {
        for (const s of p.breaks)
            clipPush(s, hipWidth, idx);
        for (const s of p.hips)
            clipPush(s, hipWidth, idx);
        for (const s of p.ridges)
            clipPush(s, ridgeWidth, idx);
