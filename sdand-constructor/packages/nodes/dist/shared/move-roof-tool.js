...
import { collectAlignmentAnchors, emitter, movingAlignmentAnchors, nodeRegistry, resolveAlignment, sceneRegistry, useLiveTransforms, useScene, } from '@pascal-app/core';
import { CursorSphere, commitFreshPlacementSubtree, DragBoundingBox, getFloorStackPreviewPosition, resolvePlanarCursorPosition, snapFenceDraftPoint, stripPlacementMetadataFlags, triggerSFX, useAlignmentGuides, useEditor, useFreshPlacementVisibility, } from '@pascal-app/editor';
import { useViewer } from '@pascal-app/viewer';
import { useCallback, useEffect, useRef, useState } from 'react';
import * as THREE from 'three';
/** Figma-style alignment-snap threshold (meters), matching the other tools. */
const ALIGNMENT_THRESHOLD_M = 0.08;
export const MoveRoofTool = ({ node: movingNode }) => {
    const { isFreshPlacement, previewVisible: cursorVisible, revealFreshPlacement, useAbsoluteCursorPlacement, } = useFreshPlacementVisibility({
        node: movingNode,
    ...
    const [previewRotation, setPreviewRotation] = useState(movingNode.rotation);
    const [cursorWorldPos, setCursorWorldPos] = useState(() => {
        ...
        if (obj) {
            ...
            if (buildingObj)
                buildingObj.worldToLocal(worldPos);
        ...
        if ((movingNode.type === 'roof-segment' || movingNode.type === 'stair-segment') &&
            ...
            if (parentNode && 'position' in parentNode && 'rotation' in parentNode) {
                const parentAngle = parentNode.rotation;
                const px = parentNode.position[0];
                const py = parentNode.position[1];
                const pz = parentNode.position[2];
    ...
    useEffect(() => {
        ...
        const committedMeta = stripPlacementMetadataFlags(movingNode.metadata);
        const original = {
        ...
        let segmentWrapperGroup = null;
        let mergedRoofMesh = null;
        if (movingNode.type === 'roof-segment' || movingNode.type === 'stair-segment') {
        ...
        const getPreviewPosition = (position, rotation = pendingRotation) => {
            if (!isFloorPlaced)
                return position;
        ...
        const levelNode = levelId && useScene.getState().nodes[levelId]?.type === 'level'
            ? useScene.getState().nodes[levelId]
            : null;
        ...
        const alignmentCandidates = alignTopLevel
            ? collectAlignmentAnchors(useScene.getState().nodes, movingNode.id, movingNode.type === 'stair' ? levelId : undefined)
            : [];
        const alignLocalPoint = (lx, lz, bypass) => {
            if (!alignTopLevel || bypass || alignmentCandidates.length === 0) {
                useAlignmentGuides.getState().clear();
                return [lx, lz];
            }
            const moving = movingNode.type === 'stair' || movingNode.type === 'roof'
                ? movingAlignmentAnchors(movingNode, useScene.getState().nodes, lx, lz, pendingRotation)
                : [];
            const ar = resolveAlignment({
                moving: moving.length > 0 ? moving : [{ nodeId: movingNode.id, kind: 'corner', x: lx, z: lz }],
        ...
        const localToWorldPoint = (localPoint, y) => {
            if (buildingObj) {
        ...
        const computeLocal = (gridX, gridZ, y, buildingLocalX, buildingLocalZ) => {
            ...
            if ((movingNode.type === 'roof-segment' || movingNode.type === 'stair-segment') &&
                ...
                if (parentNode && 'position' in parentNode && 'rotation' in parentNode) {
                    ...
                    const dx = gridX - parentNode.position[0];
                    const dz = gridZ - parentNode.position[2];
                    const angle = -parentNode.rotation;
                    return [
        ...
        const localPositionToToolLocal = (position) => {
            if ((movingNode.type === 'roof-segment' || movingNode.type === 'stair-segment') &&
                movingNode.parentId) {
                const parentObj = sceneRegistry.nodes.get(movingNode.parentId);
                if (parentObj) {
        ...
        const onGridMove = (event) => {
            ...
            const [rawLocalX, rawLocalZ] = computeLocal(rawGridX, rawGridZ, y, snappedLocal[0], snappedLocal[1]);
            ...
            setCursorWorldPos(isFloorPlaced ? previewPosition : localPositionToToolLocal(lastLocalPosition));
        ...
        const onGridClick = (event) => {
            ...
            useAlignmentGuides.getState().clear();
            wasCommitted = true;
            let committedId = movingNode.id;
            if (isNew) {
                committedId =
                    commitFreshPlacementSubtree(movingNode.id, {
        ...
        return () => {
            ...
            if (segmentWrapperGroup)
                segmentWrapperGroup.visible = false;
            if (mergedRoofMesh)
                mergedRoofMesh.visible = true;
    ...
    return (_jsxs("group", { visible: cursorVisible, children: [_jsx(CursorSphere, { position: cursorWorldPos, showTooltip: false }), showBoundingBox && (_jsx(DragBoundingBox, { nodeId: movingNode.id, position: cursorWorldPos, rotationY: previewRotation }))] }));
};
