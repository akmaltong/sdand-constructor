import { isCurvedWall, } from '@pascal-app/core';
/**
 ...
 */
export function buildOpeningPlacementDimensions(opening, ctx) {
    ...
    const childIds = (wall.children ?? []);
    let leftBoundary = null;
    let rightBoundary = null;
    for (const childId of childIds) {
        if (childId === opening.id)
            continue;
        const sibling = ctx.resolve(childId);
        if (!sibling || (sibling.type !== 'door' && sibling.type !== 'window'))
            continue;
        const sib = sibling;
 ...
 */
function computeOutwardNormal(wall, ctx, dirX, dirZ) {
    const nx = -dirZ;
    const nz = dirX;
    // Find the level by walking up via wall.parentId.
    const level = wall.parentId
        ? ctx.resolve(wall.parentId)
        : null;
