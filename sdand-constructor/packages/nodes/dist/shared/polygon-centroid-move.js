import { collectAlignmentAnchors, polygonAnchors, resolveAlignment, sceneRegistry, useLiveTransforms, useScene, } from '@pascal-app/core';
import { getSegmentGridStep, useAlignmentGuides } from '@pascal-app/editor';
...
function translatePolygon(polygon, dx, dz) {
    return polygon.map(([x, z]) => [x + dx, z + dz]);
}
...
export function createPolygonCentroidMoveTarget(args) {
    ...
    const originalPolygon = node.polygon.map(([x, z]) => [x, z]);
    const hasHoles = Array.isArray(node.holes);
    const originalHoles = (node.holes ?? []).map((hole) => hole.map(([x, z]) => [x, z]));
    ...
    return {
        ...
        apply({ planPoint, modifiers }) {
            ...
            const snap = (value) => (modifiers.shiftKey ? value : Math.round(value / step) * step);
            const target = resolveCursor(planPoint, { snap });
            ...
            if (mesh)
                mesh.position.set(dx, meshY, dz);
        },
        canCommit() {
            const live = useScene.getState().nodes[id];
            if (!live || live.type !== typeGuard)
                return false;
