import { getRoofSegmentSurfaceY, getSegmentSlopeFrame, ROOF_SHAPE_DEFAULTS, } from '@pascal-app/core';
...
export function getSurfaceY(lx, lz, seg) {
...
function buildSlopeNormal(dx, dz, tan) {
    return new THREE.Vector3(dx * tan, 1, dz * tan).normalize();
}
export function getAnalyticalNormal(lx, lz, seg) {
    ...
    if (roofType === 'gable' || roofType === 'gambrel') {
        if (roofType === 'gambrel') {
            ...
            const lowerWidthRatio = seg.gambrelLowerWidthRatio ?? ROOF_SHAPE_DEFAULTS.gambrelLowerWidthRatio;
            const lowerHeightRatio = seg.gambrelLowerHeightRatio ?? ROOF_SHAPE_DEFAULTS.gambrelLowerHeightRatio;
    ...
    if (roofType === 'mansard') {
        ...
        if (onZ)
            return buildSlopeNormal(0, lz >= 0 ? 1 : -1, tan);
...
export function surfaceQuatFromNormal(normal, out) {
