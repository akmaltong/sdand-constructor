import { getScaledDimensions, isCurvedWall, } from '@pascal-app/core';
/**
...
export function projectWallLocalPointToPlan(wall, localX, localZ = 0) {
 ...
 */
export function findClosestWallInPlan(planPoint, nodes, parentLevelId, excludeWallId) {
    if (!parentLevelId)
        return null;
    ...
    for (const childId of childIds) {
        ...
        if (distance > WALL_SNAP_DISTANCE_M)
            continue;
        if (best && distance >= Math.abs(best.perpDistance) && best.wall.id !== wall.id)
            continue;
        ...
        const side = perpRaw >= 0 ? 'front' : 'back';
 ...
 *  footprint. */
function wallAttachmentSpan(node) {
...
export function snapLocalXToNeighbors(args) {
    ...
    // edges and centre.
    const candidateStops = [0, wallLength];
