...
function shelfRotateHandle() {
    return {
        ...
        apply: (initial, delta) => {
            ...
            return { rotation: [r[0], (r[1] ?? 0) - delta, r[2]] };
        },
        placement: {
            position: (n) => {
...
function shelfMoveHandle() {
    return {
        ...
        placement: {
            ...
            position: (n) => {
                const shelf = sanitizeShelfDimensions(n);
...
export const shelfDefinition = {
    ...
    capabilities: {
        ...
        surfaces: {
            top: { height: (n) => shelfRowSurfaceYs(n).at(-1) ?? 0 },
            custom: (n) => shelfRowSurfaceYs(n).map((y) => ({
                position: [0, y, 0],
                normal: [0, 1, 0],
            })),
        },
        ...
        floorPlaced: {
            footprint: (node) => {
                ...
                return {
                    dimensions: [shelf.width, shelf.height, shelf.depth],
    ...
    geometryKey: (n) => {
        const s = sanitizeShelfDimensions(n);
    ...
    mcp: {
        description: 'A parametric shelving unit. Four styles (wall-shelf / bookshelf / open-rack / cubby) with configurable rows, columns, sides, and back. Items host on each row.',
    },
};
