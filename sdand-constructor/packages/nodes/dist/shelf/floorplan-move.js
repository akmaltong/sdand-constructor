...
import { applyFloorplanAlignment, getFloorStackPreviewPosition, triggerSFX, useEditor, } from '@pascal-app/editor';
 ...
 */
export const shelfFloorplanMoveTarget = ({ node, nodes }) => {
    ...
    const session = {
        ...
        apply({ planPoint, modifiers }) {
            ...
            const { point: snapped } = applyFloorplanAlignment(gridSnapped, movingFootprintAnchors(node, gridSnapped[0], gridSnapped[1], originalRotationY), candidates, { bypass: modifiers.altKey });
        ...
        canCommit() {
            const live = useScene.getState().nodes[shelfId];
            if (!live || live.type !== 'shelf')
                return false;
