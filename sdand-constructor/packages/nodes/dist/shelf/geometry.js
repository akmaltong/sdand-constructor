...
import { applyMaterialPresetToMaterials, createDefaultMaterial, createMaterial, DEFAULT_SHELF_MATERIAL, } from '@pascal-app/viewer';
...
function getShelfMaterial(node, shading) {
...
export function buildShelfGeometry(rawNode, _ctx, shading = 'rendered') {
 ...
 */
function buildWallShelf(group, node, material) {
 ...
 * silhouette).
 */
function buildBookshelf(group, node, material) {
 ...
 */
function buildOpenRack(group, node, material) {
    const unitHeight = node.height + node.thickness;
    ...
    if (node.withBack) {
        ...
        for (const y of [boardThickness, unitHeight - boardThickness]) {
            const brace = new Mesh(new BoxGeometry(node.width - braceThickness * 2, braceThickness, braceThickness), material);
 ...
 */
function buildCubby(group, node, material) {
    const unitHeight = node.height + node.thickness;
    ...
    if (node.columns > 1) {
        ...
        for (let r = 0; r < node.rows; r++) {
            ...
            for (let c = 1; c < node.columns; c++) {
                const x = -innerWidth / 2 + c * colStep;
                const divider = new Mesh(new BoxGeometry(node.thickness, dividerHeight, node.depth), material);
 ...
 */
function addCornerPosts(group, node, material, unitHeight, postStyle) {
    const postThickness = postStyle === 'rack' ? Math.max(0.025, node.thickness * 1.5) : Math.max(0.02, node.thickness);
    ...
    for (const xSign of [-1, 1]) {
        for (const zSign of [-1, 1]) {
            ...
            post.position.set(xSign * (node.width / 2 - inset), unitHeight / 2, zSign * (node.depth / 2 - inset));
            group.add(post);
        }
    }
}
 ...
 */
export function shelfRowSurfaceYs(node) {
    ...
    if (safe.withBottom && bottomApplies)
        ys.unshift(safe.thickness);
    return ys;
}
