...
import { getSceneTheme, NodeRenderer, unionPolygons, useNodeEvents, useViewer, } from '@pascal-app/viewer';
...
export const SiteRenderer = ({ node }) => {
    ...
    const livePolygon = useLiveNodeOverrides((state) => state.overrides.get(node.id)?.polygon ?? null);
    ...
    const hasScanVenue = useScene((state) => Object.values(state.nodes).some((n) => n.type === 'scan'));
    // Cache slab polygon references to keep the selector stable across unrelated store updates
    const slabPolygonsCache = useRef([]);
    const slabPolygons = useScene((state) => {
        ...
        const next = nodeList
            ...
            if (!Number.isFinite(lowestLevelIndex))
                return true;
            const parentLevel = n.parentId ? levelIndexById.get(n.parentId) : undefined;
