import { SkylightNode as SkylightNodeSchema, } from '@pascal-app/core';
import { buildSkylightFloorplan } from './floorplan';
import { closeSkylightOpenState, isOperableSkylightNode, toggleSkylightOpenState, } from './interaction';
...
// segment-local X).
function skylightWidthHandle(side) {
...
function skylightHeightHandle(side) {
...
// tall lantern curbs.
function skylightCurbHeightHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
function skylightFrameThicknessHandle() {
    return {
        kind: 'linear-resize',
        axis: 'z',
        anchor: 'min',
 ...
 */
export const skylightDefinition = {
    kind: 'skylight',
    schemaVersion: 1,
    schema: SkylightNode,
    category: 'structure',
    ...
    mcp: {
        description: 'A skylight on a roof segment. Five type variants (flat / walk-on / lantern / opening / sliding) — geometry beyond box stub coming later.',
    },
