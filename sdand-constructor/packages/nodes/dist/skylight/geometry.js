...
export function paneSize(value) {
    return Math.max(0.02, value);
}
export function clamp01(value) {
    return Math.min(1, Math.max(0, value));
}
export function buildLanternGlassGeometry(width, depth, lanternHeight, topScale) {
    ...
    const positions = resolvedTopScale <= 1e-4
        ? [
            -baseHalfW,
            0,
            baseHalfD,
    ...
    const indices = resolvedTopScale <= 1e-4
