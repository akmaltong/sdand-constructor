...
const SkylightPreview = ({ node }) => {
    const frame = useMemo(() => buildFrameGeometry({
        curb: node.curb,
    ...
    if (!frame)
        return null;
    return (_jsxs("group", { children: [_jsx("mesh", { geometry: frame, material: ghostMaterial, raycast: () => { } }), _jsx("mesh", { geometry: glass, material: ghostMaterial, raycast: () => { } })] }));
};
