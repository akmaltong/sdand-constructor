...
const easeSkylightAnimation = (value) => value * value * (3 - 2 * value);
const SkylightAnimationSystem = () => {
    useFrame(({ clock }) => {
        ...
        for (const [skylightId, animation] of entries) {
            const typedSkylightId = skylightId;
            ...
            const value = animation.from + (animation.to - animation.from) * easeSkylightAnimation(progress);
