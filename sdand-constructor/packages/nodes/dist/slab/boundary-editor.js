 ...
 */
export const SlabBoundaryEditor = ({ slabId }) => {
    ...
    if (!slab?.polygon || slab.polygon.length < 3)
        return null;
    return (_jsx(PolygonEditor, { allowEdgeMove: true, color: "#a3a3a3", levelId: resolveLevelId(slab, useScene.getState().nodes), minVertices: 3, onPolygonChange: handlePolygonChange, onPolygonPreview: handlePolygonPreview, polygon: slab.polygon, surfaceHeight: slab.elevation ?? 0.05 }));
};
