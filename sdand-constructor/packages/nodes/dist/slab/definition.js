...
import { slabAddVertexAffordance, slabMoveEdgeAffordance, slabMoveVertexAffordance, } from './floorplan-affordances';
...
function pointIsOnSolidSlab(point, slab) {
    if (!pointInPolygon2D(point, slab.polygon, { includeBoundary: false }))
        return false;
    return !(slab.holes ?? []).some((hole) => hole.length >= 3 && pointInPolygon2D(point, hole, { includeBoundary: true }));
}
function slabHandleAnchor(slab) {
    const polygon = slab.polygon ?? [];
    const fallback = polygonVertexAverage(polygon);
    if (polygon.length < 3)
        return fallback;
    if (pointIsOnSolidSlab(fallback, slab))
        return fallback;
    ...
    for (const candidate of candidates) {
        if (!pointIsOnSolidSlab(candidate, slab))
            continue;
...
export const slabDefinition = {
    ...
    toolHints: [
        { key: 'Left click', label: 'Обвести контур пола' },
        { key: 'Enter', label: 'Завершить пол' },
