import { getRenderableSlabPolygon, } from '@pascal-app/core';
/**
 ...
 */
export function buildSlabFloorplan(node, ctx) {
    const polygon = node.polygon;
    if (!polygon || polygon.length < 3)
        return null;
    const visualPolygon = getRenderableSlabPolygon(node);
    if (!visualPolygon || visualPolygon.length < 3)
        return null;
    ...
    const outer = visualPolygon.map(([x, z]) => [x, z]);
    const ring = (points) => {
    ...
    const segments = [ring(outer)];
    const holes = node.holes ?? [];
    for (const hole of holes) {
        if (hole.length < 3)
            continue;
        const holePts = hole.map(([x, z]) => [x, z]);
    ...
    if (isSelected) {
        ...
        holes.forEach((hole, holeIndex) => {
            if (hole.length >= 3)
                appendRingEditor(children, hole, holeIndex);
        });
    }
 ...
 */
function appendRingEditor(children, ring, holeIndex) {
