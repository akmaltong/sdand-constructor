...
export const SlabHoleEditor = ({ slabId, holeIndex, }) => {
    ...
    if (!(slab && hole) || hole.length < 3 || metadata?.source !== 'manual')
        return null;
    return (_jsx(PolygonEditor, { allowEdgeMove: true, allowPolygonMove: true, color: "#ef4444", levelId: resolveLevelId(slab, useScene.getState().nodes), minVertices: 3, onPolygonChange: handlePolygonChange, onPolygonPreview: handlePolygonPreview, polygon: hole, surfaceHeight: slab.elevation ?? 0.05 }));
};
