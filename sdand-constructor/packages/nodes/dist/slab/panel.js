...
export function SlabPanel() {
    ...
    if (!(node && node.type === 'slab' && selectedId))
        return null;
    ...
    return (_jsxs(PanelWrapper, { icon: "/icons/floor.png", onClose: handleClose, title: node.name || 'Slab', width: 320, children: [_jsxs(PanelSection, { title: "Elevation", children: [_jsx(SliderControl, { label: "Height", max: 1, min: -1, onChange: (v) => handleUpdate({ elevation: v }), precision: 3, step: 0.01, unit: "m", value: Math.round(node.elevation * 1000) / 1000 }), _jsxs("div", { className: "mt-2 grid grid-cols-2 gap-1.5 px-1 pb-1", children: [_jsx(ActionButton, { label: "Sunken (-15cm)", onClick: () => handleUpdate({ elevation: -0.15 }) }), _jsx(ActionButton, { label: "Ground (0m)", onClick: () => handleUpdate({ elevation: 0 }) }), _jsx(ActionButton, { label: "Raised (+5cm)", onClick: () => handleUpdate({ elevation: 0.05 }) }), _jsx(ActionButton, { label: "Step (+15cm)", onClick: () => handleUpdate({ elevation: 0.15 }) })] })] }), _jsx(PanelSection, { title: "Info", children: _jsxs("div", { className: "flex items-center justify-between px-2 py-1 text-muted-foreground text-sm", children: [_jsx("span", { children: "Area" }), _jsxs("span", { className: "font-mono text-white", children: [area.toFixed(2), " m\u00B2"] })] }) }), _jsxs(PanelSection, { title: "Holes", children: [node.holes && node.holes.length > 0 ? (_jsx("div", { className: "flex flex-col gap-1 pb-2", children: node.holes.map((hole, index) => {
