import { SolarPanelNode as SolarPanelNodeSchema, } from '@pascal-app/core';
...
function solarPanelWidthHandle(side) {
...
function solarPanelHeightHandle(side) {
...
function solarPanelRotateHandle() {
    return {
        kind: 'arc-resize',
        axis: 'angular',
        shape: 'rotate',
        ...
        decoration: {
            kind: 'ring',
            radius: (n) => Math.hypot(totalArrayWidth(n) / 2, totalArrayHeight(n) / 2) + ROTATE_RING_OFFSET,
            y: () => 0,
        },
        portal: 'grandparent',
    };
...
function solarPanelFrameDepthHandle() {
    return {
        kind: 'linear-resize',
        axis: 'y',
        anchor: 'min',
...
function solarPanelFrameThicknessHandle() {
    return {
        kind: 'linear-resize',
        axis: 'z',
        anchor: 'min',
 ...
 */
export const solarPanelDefinition = {
    kind: 'solar-panel',
    schemaVersion: 1,
    ...
    mcp: {
        description: 'A solar panel array on a roof segment. rows × columns grid of individual panels with configurable size, gap, mounting (flush / tilted), and frame.',
    },
};
