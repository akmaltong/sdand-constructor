...
let _defaultPanelMaterial = null;
export function getDefaultPanelMaterial() {
    if (_defaultPanelMaterial)
 ...
 */
export function buildSolarPanelGeometry(node) {
    const { rows, columns, panelWidth, panelHeight, gapX, gapY, frameThickness, frameDepth, standoffHeight, } = node;
    const frameGeos = [];
    const panelGeos = [];
...
function getSlopeDepthBounds(segment, panelLocalZ) {
 ...
 */
export function computeAutoFit(segment, panel) {
...
export function flippedPanelDims(panel) {
