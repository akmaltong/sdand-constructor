 ...
 * binding.
 */
const SolarPanelRenderer = ({ node: storeNode }) => {
    ...
    const panelMaterial = useMemo(() => {
        if (node.panelMaterial)
            return createMaterial(node.panelMaterial, shading);
        return (createMaterialFromPresetRef(node.panelMaterialPreset, shading) ?? getDefaultPanelMaterial());
    ...
    const surfaceFrame = useMemo(() => {
        ...
        return getRoofOuterSurfaceFrameAtPoint(effectiveSegment, node.position[0] ?? 0, node.position[2] ?? 0);
    ...
    return (_jsx("group", { position: effectiveSegment.position, "rotation-y": effectiveSegment.rotation, children: _jsx("group", { position: [node.position[0] ?? 0, surfaceY, node.position[2] ?? 0], quaternion: composedQuat, ref: ref, visible: node.visible, children: _jsx("mesh", { castShadow: true, geometry: geometry, material: [frameMaterial, panelMaterial], name: "solar-panel-surface", receiveShadow: true, ...handlers }) }) }));
};
