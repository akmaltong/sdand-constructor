import { StairNode as StairNodeSchema, stairFootprintAABB, } from '@pascal-app/core';
...
function readCurvedStairGeometry(node) {
...
function isCurvedOrSpiral(node) {
    return node.stairType === 'curved' || node.stairType === 'spiral';
}
function rotateLocalXZ(x, z, angle) {
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
    return [x * cos + z * sin, -x * sin + z * cos];
}
function fallbackStraightStairMoveBounds(node) {
    const width = Math.max(node.width ?? 1, MIN_CURVED_WIDTH);
...
function readStraightStairMoveBounds(node, sceneApi) {
    const segments = (node.children ?? [])
        .map((childId) => sceneApi.get(childId))
    ...
    segments.forEach((segment, index) => {
        ...
        height = Math.max(height, transform.position[1] + Math.max(segment.height, segment.thickness, 0.01));
    });
...
function readStairMoveBounds(node, sceneApi) {
    if (!isCurvedOrSpiral(node))
        return readStraightStairMoveBounds(node, sceneApi);
...
function curvedInnerRadiusHandle() {
    return {
        kind: 'linear-resize',
        axis: 'x',
        anchor: 'min',
        min: (n) => n.stairType === 'spiral' ? MIN_CURVED_INNER_RADIUS_SPIRAL : MIN_CURVED_INNER_RADIUS_CURVED,
        currentValue: (n) => {
            const minIR = n.stairType === 'spiral' ? MIN_CURVED_INNER_RADIUS_SPIRAL : MIN_CURVED_INNER_RADIUS_CURVED;
...
function curvedSweepHandle(end) {
    return {
        kind: 'arc-resize',
        axis: 'angular',
        end,
        apply: (initial, delta) => {
            ...
            const clampedAbs = Math.min(MAX_CURVED_SWEEP, Math.max(MIN_CURVED_SWEEP, Math.abs(targetSweep)));
...
function stairRotateGizmoPosition(n) {
    ...
    return {
        ...
        decoration: {
            ...
            radius: (n) => {
                ...
                const width = Math.max(n.width ?? 1, MIN_CURVED_WIDTH);
                return (Math.hypot(width / 2 + STAIR_ROTATE_CORNER_OFFSET, STAIR_ROTATE_CORNER_OFFSET) +
...
function stairHandles(node) {
    ...
    const handles = [];
    if (isCurvedOrSpiral(node)) {
        handles.push(curvedRiseHandle(), curvedWidthHandle(), curvedInnerRadiusHandle(), curvedSweepHandle('start'), curvedSweepHandle('end'));
    }
    handles.push(stairRotateHandle(), stairMoveHandle());
    return handles;
}
import { computeStairSegmentFloorStackTransforms, getStairFloorPlacedFootprints, } from './floor-stack';
import { buildStairFloorplan } from './floorplan';
import { curvedStairInnerRadiusAffordance, curvedStairSweepAffordance, curvedStairWidthAffordance, segmentLengthAffordance, segmentWidthAffordance, stairRotateAffordance, } from './floorplan-affordances';
...
export const stairDefinition = {
    ...
    surfaceRole: 'joinery',
    defaults: () => {
        const stub = StairNodeSchema.parse({ id: 'stair_default', type: 'stair' });
    ...
    capabilities: {
        ...
        deletable: true,
        floorPlaced: {
            footprints: (node, ctx) => ctx ? getStairFloorPlacedFootprints(node, ctx.nodes) : [],
        },
    },
    ...
    presentation: {
        label: 'Stair',
        description: 'A stair composed of one or more flights with configurable treads, risers, railings.',
