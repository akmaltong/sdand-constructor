export function getStairFloorPlacedFootprints(stair, nodes) {
    const segments = (stair.children ?? [])
        .map((childId) => nodes[childId])
        .filter((node) => node?.type === 'stair-segment');
    return getStairSegmentFloorPlacedFootprints(stair, segments);
}
export function getStairSegmentFloorPlacedFootprints(stair, segments) {
    ...
    return segments.map((segment, index) => {
        ...
        const [centerOffsetWorldX, centerOffsetWorldZ] = rotateXZ(centerInGroupX, centerInGroupZ, stair.rotation);
        return {
            position: [
...
export function computeStairSegmentFloorStackTransforms(segments) {
    const transforms = [];
    let currentX = 0;
    let currentY = 0;
    let currentZ = 0;
