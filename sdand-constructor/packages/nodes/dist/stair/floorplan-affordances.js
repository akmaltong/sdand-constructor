 ...
 */
export const segmentWidthAffordance = {
    start({ payload, nodes, initialPlanPoint }) {
        const { segmentId, side, axisX } = payload;
        const segmentNodeId = segmentId;
        const segment = nodes[segmentNodeId];
        if (!segment || segment.type !== 'stair-segment')
            return noopSession();
 ...
 */
export const segmentLengthAffordance = {
    start({ payload, nodes, initialPlanPoint }) {
        const { segmentId, axisZ } = payload;
        const segmentNodeId = segmentId;
        const segment = nodes[segmentNodeId];
        if (!segment || segment.type !== 'stair-segment')
            return noopSession();
 ...
 * = -rotation).
 */
export const curvedStairWidthAffordance = {
    start({ node, initialPlanPoint }) {
        const stairId = node.id;
        ...
        const initialRadial = (initialPlanPoint[0] - cx) * radialX + (initialPlanPoint[1] - cz) * radialZ;
        let lastWidth = initialWidth;
        return {
            affectedIds: [stairId],
            apply({ planPoint }) {
 ...
 */
export const curvedStairInnerRadiusAffordance = {
    start({ node, initialPlanPoint }) {
        const stairId = node.id;
        ...
        return {
            ...
            apply({ planPoint }) {
                ...
                const newInner = Math.min(maxInnerRadius, Math.max(minInnerRadius, initialInnerRadius + innerDelta));
 ...
 */
export const stairRotateAffordance = {
    start({ node, initialPlanPoint }) {
        const stairId = node.id;
 ...
 */
export const curvedStairSweepAffordance = {
    start({ node, payload, initialPlanPoint }) {
        const { end } = payload;
        const stairId = node.id;
        const initialSweep = node.sweepAngle ?? (node.stairType === 'spiral' ? Math.PI * 2 : Math.PI / 2);
