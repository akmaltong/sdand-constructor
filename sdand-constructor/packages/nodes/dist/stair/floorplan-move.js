import { collectAlignmentAnchors, movingAlignmentAnchors, snapScalar, useScene, } from '@pascal-app/core';
 ...
 */
export const stairFloorplanMoveTarget = ({ node, nodes }) => {
    ...
    const session = {
        ...
        apply({ planPoint, modifiers }) {
            ...
            const { point: aligned } = applyFloorplanAlignment([gx, gz], movingAnchors.length > 0
                ? movingAnchors
                : [{ nodeId: node.id, kind: 'corner', x: gx, z: gz }], candidates, { bypass: modifiers.altKey });
            const sx = aligned[0];
            const sz = aligned[1];
            if (lastValid && lastValid.position[0] === sx && lastValid.position[2] === sz)
                return;
            lastValid = { position: [sx, startY, sz] };
            useScene.getState().updateNodes([{ id: node.id, data: lastValid }]);
        },
        canCommit() {
