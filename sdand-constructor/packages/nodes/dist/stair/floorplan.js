...
import { buildFloorplanStairEntry, buildSvgAnnularSectorPath, buildSvgArcPath, buildSvgArrowHeadPoints, getArcPlanPoint, } from '@pascal-app/editor';
/**
 ...
 */
export function buildStairFloorplan(stair, ctx) {
    const segments = (ctx.children ?? []).filter((child) => child.type === 'stair-segment' && child.visible !== false);
    ...
    if (stairType === 'straight') {
        for (const segmentEntry of entry.segments) {
            ...
            if (isSelected && !view?.moving) {
                ...
                if (c0 && c1 && c2 && c3) {
                    ...
                    const axisX = [(c1.x - c0.x) / width, (c1.y - c0.y) / width];
                    const axisZ = [(c3.x - c0.x) / length, (c3.y - c0.y) / length];
                    const rightMid = [(c1.x + c2.x) / 2, (c1.y + c2.y) / 2];
                    const leftMid = [(c0.x + c3.x) / 2, (c0.y + c3.y) / 2];
                    const frontEdgeMid = [(c2.x + c3.x) / 2, (c2.y + c3.y) / 2];
        ...
        const visualSweep = Math.sign(rawVisualSweep || 1) * Math.min(Math.abs(rawVisualSweep), sweepCap);
        const visualSectorEndAngle = sectorStartAngle + visualSweep;
        const stairCenter = { x: stair.position[0], y: stair.position[2] };
        const innerRadius = Math.max(stairType === 'spiral' ? 0.05 : 0.2, stair.innerRadius ?? (stairType === 'spiral' ? 0.2 : 0.9));
        ...
        children.push({
            kind: 'path',
            d: buildSvgAnnularSectorPath(stairCenter, innerRadius, outerRadius, sectorStartAngle, visualSectorEndAngle),
            fill,
            fillOpacity: 1,
            stroke: 'none',
            opacity: 0.92,
        ...
        for (let index = 0; index <= stepCount; index += 1) {
            ...
            const stepWidth = stairType === 'spiral' ? (isEmphasised ? 1.8 : 1.15) : isEmphasised ? 1.5 : 1.1;
            children.push({
                kind: 'line',
                x1: inner.x,
                y1: inner.y,
        ...
        if (stairType === 'curved') {
            ...
            children.push({
                kind: 'path',
                d: buildSvgArcPath(stairCenter, centerlineRadius, sectorStartAngle + margin, sectorEndAngle - margin),
                fill: 'none',
                stroke: stairAccent,
...
function toFloorplanPoints(points) {
...
function getNormalizedFloorplanStairSweepAngle(stair) {
    const stairType = stair.stairType ?? 'straight';
...
function getFloorplanSpiralLandingSweep(stair, sweepAngle) {
    if ((stair.stairType ?? 'straight') !== 'spiral' ||
