...
// ─── Public builders ─────────────────────────────────────────────────
export function buildTurbineVentBase(node) {
...
export function buildTurbineVentHead(node) {
    const d = resolveDims(node);
    const p = [];
    const n = [];
    const uv = [];
...
export function buildTurbineVentGeometry(node) {
    const d = resolveDims(node);
...
function buildVanes(p, n, uv, count, rings, dw, totalTwist, thk) {
    const y0 = rings[0].y;
    ...
    for (let i = 0; i < count; i++) {
        ...
        const out = [Math.cos(midA), 0, Math.sin(midA)];
        const inward = [-out[0], 0, -out[2]];
        // Tangential directions for the two angular edges.
        const left = [Math.sin(midA), 0, -Math.cos(midA)];
        const right = [-left[0], 0, -left[2]];
        for (let k = 0; k < K - 1; k++) {
            // Outer + inner faces.
...
// ─── Primitive builders ──────────────────────────────────────────────
function cylinderWall(p, n, uv, r, y0, y1, segs) {
    for (let i = 0; i < segs; i++) {
        ...
        pushQuad(p, n, uv, [r * ca, y0, r * sa], [r * cb, y0, r * sb], [r * cb, y1, r * sb], [r * ca, y1, r * sa], out);
    }
}
function disc(p, n, uv, r, y, hint, segs = 32) {
...
function dome(p, n, uv, rb, y0, hd, lat, lng) {
    const grid = [];
    for (let i = 0; i <= lat; i++) {
...
function knob(p, n, uv, ro, y0, headH) {
    const kr = ro * 0.12;
    const kh = headH * 0.06;
...
function pushQuad(positions, normals, uvs, ap, bp, cp, dp, hint) {
    const a = v(ap);
    const b = v(bp);
    const c = v(cp);
    const d = v(dp);
...
function pushTri(positions, normals, uvs, ap, bp, cp, hint) {
    const a = v(ap);
    const b = v(bp);
    const c = v(cp);
...
function toGeometry(positions, normals, uvs) {
