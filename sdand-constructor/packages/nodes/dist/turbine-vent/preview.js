 ...
 */
const TurbineVentPreview = ({ node }) => {
    const geometry = useMemo(() => buildTurbineVentGeometry(node), [node.style, node.diameter, node.height, node.neckHeight, node.vaneCount, node.baseOverhang]);
