 ...
 * untouched.
 */
const TurbineVentRenderer = ({ node: storeNode }) => {
    ...
    const segment = useScene((state) => node.roofSegmentId
        ...
        : undefined);
    const baseGeometry = useMemo(() => buildTurbineVentBase(node), [node.diameter, node.height, node.neckHeight, node.baseOverhang]);
    const headGeometry = useMemo(() => buildTurbineVentHead(node), [node.style, node.diameter, node.height, node.neckHeight, node.vaneCount]);
    ...
    return (_jsx("group", { position: segPos, "rotation-y": segRotY, children: _jsxs("group", { position: [node.position[0] ?? 0, node.position[1] ?? 0, node.position[2] ?? 0], quaternion: composedQuat, ref: ref, visible: node.visible, children: [_jsx("mesh", { castShadow: true, geometry: baseGeometry, material: material, name: "turbine-vent-base", receiveShadow: true, ...handlers }), _jsx("group", { position: [0, neckH, 0], ref: headRef, children: _jsx("mesh", { castShadow: true, geometry: headGeometry, material: material, name: "turbine-vent-head", receiveShadow: true, ...handlers }) })] }) }));
};
