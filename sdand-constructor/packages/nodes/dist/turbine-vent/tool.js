...
import { emitter, sceneRegistry, TurbineVentNode, useScene, } from '@pascal-app/core';
...
const TurbineVentTool = () => {
    ...
    const previewNode = useMemo(() => TurbineVentNode.parse({
    ...
    useEffect(() => {
        ...
        const onClick = (event) => {
            ...
            state.createNode(vent, hit.segment.id);
    ...
    if (!activeBuildingId || !previewPos || !previewSurfaceQuat)
        return null;
    return (_jsx("group", { position: previewPos, children: _jsx("group", { "rotation-y": previewYaw, children: _jsx("group", { quaternion: previewSurfaceQuat, children: _jsx(TurbineVentPreview, { node: previewNode }) }) }) }));
};
