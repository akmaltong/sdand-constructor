import { getPerpendicularWallMoveAxis, getPlannedLinkedWallUpdates, planWallMoveJunctions, useLiveNodeOverrides, useScene, } from '@pascal-app/core';
import { getFloorplanWallThickness, getSegmentGridStep, isSegmentLongEnough, snapScalarToGrid, useWallMoveGhosts, } from '@pascal-app/editor';
 ...
 */
export const wallFloorplanMoveTarget = ({ node }) => {
    ...
    const session = {
        affectedIds: [wallId, ...linkedOriginals.map((w) => w.id)],
        apply({ planPoint, modifiers }) {
            ...
            if (dx === lastDelta[0] && dz === lastDelta[1])
                return;
            lastDelta = [dx, dz];
            const nextStart = [originalStart[0] + dx, originalStart[1] + dz];
            const nextEnd = [originalEnd[0] + dx, originalEnd[1] + dz];
            lastNextStart = nextStart;
            lastNextEnd = nextEnd;
            const plan = planWallMoveJunctions(linkedOriginals, originalStart, originalEnd, nextStart, nextEnd);
            const plannedUpdates = getPlannedLinkedWallUpdates(plan, originalStart, originalEnd, nextStart, nextEnd);
            ...
            const linkedUpdates = linkedOriginals.map((wall) => {
                ...
                if (planned) {
                    return { id: wall.id, start: planned.start, end: planned.end };
                }
                return { id: wall.id, start: wall.start, end: wall.end };
            });
            ...
            useLiveNodeOverrides
                ...
                ...linkedUpdates.map((upd) => [upd.id, { start: upd.start, end: upd.end }]),
            ]);
            ...
            const ghostBridges = bridgePreviews.map(({ ghost, wall }) => ({
                id: ghost.id,
                start: ghost.start,
                end: ghost.end,
                color: ghost.color,
        ...
        canCommit() {
            const live = useScene.getState().nodes[wallId];
            if (!live || live.type !== 'wall')
                return false;
        ...
        commit() {
            const sceneState = useScene.getState();
            const liveWall = sceneState.nodes[wallId];
            if (!liveWall || liveWall.type !== 'wall') {
                // Bail without leaving stale overrides behind.
                const overrides = useLiveNodeOverrides.getState();
                overrides.clear(wallId);
                for (const wall of linkedOriginals)
                    overrides.clear(wall.id);
                return;
            }
            const plan = planWallMoveJunctions(linkedOriginals, originalStart, originalEnd, lastNextStart, lastNextEnd);
            const linkedWallUpdates = getPlannedLinkedWallUpdates(plan, originalStart, originalEnd, lastNextStart, lastNextEnd);
