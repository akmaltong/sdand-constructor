import { calculateLevelMiters, getWallCurveLength, getWallMidpointHandlePoint, getWallPlanFootprint, isCurvedWall, } from '@pascal-app/core';
...
function floorplanWallThickness(wall) {
    const baseThickness = wall.thickness ?? 0.1;
    const scaledThickness = baseThickness * FLOORPLAN_WALL_THICKNESS_SCALE;
    return Math.min(baseThickness + FLOORPLAN_MAX_EXTRA_THICKNESS, Math.max(baseThickness, scaledThickness, FLOORPLAN_MIN_VISIBLE_WALL_THICKNESS));
}
function exaggerateWallThickness(wall) {
    return { ...wall, thickness: floorplanWallThickness(wall) };
}
function formatLengthMetric(meters) {
 ...
 */
export function buildWallFloorplan(node, ctx) {
    const siblings = ctx.siblings.filter((s) => s.type === 'wall');
    ...
    const stroke = showSelectedChrome && palette
    ...
    if (isSelected) {
        children.push({
            ...
            payload: { wallId: node.id, endpoint: 'start' },
        });
        children.push({
            kind: 'endpoint-handle',
            ...
            state: 'idle',
            affordance: 'move-endpoint',
            payload: { wallId: node.id, endpoint: 'end' },
        });
        ...
        if (length >= 0.1) {
            ...
            if (isCurvedWall(node)) {
                ...
                const nx = -dz / length;
                const nz = dx / length;
                const wallSiblings = ctx.siblings.filter((s) => s.type === 'wall');
 ...
 */
function hasCurveBlockingChildren(children) {
