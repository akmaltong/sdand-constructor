import { getEffectiveWallSurfaceMaterial, sceneRegistry, } from '@pascal-app/core';
...
export function buildWallSurfaceMaterialPatch(node, targetSide, material, materialPreset) {
    const nextSurfaceMaterial = { material, materialPreset };
    const nextInterior = targetSide === 'interior'
        ? nextSurfaceMaterial
        : getEffectiveWallSurfaceMaterial(node, 'interior');
    const nextExterior = targetSide === 'exterior'
        ? nextSurfaceMaterial
 ...
 */
function applyWallPreview(node, role, material, materialPreset) {
    ...
    if (!nextMaterial)
        return null;
    const previousMaterial = wallMesh.material;
...
export const wallPaint = {
    resolveRole: ({ node, materialIndex, normal, localPosition }) => resolveWallRole({ node: node, materialIndex, normal, localPosition }),
    buildPatch: ({ node, role, material, materialPreset }) => buildWallSurfaceMaterialPatch(node, role, material, materialPreset),
    applyPreview: ({ node, role, material, materialPreset }) => applyWallPreview(node, role, material, materialPreset),
    getEffectiveMaterial: ({ node, role }) => {
        const spec = getEffectiveWallSurfaceMaterial(node, role);
