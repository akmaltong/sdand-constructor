...
import { calculateLevelMiters, collectAlignmentAnchors, emitter, getWallMiterBoundaryPoints, resolveAlignment, useScene, } from '@pascal-app/core';
import { CursorSphere, createWallOnCurrentLevel, EDITOR_LAYER, formatAngleRadians, getAngleArcToSegmentReference, getAngleToSegmentReference, getSegmentAngleReferenceAtPoint, markToolCancelConsumed, snapWallDraftPointDetailed, triggerSFX, useAlignmentGuides, useEditor, useWallSnapIndicator, WALL_FINE_GRID_STEP, } from '@pascal-app/editor';
import { getSceneTheme, useViewer } from '@pascal-app/viewer';
import { Html } from '@react-three/drei';
import { useEffect, useMemo, useRef, useState } from 'react';
import { BoxGeometry, BufferGeometry, DoubleSide, Vector3 } from 'three';
/**
...
function pointMatches(a, b, tolerance = 1e-5) {
    return distanceSquared(a, b) <= tolerance * tolerance;
}
function getNearestAxisAngleLabel(start, end, y) {
    const dx = end[0] - start[0];
    const dz = end[1] - start[1];
    const length = Math.hypot(dx, dz);
    if (length < 0.01)
        return null;
    const draftVector = [dx, dz];
    ...
    if (!nearestAxisAngle)
        return null;
    const radius = clamp(length * 0.22, DRAFT_AXIS_ANGLE_ARC_MIN_RADIUS, DRAFT_AXIS_ANGLE_ARC_MAX_RADIUS);
...
function toWallPlanPoint(point) {
    return [point.x, point.y];
}
function getWallEndpointKind(point, wall) {
    if (pointMatches(point, wall.start))
        return 'start';
    if (pointMatches(point, wall.end))
        return 'end';
    return null;
}
function buildDraftWall(start, end) {
    return {
        object: 'node',
        id: 'wall_draft',
        type: 'wall',
...
function getWallFaceAngleCandidates(point, wall, miterData) {
...
function getMatchingFaceAnglePairs(draftCandidates, connectedCandidates) {
    const candidates = [];
    ...
    const usedConnectedIndexes = new Set();
    const pairs = [];
...
function getAngleSource(endpointPoint, endpointDraftVector, connectedReference, facePairs) {
...
function getDraftAngleLabels(start, end, walls, baseY, previewHeight) {
    const draftFromStart = [end[0] - start[0], end[1] - start[1]];
    const draftFromEnd = [start[0] - end[0], start[1] - end[1]];
    ...
    for (const endpoint of endpoints) {
        const connectedWall = walls.find((wall) => Boolean(getSegmentAngleReferenceAtPoint(endpoint.point, wall)));
        if (!connectedWall)
            continue;
        ...
        const { arcCenter, connectedVector, draftVector } = getAngleSource(endpoint.point, endpoint.draftVector, connectedReference, facePairs);
        ...
        const radius = clamp(Math.min(draftLength, referenceLength) * 0.28, DRAFT_ANGLE_ARC_MIN_RADIUS, DRAFT_ANGLE_ARC_MAX_RADIUS);
        labels.push({
            id: endpoint.id,
            label: formatAngleRadians(angle),
            position: [
...
function getDraftMeasurementState(start, end, walls, unit, baseY, previewHeight) {
    const dx = end[0] - start[0];
    const dz = end[1] - start[1];
    const length = Math.hypot(dx, dz);
    if (length < 0.01)
...
function updateWallPreview(mesh, start, end, previewHeight, previewThickness) {
...
export const WallTool = () => {
    ...
    const previewThickness = typeof wallDefaults?.thickness === 'number' ? wallDefaults.thickness : DRAFT_WALL_THICKNESS;
    ...
    useEffect(() => {
        ...
        const onGridMove = (event) => {
            ...
            const localPoint = [event.localPosition[0], event.localPosition[2]];
            ...
            if (buildingState.current === 1) {
                ...
                setAxisGuide({
                    ...
                    angleLabel: getNearestAxisAngleLabel([startingPoint.current.x, startingPoint.current.z], snappedLocal, startingPoint.current.y),
                });
                const currentWallEnd = [snappedLocal[0], snappedLocal[1]];
                if (previousWallEnd &&
                    (currentWallEnd[0] !== previousWallEnd[0] || currentWallEnd[1] !== previousWallEnd[1])) {
                    triggerSFX('sfx:grid-snap');
                }
                previousWallEnd = currentWallEnd;
                updateWallPreview(wallPreviewRef.current, startingPoint.current, endingPoint.current, previewHeightRef.current, previewThicknessRef.current);
                setDraftMeasurement(getDraftMeasurementState([startingPoint.current.x, startingPoint.current.z], snappedLocal, walls, unit, startingPoint.current.y, previewHeightRef.current));
            }
            else {
        ...
        const onGridClick = (event) => {
            ...
            const walls = getCurrentLevelWalls();
            const localClick = [event.localPosition[0], event.localPosition[2]];
            const clickStep = shiftPressed.current ? WALL_FINE_GRID_STEP : undefined;
            const bypassAlign = event.nativeEvent?.altKey === true;
            if (buildingState.current === 0) {
                const snappedStart = alignPoint(snapWallDraftPointDetailed({
                    point: localClick,
                    walls,
                    step: clickStep,
                    magnetic: useEditor.getState().magneticSnap,
                ...
                const snappedEnd = alignPoint(snapWallDraftPointDetailed({
                    point: localClick,
                    walls,
                    step: clickStep,
                    magnetic: useEditor.getState().magneticSnap,
                ...
                const createdWall = createWallOnCurrentLevel([startingPoint.current.x, startingPoint.current.z], snappedEnd);
                if (!createdWall)
                    return;
    ...
    return (_jsxs("group", { children: [_jsx(WallAxisGuides, { guide: axisGuide, labelColor: measurementColor, labelShadowColor: measurementShadowColor }), _jsx(CursorSphere, { height: previewHeight, ref: cursorRef }), _jsxs("mesh", { layers: EDITOR_LAYER, ref: wallPreviewRef, renderOrder: 1, visible: false, children: [_jsx("shapeGeometry", {}), _jsx("meshBasicMaterial", { color: "#818cf8", depthTest: false, depthWrite: false, opacity: 0.5, side: DoubleSide, transparent: true })] }), draftMeasurement && (_jsxs(_Fragment, { children: [_jsx(DraftMeasurementLabel, { color: measurementColor, label: draftMeasurement.lengthLabel, position: draftMeasurement.lengthPosition, shadowColor: measurementShadowColor }), draftMeasurement.angleLabels.map((angleLabel) => (_jsxs("group", { children: [_jsx(DraftAngleArc, { arc: angleLabel.arc, color: measurementColor }), _jsx(DraftMeasurementLabel, { color: measurementColor, label: angleLabel.label, position: angleLabel.position, shadowColor: measurementShadowColor })] }, angleLabel.id)))] }))] }));
};
function WallAxisGuides({ guide, labelColor, labelShadowColor, }) {
    if (!guide)
        return null;
    const [x, z] = guide.origin;
    return (_jsxs(_Fragment, { children: [_jsxs("group", { position: [x, guide.y + DRAFT_AXIS_GUIDE_Y_OFFSET, z], children: [_jsx(WallAxisGuideLine, { axis: "x" }), _jsx(WallAxisGuideLine, { axis: "z" })] }), guide.angleLabel && (_jsxs(_Fragment, { children: [_jsx(DraftAngleArc, { arc: guide.angleLabel.arc, color: "#818cf8" }), _jsx(DraftMeasurementLabel, { color: labelColor, label: guide.angleLabel.label, position: guide.angleLabel.position, shadowColor: labelShadowColor })] }))] }));
}
function WallAxisGuideLine({ axis }) {
    return (_jsxs("mesh", { frustumCulled: false, layers: EDITOR_LAYER, renderOrder: 0, rotation: [0, axis === 'z' ? Math.PI / 2 : 0, 0], children: [_jsx("boxGeometry", { args: [DRAFT_AXIS_GUIDE_LENGTH, DRAFT_AXIS_GUIDE_HEIGHT, DRAFT_AXIS_GUIDE_WIDTH] }), _jsx("meshBasicMaterial", { color: "#818cf8", depthTest: false, depthWrite: false, opacity: 0.36, transparent: true })] }));
}
function DraftAngleArc({ arc, color }) {
    const geometry = useMemo(() => {
        const segmentCount = Math.max(8, Math.ceil((Math.abs(arc.endAngle - arc.startAngle) / Math.PI) * DRAFT_ANGLE_ARC_SEGMENTS));
        const points = Array.from({ length: segmentCount + 1 }, (_, index) => {
            const t = index / segmentCount;
            const angle = arc.startAngle + (arc.endAngle - arc.startAngle) * t;
            return new Vector3(arc.center[0] + Math.cos(angle) * arc.radius, arc.y, arc.center[1] + Math.sin(angle) * arc.radius);
        });
        return new BufferGeometry().setFromPoints(points);
    }, [arc]);
    return (_jsx("line", { frustumCulled: false, geometry: geometry, layers: EDITOR_LAYER, renderOrder: 2, children: _jsx("lineBasicNodeMaterial", { color: color, depthTest: false, depthWrite: false, linewidth: 2, opacity: 0.95, transparent: true }) }));
}
function DraftMeasurementLabel({ color, label, position, shadowColor, }) {
    return (_jsx(Html, { center: true, position: position, style: { pointerEvents: 'none', userSelect: 'none' }, zIndexRange: [100, 0], children: _jsx("div", { className: "whitespace-nowrap font-bold font-mono text-[15px]", style: {
                color,
