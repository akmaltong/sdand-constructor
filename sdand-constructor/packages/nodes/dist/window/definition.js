...
function readWallLength(w, scene) {
    if (!w.wallId)
        return Number.POSITIVE_INFINITY;
    const wall = scene.get(w.wallId);
    if (!wall)
        return Number.POSITIVE_INFINITY;
    return Math.hypot(wall.end[0] - wall.start[0], wall.end[1] - wall.start[1]);
}
function readWallHeight(w, scene) {
    if (!w.wallId)
        return Number.POSITIVE_INFINITY;
    const wall = scene.get(w.wallId);
    return wall?.height ?? Number.POSITIVE_INFINITY;
}
function windowWidthHandle(side) {
...
function windowHeightHandle(edge) {
...
export const windowDefinition = {
    ...
    defaults: () => {
        const stub = WindowNode.parse({ id: 'window_default', type: 'window' });
