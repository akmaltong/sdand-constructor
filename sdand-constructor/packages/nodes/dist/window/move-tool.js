...
import { collectAlignmentAnchors, emitter, isCurvedWall, sceneRegistry, spatialGridManager, useLiveTransforms, useScene, WindowNode, } from '@pascal-app/core';
import { calculateCursorRotation, calculateItemRotation, EDITOR_LAYER, getSideFromNormal, isValidWallSideFace, snapToHalf, triggerSFX, useAlignmentGuides, useEditor, } from '@pascal-app/editor';
 ...
 */
const MoveWindowTool = ({ node: movingWindowNode }) => {
    ...
    useEffect(() => {
        ...
        const meta = typeof movingWindowNode.metadata === 'object' && movingWindowNode.metadata !== null
            ? movingWindowNode.metadata
            : {};
        const isNew = !!meta.isNew;
        // Save original state (only used in move mode)
        const original = {
            position: [...movingWindowNode.position],
            rotation: [...movingWindowNode.rotation],
        ...
        let currentWallId = movingWindowNode.parentId;
        let dragAnchor = null;
        let lastTarget = null;
        const markWallDirty = (wallId) => {
            if (wallId)
                useScene.getState().dirtyNodes.add(wallId);
        };
        const lastWallDirtyAt = new Map();
        const markWallDirtyThrottled = (wallId) => {
            if (!wallId)
                return;
        ...
        const getLevelYOffset = () => {
            const id = getLevelId();
            return id ? (sceneRegistry.nodes.get(id)?.position.y ?? 0) : 0;
        };
        const getSlabElevation = (wallEvent) => spatialGridManager.getSlabElevationForWall(wallEvent.node.parentId ?? '', wallEvent.node.start, wallEvent.node.end);
        const hideCursor = () => {
        ...
        const alignmentCandidates = collectAlignmentAnchors(useScene.getState().nodes, movingWindowNode.id);
        const updateCursor = (worldPosition, cursorRotationY, valid) => {
        ...
        const resolveMoveTarget = (event) => {
            ...
            if (!dragAnchor || dragAnchor.wallId !== event.node.id) {
                dragAnchor = {
                    ...
                    startY: event.node.id === original.parentId ? original.position[1] : snapToHalf(rawLocalY),
                };
            }
            ...
            const { clampedX, clampedY } = clampToWall(event.node, localX, targetLocalY, movingWindowNode.width, movingWindowNode.height);
            const valid = !hasWallChildOverlap(event.node.id, clampedX, clampedY, movingWindowNode.width, movingWindowNode.height, movingWindowNode.id);
            return {
                wallNode: event.node,
                wallId: event.node.id,
                side,
        ...
        const applyPreview = (target) => {
            if (currentWallId !== target.wallId) {
                ...
                const windowMesh = sceneRegistry.nodes.get(movingWindowNode.id);
                if (windowMesh) {
            ...
            updateCursor(wallLocalToWorld(target.wallNode, target.clampedX, target.clampedY, getLevelYOffset(), getSlabElevation(target.event)), target.cursorRotation, target.valid);
        };
        const onWallEnter = (event) => {
        ...
        const onWallClick = (event) => {
            ...
            if (isNew) {
                ...
                const cloned = structuredClone(movingWindowNode);
                delete cloned.id;
                ...
                useScene.getState().createNode(node, target.wallId);
                placedId = node.id;
            }
            else {
        ...
        const onWallLeave = () => {
            ...
            lastTarget = null;
            if (isNew)
                return; // No original to restore for duplicates
        ...
        return () => {
            ...
            const current = useScene.getState().nodes[movingWindowNode.id];
    ...
    const edgesGeo = useMemo(() => {
        const boxGeo = new BoxGeometry(movingWindowNode.width, movingWindowNode.height, movingWindowNode.frameDepth ?? 0.07);
        const geo = new EdgesGeometry(boxGeo);
        boxGeo.dispose();
        return geo;
    }, [movingWindowNode]);
    return (_jsx("group", { ref: cursorGroupRef, visible: false, children: _jsx("lineSegments", { geometry: edgesGeo, layers: EDITOR_LAYER, material: edgeMaterial }) }));
};
