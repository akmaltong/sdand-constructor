...
import { collectAlignmentAnchors, emitter, isCurvedWall, sceneRegistry, spatialGridManager, useScene, WindowNode, } from '@pascal-app/core';
import { calculateCursorRotation, calculateItemRotation, EDITOR_LAYER, getSideFromNormal, isValidWallSideFace, snapToHalf, triggerSFX, useAlignmentGuides, } from '@pascal-app/editor';
...
const WindowTool = () => {
    ...
    useEffect(() => {
        ...
        const onWallEnter = (event) => {
            ...
            useScene.getState().createNode(node, event.node.id);
            draftRef.current = node;
            const valid = !hasWallChildOverlap(event.node.id, clampedX, clampedY, width, height, node.id);
            updateCursor(wallLocalToWorld(event.node, clampedX, clampedY, getLevelYOffset(), getSlabElevation(event)), cursorRotation, valid);
        ...
        const onWallMove = (event) => {
            ...
            const valid = !hasWallChildOverlap(event.node.id, clampedX, clampedY, width, height, draftRef.current?.id);
            updateCursor(wallLocalToWorld(event.node, clampedX, clampedY, getLevelYOffset(), getSlabElevation(event)), cursorRotation, valid);
            event.stopPropagation();
        };
        const onWallClick = (event) => {
            ...
            const localY = snapToHalf(event.localPosition[1]);
            const { clampedX, clampedY } = clampToWall(event.node, localX, localY, draftRef.current.width, draftRef.current.height);
            const valid = !hasWallChildOverlap(event.node.id, clampedX, clampedY, draftRef.current.width, draftRef.current.height, draftRef.current.id);
            if (!valid)
                return;
            ...
            const windowCount = Object.values(state.nodes).filter((n) => {
                if (n.type !== 'window')
                    return false;
                const wall = n.parentId ? state.nodes[n.parentId] : undefined;
    ...
    boxGeo.dispose();
    return (_jsx("group", { ref: cursorGroupRef, visible: false, children: _jsx("lineSegments", { geometry: edgesGeo, layers: EDITOR_LAYER, material: edgeMaterial, ref: edgesRef }) }));
};
