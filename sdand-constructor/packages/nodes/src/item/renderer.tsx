...
// Sdand: маркеры для примитивных стендов, вместо .glb-модели.
//   `primitive:box:<hex>`     — куб указанного цвета
//   `primitive:tex:<url>`     — куб с натянутой на грани текстурой (баннер)
// URL внутри `primitive:tex:` может быть data:/blob: — от импорта пользователя.
type PrimitiveAsset =
  | { kind: 'box'; color: string }
  | { kind: 'tex'; url: string }

function parsePrimitive(src: string | undefined): PrimitiveAsset | null {
  if (!src) return null
  ...
  return (
    <group position={node.position} ref={ref} rotation={node.rotation} visible={node.visible}>
      {roomClearPreview ? (
        ...
        <>
          {primitive.kind === 'box' ? (
            <PrimitiveBoxItem color={primitive.color} node={node} />
          ) : (
