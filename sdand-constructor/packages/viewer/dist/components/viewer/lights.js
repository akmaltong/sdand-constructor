...
const SHADOWS_DISABLED = typeof window !== 'undefined' &&
    new Set((new URLSearchParams(window.location.search).get('disable') ?? '')
        .split(',')
        .map((s) => s.trim())).has('shadows');
...
const SHADOW_EXCLUDED_TYPES = ['site'];
...
export function Lights() {
    ...
    useFrame((state, delta) => {
        ...
        if (shadows) {
            ...
            if (now - lastBoundsTime.current >= BOUNDS_REFRESH_INTERVAL) {
                ...
                for (const [id, obj] of sceneRegistry.nodes) {
                    if (SHADOW_EXCLUDED_TYPES.some((t) => sceneRegistry.byType[t].has(id)))
                        continue;
                ...
                const finiteBounds = !box.isEmpty() &&
        ...
        for (let index = 0; index < theme.lights.length; index++) {
            ...
            if (config.castShadow && light.shadow) {
                if (light.shadow.intensity !== undefined) {
                    light.shadow.intensity = THREE.MathUtils.lerp(light.shadow.intensity, config.intensity <= 1 ? config.intensity : MAX_SHADOW_INTENSITY, dt);
                }
            }
        }
        if (hemiRef.current && theme.hemi) {
            hemiRef.current.intensity = THREE.MathUtils.lerp(hemiRef.current.intensity, theme.hemi.intensity, dt);
        ...
        if (ambientRef.current) {
            ambientRef.current.intensity = THREE.MathUtils.lerp(ambientRef.current.intensity, theme.ambient.intensity, dt);
            targets.ambColor.set(theme.ambient.color);
            ambientRef.current.color.lerp(targets.ambColor, dt);
        }
    });
    return (_jsxs(_Fragment, { children: [theme.lights.map((light, index) => (_jsx("directionalLight", { castShadow: Boolean(light.castShadow) && !SHADOWS_DISABLED && shadows, position: light.position, ref: (ref) => {
                    lightRefs.current[index] = ref;
                }, "shadow-bias": -0.002, "shadow-mapSize": [1024, 1024], "shadow-normalBias": 0.3, "shadow-radius": 1.5, children: light.castShadow && !SHADOWS_DISABLED && shadows ? (_jsx("orthographicCamera", { attach: "shadow-camera", bottom: -shadowCameraSize, far: 400, left: -shadowCameraSize, near: 1, ref: shadowCamera, right: shadowCameraSize, top: shadowCameraSize })) : null }, `${index}-${light.position.join(',')}`))), theme.hemi ? _jsx("hemisphereLight", { ref: hemiRef }) : null, _jsx("ambientLight", { ref: ambientRef })] }));
}
