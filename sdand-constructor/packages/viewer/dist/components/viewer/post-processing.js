...
const PERF_POST_FX_DISABLED = typeof window !== 'undefined' &&
    new Set((new URLSearchParams(window.location.search).get('disable') ?? '')
        .split(',')
        .map((s) => s.trim())).has('postFx');
...
export const DEFAULT_HOVER_STYLES = {
    default: DEFAULT_HOVER_STYLE,
};
function sanitizeOutlineObjects(objects) {
    let nextIndex = 0;
...
const PostProcessingPasses = ({ hoverStyles = DEFAULT_HOVER_STYLES, }) => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            let scenePassDepth = null;
            let scenePassNormal = null;
            ...
            if (outlineEnabled) {
                ...
                const hoverOutline = outlineNode.secondaryVisibleEdge
                    ...
                    .mul(hoverStrength)
                    .mul(osc);
                compositeWithOutlines = vec4(add(sceneColor.rgb, selectedOutline.add(hoverOutline)), sceneColor.a);
            }
            ...
            hasPipelineErrorRef.current = true;
            console.error('[viewer/post-processing] Failed to set up post-processing pipeline. Rendering without post FX.', {
                version: pipelineVersion,
