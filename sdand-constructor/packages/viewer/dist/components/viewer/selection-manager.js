...
import { emitter, getSelectableKinds, pointInPolygon, sceneRegistry, useScene, } from '@pascal-app/core';
...
export const SelectionManager = () => {
    ...
    useEffect(() => {
        ...
        const registryKinds = getSelectableKinds().filter((k) => !allTypes.includes(k));
...
const PointerMissedHandler = ({ clickHandledRef, }) => {
    ...
    useEffect(() => {
        const handleClick = (event) => {
            ...
            if (viewerState.cameraDragging || viewerState.inputDragging)
                return;
            if (event.button !== 0)
                return;
...
const OutlinerSync = () => {
    ...
    useEffect(() => {
        ...
        if (hoveredId) {
            const hoveredNode = nodes[hoveredId];
