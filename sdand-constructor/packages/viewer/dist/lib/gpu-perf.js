...
export const PERF_OVERLAY_ENABLED = typeof window !== 'undefined' && new URLSearchParams(window.location.search).has('perf');
const MAX_SAMPLES = 256;
const samples = [];
export function pushGpuSample(ms) {
    samples.push(ms);
    if (samples.length > MAX_SAMPLES)
        samples.shift();
}
export function drainGpuSamples() {
    if (samples.length === 0)
        return [];
