import { abs, colorToDirection, float, max, min, mix, screenSize, screenUV, smoothstep, vec2, } from 'three/tsl';
...
export function inkedEdges({ depthTex, normalTex, inkColor, radius, opacity, sceneRgb, }) {
    ...
    const nDiff = max(max(float(1).sub(nC.dot(nR)), float(1).sub(nC.dot(nL))), max(float(1).sub(nC.dot(nU)), float(1).sub(nC.dot(nD))));
    ...
    return mix(sceneRgb, inkColor, edgeMask);
}
