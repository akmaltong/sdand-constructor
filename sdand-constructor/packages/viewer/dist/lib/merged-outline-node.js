 ...
 */
import { DepthTexture, FloatType, RenderTarget, Vector2 } from 'three';
import { color, exp, Fn, float, int, Loop, min, mul, nodeObject, orthographicDepthToViewZ, passTexture, perspectiveDepthToViewZ, positionView, reference, screenUV, texture, textureSize, uniform, uv, vec2, vec3, vec4, } from 'three/tsl';
import { NodeMaterial, NodeUpdateType, QuadMesh, RendererUtils, SpriteNodeMaterial, TempNode, } from 'three/webgpu';
...
let _rendererState; // eslint-disable-line @typescript-eslint/no-explicit-any
// ---------------------------------------------------------------------------
// Helper: render targets for one outline group
// ---------------------------------------------------------------------------
function makeGroupTargets(downSampleRatio) {
...
export class MergedOutlineNode extends TempNode {
    ...
    primaryObjects;
    secondaryObjects;
    primaryEdgeThicknessNode;
    secondaryEdgeThicknessNode;
    primaryEdgeGlowNode;
    secondaryEdgeGlowNode;
    downSampleRatio;
    updateBeforeType;
    _depthRT;
    _depthTexUniform;
    ...
    _cameraFar;
    _depthMaterial;
    _depthSpriteMaterial;
    _prepareMaskMatA;
    _prepareMaskSpriteMatA;
    _copyMatA;
    _edgeDetectMatA;
    _blurMat1A;
    _blurMat2A;
    ...
    _wroteGroupALastFrame = false;
    _wroteGroupBLastFrame = false;
    _textureNodeA;
    _textureNodeB;
    constructor(scene, camera, params = {}) {
        super('vec4');
        const { primaryObjects = [], secondaryObjects = [], primaryEdgeThickness = float(1), secondaryEdgeThickness = float(1), primaryEdgeGlow = float(0), secondaryEdgeGlow = float(0), downSampleRatio = 2, } = params;
        this.scene = scene;
        this.camera = camera;
    ...
    updateBefore(frame) {
        ...
        renderer.setRenderObjectFunction((obj, sc, cam, geo, _mat, grp, lights, clip) => {
            if (!hasDrawableGeometry(geo))
                return;
        ...
        if (hasPrimary)
            this._runEdgePipeline(renderer, 'A');
        if (hasSecondary)
            this._runEdgePipeline(renderer, 'B');
        RendererUtils.restoreRendererAndSceneState(renderer, scene, _rendererState);
    }
    _runEdgePipeline(renderer, group) {
        const isA = group === 'A';
    ...
    setup(_builder) {
        ...
        // ── Edge detection ────────────────────────────────────────────────────────
        const buildEdgeDetect = (maskDownTex) => Fn(() => {
        ...
        // ── Separable blur ────────────────────────────────────────────────────────
        const MAX_RADIUS = 4;
        const gaussianPdf = Fn(([x, sigma]) => float(0.398_94).mul(exp(float(-0.5).mul(x).mul(x).div(sigma.mul(sigma))).div(sigma)));
        const buildBlur = (maskDownTex, blurColorTex, blurDir, kernelRadius) => Fn(() => {
            const resolution = textureSize(maskDownTex);
            const invSize = vec2(1).div(resolution).toVar();
            const uvNode = uv();
            ...
            Loop({ start: int(1), end: int(MAX_RADIUS), type: 'int', condition: '<=' }, ({ i }) => {
                ...
                diffuseSum.addAssign(blurColorTex
        ...
        this._blurMat1A.fragmentNode = buildBlur(this._maskDownTexA, this._blurColorTexA, this._blurDirectionA, this.primaryEdgeThicknessNode);
        this._blurMat1A.needsUpdate = true;
        this._blurMat2A.fragmentNode = buildBlur(this._maskDownTexA, this._blurColorTexA, this._blurDirectionA, float(MAX_RADIUS));
        this._blurMat2A.needsUpdate = true;
        this._blurMat1B.fragmentNode = buildBlur(this._maskDownTexB, this._blurColorTexB, this._blurDirectionB, this.secondaryEdgeThicknessNode);
        this._blurMat1B.needsUpdate = true;
        this._blurMat2B.fragmentNode = buildBlur(this._maskDownTexB, this._blurColorTexB, this._blurDirectionB, float(MAX_RADIUS));
        this._blurMat2B.needsUpdate = true;
        // ── Composite ─────────────────────────────────────────────────────────────
        const buildComposite = (maskTex, edge1Tex, edge2Tex, edgeGlowNode) => Fn(() => maskTex.r.mul(edge1Tex.add(edge2Tex.mul(edgeGlowNode))))();
        this._compositeMatA.fragmentNode = buildComposite(this._maskTexA, this._edge1TexA, this._edge2TexA, this.primaryEdgeGlowNode);
        this._compositeMatA.needsUpdate = true;
        this._compositeMatB.fragmentNode = buildComposite(this._maskTexB, this._edge1TexB, this._edge2TexB, this.secondaryEdgeGlowNode);
...
export const mergedOutline = (scene, camera, params) => new MergedOutlineNode(scene, camera, params);
