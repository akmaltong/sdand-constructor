import { clampDoorOperationState, DoorNode as DoorNodeSchema, getDoorRenderOpenAmount, getEffectiveNode, sceneRegistry, useInteractive, useLiveNodeOverrides, useScene, } from '@pascal-app/core';
...
let revealMaterial = defaultRevealMaterial;
...
function normalizeDoorNodeForRender(node) {
...
function addRotatedBox(parent, material, w, h, d, x, y, z, rotationY) {
    const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), material);
    m.position.set(x, y, z);
    m.rotation.y = rotationY;
    parent.add(m);
}
function addBoxWithRotation(parent, material, w, h, d, x, y, z, rotation) {
    const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), material);
    m.position.set(x, y, z);
    m.rotation.set(rotation[0], rotation[1], rotation[2]);
    parent.add(m);
}
function addShape(parent, material, shape, depth) {
...
function addShapeAt(parent, material, shape, depth, x, y, z) {
    const geometry = new THREE.ExtrudeGeometry(shape, {
        depth,
        bevelEnabled: false,
        curveSegments: 24,
...
function getOneSidedArchBoundaryYAtX(x, left, right, top, archHeight, curvedSide) {
    const width = right - left;
    if (width <= 1e-6)
        return top;
    const clampedArchHeight = getClampedArchHeight(width, Number.MAX_SAFE_INTEGER, archHeight);
    const springY = top - clampedArchHeight;
    const distanceFromApex = curvedSide === 'left'
        ? Math.max(0, Math.min((right - x) / width, 1))
        : Math.max(0, Math.min((x - left) / width, 1));
    return (springY + clampedArchHeight * Math.sqrt(Math.max(1 - distanceFromApex * distanceFromApex, 0)));
}
...
function createArchBandShape(width, outerSpringY, outerTopY, innerSpringY, innerTopY, insetX) {
    ...
    const shape = new THREE.Shape();
    const segments = 32;
    const getSafeInnerBoundaryY = (x) => Math.min(getArchBoundaryY(x, innerHalfWidth, safeInnerSpringY, innerArchHeight), getArchBoundaryY(x, halfWidth, outerSpringY, outerArchHeight) - 0.001);
...
function createArchHeadBarShape(width, bottomY, springY, topY) {
    const halfWidth = width / 2;
...
function normalizeTopCornerRadii(radii, width, height) {
    const next = { ...radii };
    const scale = Math.min(1, width / Math.max(next.topLeft + next.topRight, 1e-6), height / Math.max(next.topLeft, 1e-6), height / Math.max(next.topRight, 1e-6));
    if (scale < 1) {
        next.topLeft *= scale;
        next.topRight *= scale;
    }
...
function getDoorTopRadii(node, width, height) {
...
function createRoundedTopShape(left, right, bottom, top, radii) {
    const shape = new THREE.Shape();
...
function createRoundedDoorFrameShape(width, height, frameThickness, radii) {
    const halfWidth = width / 2;
    ...
    }, innerRight - innerLeft, innerTop - bottom);
...
function shapeToReversedPath(shape) {
    return new THREE.Path(shape.getPoints(40).reverse());
}
function createRoundedLeafFrameShape(width, bottom, top, radii, insetX, insetY) {
    const halfWidth = width / 2;
    ...
    if (innerRight <= innerLeft + 0.01 || innerTop <= innerBottom + 0.01)
        return outer;
    const innerRadii = normalizeTopCornerRadii({
    ...
    }, innerRight - innerLeft, innerTop - innerBottom);
    outer.holes.push(shapeToReversedPath(createRoundedTopShape(innerLeft, innerRight, innerBottom, innerTop, innerRadii)));
    return outer;
}
function createRoundedClippedLeafFrameShape(left, right, bottom, top, fullLeft, fullRight, radii, insetX, insetY) {
    const outerRadii = normalizeTopCornerRadii(radii, fullRight - fullLeft, top - bottom);
    const outer = createTopClippedRectShape(left, right, bottom, top, (x) => getRoundedBoundaryYAtX(x, fullLeft, fullRight, top, {
    ...
    const innerRadii = normalizeTopCornerRadii({
        topLeft: Math.max(outerRadii.topLeft - Math.max(insetX, insetY), 0),
        topRight: Math.max(outerRadii.topRight - Math.max(insetX, insetY), 0),
    }, innerFullRight - innerFullLeft, innerTop - innerBottom);
    const holeShape = createTopClippedRectShape(innerLeft, innerRight, innerBottom, innerTop, (x) => getRoundedBoundaryYAtX(x, innerFullLeft, innerFullRight, innerTop, {
        topLeft: innerRadii.topLeft,
        topRight: innerRadii.topRight,
    }));
    if (holeShape)
        outer.holes.push(shapeToReversedPath(holeShape));
    return outer;
}
function createArchedLeafFrameShape(width, bottom, top, archHeight, insetX, insetY) {
    const halfWidth = width / 2;
    ...
    if (innerRight <= innerLeft + 0.01 || innerTop <= innerBottom + 0.01)
        return outer;
    const innerArchHeight = getClampedArchHeight(innerRight - innerLeft, innerTop - innerBottom, Math.max(archHeight - insetY, 0.01));
    outer.holes.push(shapeToReversedPath(createArchShape(innerLeft, innerRight, innerBottom, innerTop, innerArchHeight)));
    return outer;
}
function createArchedClippedLeafFrameShape(left, right, bottom, top, fullLeft, fullRight, archHeight, insetX, insetY) {
    const fullCenterX = (fullLeft + fullRight) / 2;
    const fullHalfWidth = (fullRight - fullLeft) / 2;
    const springY = top - archHeight;
    const outer = createTopClippedRectShape(left, right, bottom, top, (x) => getArchBoundaryY(x - fullCenterX, fullHalfWidth, springY, archHeight));
    if (!outer)
        return null;
    const innerLeft = left + insetX;
    const innerRight = right - insetX;
    ...
    const innerFullLeft = fullLeft + insetX;
    const innerFullRight = fullRight - insetX;
    const innerArchHeight = getClampedArchHeight(innerFullRight - innerFullLeft, innerTop - innerBottom, Math.max(archHeight - insetY, 0.01));
    ...
    const holeShape = createTopClippedRectShape(innerLeft, innerRight, innerBottom, innerTop, (x) => getArchBoundaryY(x - innerFullCenterX, innerFullHalfWidth, innerSpringY, innerArchHeight));
    if (holeShape)
        outer.holes.push(shapeToReversedPath(holeShape));
    return outer;
}
function createOneSidedArchLeafFrameShape(left, right, bottom, top, archHeight, insetX, insetY, curvedSide) {
    const outer = createTopClippedRectShape(left, right, bottom, top, (x) => getOneSidedArchBoundaryYAtX(x, left, right, top, archHeight, curvedSide));
    if (!outer)
        return null;
    const innerLeft = left + insetX;
    const innerRight = right - insetX;
    ...
    if (innerRight <= innerLeft + 0.01 || innerTop <= innerBottom + 0.01)
        return outer;
    const innerArchHeight = getClampedArchHeight(innerRight - innerLeft, innerTop - innerBottom, Math.max(archHeight - insetY, 0.01));
    const holeShape = createTopClippedRectShape(innerLeft, innerRight, innerBottom, innerTop, (x) => getOneSidedArchBoundaryYAtX(x, innerLeft, innerRight, innerTop, innerArchHeight, curvedSide));
    if (holeShape)
        outer.holes.push(shapeToReversedPath(holeShape));
    return outer;
}
function createTopClippedRectShape(left, right, bottom, top, getBoundaryY) {
    const segments = 20;
    const points = [];
...
function addLeafSegmentContent({ addLeafBox, addLeafShape, leafWidth, leafHeight, leafCenterX, leafCenterY, leafDepth, segments, contentPadding, keepFrameWhenEmpty = false, renderPerimeterFrame = true, openingShape = 'rectangle', openingTopRadii, archHeight, archOuterSide, }) {
    ...
    if (renderPerimeterFrame && shouldRenderFrame && cpY > 0) {
        addLeafBox(baseMaterial, leafWidth, cpY, leafDepth, leafCenterX, leafCenterY + leafHeight / 2 - cpY / 2, 0);
        addLeafBox(baseMaterial, leafWidth, cpY, leafDepth, leafCenterX, leafCenterY - leafHeight / 2 + cpY / 2, 0);
    }
    if (renderPerimeterFrame && shouldRenderFrame && cpX > 0) {
        const innerH = leafHeight - 2 * cpY;
        addLeafBox(baseMaterial, cpX, innerH, leafDepth, leafCenterX - leafWidth / 2 + cpX / 2, leafCenterY, 0);
        addLeafBox(baseMaterial, cpX, innerH, leafDepth, leafCenterX + leafWidth / 2 - cpX / 2, leafCenterY, 0);
    }
    ...
    const clampedLeafArchHeight = openingShape === 'arch' ? getClampedArchHeight(leafWidth, leafHeight, archHeight) : 0;
    const leafSpringY = leafTop - clampedLeafArchHeight;
    const topBoundaryAtX = (x) => {
        ...
        if (openingShape === 'arch') {
            if (archOuterSide) {
                return getOneSidedArchBoundaryYAtX(x, leafLeft, leafRight, leafTop, clampedLeafArchHeight, archOuterSide);
            }
    ...
    for (const seg of segments) {
        ...
        if (seg.type !== 'empty') {
            ...
            for (let c = 0; c < numCols - 1; c++) {
                ...
                const dividerShape = hasShapedTop && addLeafShape
                    ? createTopClippedRectShape(dividerLeft, dividerRight, segBottom, segTop, topBoundaryAtX)
                    : null;
                if (dividerShape && addLeafShape) {
                    addLeafShape(baseMaterial, dividerShape, leafDepth + 0.001);
                }
                else {
                    addLeafBox(baseMaterial, seg.dividerThickness, segH, leafDepth + 0.001, cx + seg.dividerThickness / 2, segCenterY, 0);
                }
                cx += seg.dividerThickness;
            }
        }
        for (let c = 0; c < numCols; c++) {
            ...
            if (seg.type === 'glass') {
                ...
                const glassShape = hasShapedTop && addLeafShape
                    ? createTopClippedRectShape(segmentLeft, segmentRight, segBottom, segTop, topBoundaryAtX)
                    : null;
                ...
                const segmentLeft = colX - colW / 2;
                const segmentRight = colX + colW / 2;
                const outerPanelShape = hasShapedTop && addLeafShape
                    ? createTopClippedRectShape(segmentLeft, segmentRight, segBottom, segTop, topBoundaryAtX)
                    : null;
                ...
                if (panelW > 0.01 && panelH > 0.01) {
                    ...
                    const innerPanelShape = hasShapedTop && addLeafShape
                        ? createTopClippedRectShape(insetLeft, insetRight, insetBottom, insetTop, topBoundaryAtX)
                        : null;
...
function addDoorLeaf(mesh, { leafWidth, leafHeight, leafCenterX, leafCenterY, leafDepth, hingeX, hingeSide, swingRotation, segments, contentPadding, handle, handleBothSides = false, handleHeight, handleSide, doorCloser, panicBar, panicBarHeight, doorHeight, openingShape, openingTopRadii, archHeight, roundedBoundary, archedBoundary, archOuterSide, }) {
    const hasLeafContent = segments.some((seg) => seg.type !== 'empty');
    const leafGroup = new THREE.Group();
    leafGroup.position.set(hingeX, 0, 0);
    leafGroup.rotation.y = swingRotation;
    mesh.add(leafGroup);
    const addLeafBox = (material, w, h, d, x, y, z) => addBox(leafGroup, material, w, h, d, x - hingeX, y, z);
    const addLeafShape = (material, shape, depth) => addShapeAt(leafGroup, material, shape, depth, -hingeX, 0, 0);
    ...
    if (usesShapedLeafFrame && hasLeafContent) {
        if (openingShape === 'rounded') {
            const roundedLeafShape = roundedBoundary
                ? createRoundedClippedLeafFrameShape(leafCenterX - leafWidth / 2, leafCenterX + leafWidth / 2, leafBottom, leafTop, roundedBoundary.fullLeft, roundedBoundary.fullRight, roundedBoundary.radii, contentPadding[0], contentPadding[1])
                : createRoundedLeafFrameShape(leafWidth, leafBottom, leafTop, openingTopRadii, contentPadding[0], contentPadding[1]);
            if (roundedLeafShape) {
                if (roundedBoundary) {
            ...
            const archedLeafShape = archOuterSide
                ? createOneSidedArchLeafFrameShape(leafCenterX - leafWidth / 2, leafCenterX + leafWidth / 2, leafBottom, leafTop, archHeight, contentPadding[0], contentPadding[1], archOuterSide)
                : archedBoundary
                    ? createArchedClippedLeafFrameShape(leafCenterX - leafWidth / 2, leafCenterX + leafWidth / 2, leafBottom, leafTop, archedBoundary.fullLeft, archedBoundary.fullRight, archedBoundary.archHeight, contentPadding[0], contentPadding[1])
                    : createArchedLeafFrameShape(leafWidth, leafBottom, leafTop, getClampedArchHeight(leafWidth, leafHeight, archHeight), contentPadding[0], contentPadding[1]);
            if (archedLeafShape) {
    ...
    if (hasLeafContent && doorCloser) {
        ...
        addLeafBox(baseMaterial, 0.14, 0.015, 0.015, leafCenterX + leafWidth / 4, closerY + 0.025, leafDepth / 2 + 0.015);
    }
...
function addFoldingDoor(mesh, { insideWidth, leafHeight, leafCenterY, leafDepth, frameThickness, frameDepth, operationState, leafCount, doorHeight, handleHeight, segments, contentPadding, }) {
    ...
    addBox(mesh, baseMaterial, insideWidth, Math.min(frameThickness * 0.5, 0.025), Math.max(frameDepth * 0.45, 0.035), 0, leafCenterY + leafHeight / 2 - 0.018, 0);
    const vertices = [{ x: -insideWidth / 2, z: 0 }];
    ...
    for (let index = 0; index < panelCount; index++) {
        ...
        const addFoldingLeafBox = (material, w, h, d, x, y, z) => {
            addRotatedBox(mesh, material, w, h, d, centerX + localX.x * x + Math.sin(rotationY) * z, y, centerZ + localX.z * x + Math.cos(rotationY) * z, rotationY);
        };
        addLeafSegmentContent({
        ...
        for (const point of [start, end]) {
            addBox(mesh, revealMaterial, 0.018, leafHeight * 0.92, leafDepth + 0.016, point.x, leafCenterY, point.z);
        }
    }
    const handlePoint = vertices[vertices.length - 1];
    const handleY = handleHeight - doorHeight / 2;
    addBox(mesh, baseMaterial, 0.035, 0.16, leafDepth + 0.035, handlePoint.x - 0.035, handleY, handlePoint.z + 0.045);
    addBox(mesh, baseMaterial, 0.035, 0.16, leafDepth + 0.035, handlePoint.x - 0.035, handleY, handlePoint.z - 0.045);
}
function addPocketDoor(mesh, { insideWidth, leafHeight, leafCenterY, leafDepth, frameThickness, frameDepth, operationState, slideDirection, doorHeight, handleHeight, segments, contentPadding, }) {
    ...
    addBox(mesh, baseMaterial, insideWidth * 2, Math.min(frameThickness * 0.45, 0.024), Math.max(frameDepth * 0.38, 0.03), slideSign * (insideWidth / 2), topY - 0.018, 0);
    addBox(mesh, revealMaterial, insideWidth * 0.9, 0.018, Math.max(frameDepth * 0.32, 0.026), pocketCenterX, topY - 0.055, 0);
    addBox(mesh, revealMaterial, 0.018, leafHeight * 0.94, leafDepth + 0.014, slideSign * insideWidth * 0.5, leafCenterY, 0);
    const addPocketLeafBox = (material, w, h, d, x, y, z) => addBox(mesh, material, w, h, d, x, y, z);
    addLeafSegmentContent({
...
function addBarnDoor(mesh, { insideWidth, leafHeight, leafCenterY, leafDepth, frameThickness, frameDepth, operationState, slideDirection, doorHeight, handleHeight, segments, contentPadding, }) {
    const openAmount = clampDoorOperationState(operationState);
    const slideSign = slideDirection === 'right' ? 1 : -1;
    ...
    addBox(mesh, revealMaterial, 0.05, 0.13, 0.035, insideWidth / 2, trackY - 0.02, faceZ + 0.01);
    const addBarnLeafBox = (material, w, h, d, x, y, z) => addBox(mesh, material, w, h, d, x, y, faceZ + z);
    addLeafSegmentContent({
    ...
    addRotatedBox(mesh, revealMaterial, 0.018, leafHeight * 0.86, 0.012, leafCenterX, leafCenterY, faceZ + leafDepth / 2 + 0.014, -0.52);
    addRotatedBox(mesh, revealMaterial, 0.018, leafHeight * 0.86, 0.012, leafCenterX, leafCenterY, faceZ + leafDepth / 2 + 0.014, 0.52);
    for (const offset of [-leafWidth * 0.28, leafWidth * 0.28]) {
        ...
        addBox(mesh, revealMaterial, 0.026, 0.16, 0.026, leafCenterX + offset, wheelY - 0.075, faceZ + 0.022);
    }
    addBox(mesh, baseMaterial, 0.032, 0.22, leafDepth + 0.034, handleX, handleY, faceZ + leafDepth / 2 + 0.02);
    addBox(mesh, baseMaterial, 0.032, 0.22, leafDepth + 0.034, handleX, handleY, faceZ - leafDepth / 2 - 0.02);
}
function addSlidingDoor(mesh, { insideWidth, leafHeight, leafCenterY, leafDepth, frameThickness, frameDepth, operationState, slideDirection, doorHeight, handleHeight, segments, contentPadding, }) {
    const openAmount = clampDoorOperationState(operationState);
    ...
    addBox(mesh, revealMaterial, insideWidth, 0.018, Math.max(frameDepth * 0.28, 0.022), 0, -leafHeight / 2 + 0.04, 0);
    const addFixedPanelBox = (material, w, h, d, x, y, z) => addBox(mesh, material, w, h, d, x + fixedX, y, z + backZ);
    const addActivePanelBox = (material, w, h, d, x, y, z) => addBox(mesh, material, w, h, d, x + activeX, y, z + frontZ);
    addLeafSegmentContent({
...
function addGarageSectionalDoor(mesh, { insideWidth, leafHeight, leafCenterY, leafDepth, frameThickness, frameDepth, operationState, garagePanelCount, }) {
    ...
    addBox(mesh, revealMaterial, 0.035, Math.max(0.04, frameThickness * 0.75), travelDepth, -insideWidth / 2 + 0.035, railY, railZ);
    addBox(mesh, revealMaterial, 0.035, Math.max(0.04, frameThickness * 0.75), travelDepth, insideWidth / 2 - 0.035, railY, railZ);
    for (let index = 0; index < panelCount; index++) {
        ...
        const addSectionalTrim = (localY) => {
            addBoxWithRotation(mesh, revealMaterial, insideWidth - 0.16, 0.012, trimDepth, 0, y + localY * Math.cos(rotationX) - trimFaceOffset * Math.sin(rotationX), z + localY * Math.sin(rotationX) + trimFaceOffset * Math.cos(rotationX), [rotationX, 0, 0]);
        };
        addBoxWithRotation(mesh, baseMaterial, insideWidth, Math.max(0.04, panelHeight - panelGap), leafDepth, 0, y, z, [rotationX, 0, 0]);
        addSectionalTrim(revealOffset);
        addSectionalTrim(-revealOffset);
    }
    addBox(mesh, revealMaterial, insideWidth, 0.032, Math.max(frameDepth * 0.36, 0.03), 0, railY, 0);
}
function addGarageRollupDoor(mesh, { insideWidth, leafHeight, leafCenterY, leafDepth, frameThickness, frameDepth, operationState, }) {
    const openAmount = clampDoorOperationState(operationState);
    ...
    addBox(mesh, revealMaterial, 0.032, leafHeight, Math.max(frameDepth * 0.48, 0.035), -insideWidth / 2 + 0.03, leafCenterY, 0);
    addBox(mesh, revealMaterial, 0.032, leafHeight, Math.max(frameDepth * 0.48, 0.035), insideWidth / 2 - 0.03, leafCenterY, 0);
    if (visibleHeight > 0.01) {
        ...
        addBox(mesh, revealMaterial, insideWidth - 0.04, 0.028, leafDepth + 0.018, 0, topY - visibleHeight, leafDepth / 2 + 0.004);
    }
    const drum = new THREE.Mesh(new THREE.CylinderGeometry(drumMaxRadius, drumMaxRadius, insideWidth + frameThickness, 36), baseMaterial);
    drum.position.set(0, drumY, drumZ);
    drum.rotation.z = Math.PI / 2;
    mesh.add(drum);
    addBox(mesh, revealMaterial, insideWidth + frameThickness, 0.026, Math.max(frameDepth * 0.52, 0.04), 0, topY + 0.02, 0);
}
function addGarageTiltupDoor(mesh, { insideWidth, leafHeight, leafCenterY, leafDepth, frameThickness, frameDepth, operationState, }) {
    const openAmount = clampDoorOperationState(operationState);
    ...
    addBox(mesh, revealMaterial, 0.03, Math.max(frameThickness * 0.7, 0.035), railLength, -insideWidth / 2 + 0.04, railY, railZ);
    addBox(mesh, revealMaterial, 0.03, Math.max(frameThickness * 0.7, 0.035), railLength, insideWidth / 2 - 0.04, railY, railZ);
    addBoxWithRotation(mesh, baseMaterial, insideWidth, leafHeight, leafDepth, 0, panelCenterY, panelCenterZ, [-angle, 0, 0]);
    const insetWidth = Math.max(0.1, insideWidth - 0.22);
    const insetHeight = Math.max(0.1, leafHeight - 0.28);
    const trimDepth = 0.012;
    const trimFaceOffset = leafDepth / 2 + trimDepth + 0.006;
    const addTiltupTrim = (localX, localY, trimWidth, trimHeight) => {
        addBoxWithRotation(mesh, revealMaterial, trimWidth, trimHeight, trimDepth, localX, panelCenterY + localY * Math.cos(angle) + trimFaceOffset * Math.sin(angle), panelCenterZ - localY * Math.sin(angle) + trimFaceOffset * Math.cos(angle), [-angle, 0, 0]);
    };
...
function getEffectiveOpeningShape(node) {
...
function updateDoorMesh(rawNode, mesh) {
    ...
    const { width, height, openingKind, openingShape: rawOpeningShape, frameThickness, frameDepth, threshold, thresholdHeight, segments, handle, handleHeight, handleSide, doorCloser, panicBar, panicBarHeight, contentPadding, hingesSide, swingDirection, swingAngle: nodeSwingAngle = 0, doorType = 'hinged', operationState: nodeOperationState = 0, leafCount = 1, slideDirection = 'left', garagePanelCount = 4, } = node;
    ...
    if (openingShape === 'arch') {
        ...
        addBox(mesh, baseMaterial, frameThickness, postHeight, frameDepth, -width / 2 + frameThickness / 2, frameBottom + postHeight / 2, 0);
        addBox(mesh, baseMaterial, frameThickness, postHeight, frameDepth, width / 2 - frameThickness / 2, frameBottom + postHeight / 2, 0);
        addShape(mesh, baseMaterial, useShallowHeadBar
            ? createArchHeadBarShape(width, frameHeadBottomY, frameSpringY, frameTop)
            : createArchBandShape(width, frameSpringY, frameTop, frameInnerSpringY, frameInnerTopY, frameThickness), frameDepth);
    }
    else if (openingShape === 'rounded') {
        addShape(mesh, baseMaterial, createRoundedDoorFrameShape(width, height, frameThickness, getDoorTopRadii(node, width, height)), frameDepth);
    }
    else {
    ...
    if (threshold) {
        addBox(mesh, baseMaterial, insideWidth, thresholdHeight, frameDepth, 0, -height / 2 + thresholdHeight / 2, 0);
    }
    if (doorType === 'garage-sectional') {
        ...
        const roundedBoundary = openingShape === 'rounded'
            ? {
        ...
        addDoorLeaf(mesh, {
            ...
            doorHeight: height,
            openingShape,
            openingTopRadii: openingShape === 'rounded'
...
function hideEmptyGeometryMeshes(root) {
    root.traverse((obj) => {
        const child = obj;
...
function syncDoorCutout(node, mesh) {
    ...
    if (openingShape === 'arch') {
        ...
        cutout.geometry = new THREE.ExtrudeGeometry(createRoundedTopShape(-node.width / 2, node.width / 2, -node.height / 2, node.height / 2, getDoorTopRadii(node, node.width, node.height)), {
            depth: 1,
            bevelEnabled: false,
            curveSegments: 24,
        });
