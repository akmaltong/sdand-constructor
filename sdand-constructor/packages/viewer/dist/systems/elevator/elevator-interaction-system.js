import { openElevatorDoor, requestElevatorLevel, resolveElevatorDispatchTarget, useInteractive, useScene, } from '@pascal-app/core';
import { useThree } from '@react-three/fiber';
import { useEffect, useRef } from 'react';
import { Raycaster, Vector2 } from 'three';
function getElevatorButtonData(object) {
    let current = object;
    while (current) {
        const data = current.userData.elevatorButton;
        if (data)
            return data;
...
export function ElevatorInteractionSystem() {
    ...
    useEffect(() => {
        ...
        const handlePointerDown = (event) => {
            ...
            if (!button.levelId)
                return;
            const targetElevatorId = button.kind === 'landing'
