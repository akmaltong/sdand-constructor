'use client';
import { getEffectiveNode, nodeRegistry, sceneRegistry, useScene, } from '@pascal-app/core';
...
export const GeometrySystem = () => {
    ...
    const builtGeometryKeyRef = useRef(new Map());
    useEffect(() => {
    ...
    useFrame(() => {
        ...
        for (const [key, batch] of batches) {
            ...
            if (batch.parentId) {
                ...
                for (const node of Object.values(nodes)) {
                    if (node?.type === batch.kind && !node.parentId)
                        siblings.push(node);
                }
            }
            levelDataByBatch.set(key, def.computeLevelData(siblings));
        }
        ...
        for (const id of dirtyIds) {
            ...
            if (!group)
                continue; // mount hasn't run — keep dirty for next frame
            ...
            const built = builder(effectiveNode, ctx, shading, textures, colorPreset, sceneTheme);
...
function buildGeometryContext(node, nodes, levelData) {
    const resolve = (id) => nodes[id];
    const childIds = node.children;
    const children = Array.isArray(childIds)
        ? childIds.map((cid) => nodes[cid]).filter((n) => n !== undefined)
        : [];
    ...
    let siblings = [];
    if (parent) {
        const parentChildIds = parent.children;
        if (Array.isArray(parentChildIds)) {
            for (const sid of parentChildIds) {
                if (sid === node.id)
                    continue;
                const s = nodes[sid];
                if (s && s.type === node.type)
                    siblings.push(s);
            }
        }
        else {
            siblings = Object.values(nodes).filter((n) => n !== node && n.type === node.type && n.parentId === parentId);
        }
    }
...
function disposeChildren(group) {
    ...
    for (const child of [...group.children]) {
        const fromGeometry = child.userData
            ?.__fromGeometry;
        if (!fromGeometry)
            continue;
        group.remove(child);
...
function applyDefaultSurfaceRole(root, defaultRole, colorPreset, sceneTheme) {
    root.traverse((child) => {
        const mesh = child;
        ...
        mesh.material = createSurfaceRoleMaterial(role, colorPreset, getMaterialSide(mesh.material), sceneTheme);
    });
}
function getMeshSurfaceRole(value, fallback) {
    return typeof value === 'string' && isSurfaceRole(value) ? value : fallback;
}
function isSurfaceRole(value) {
    return (value === 'wall' ||
        value === 'floor' ||
        value === 'ceiling' ||
        value === 'roof' ||
...
function getMaterialSide(material) {
    const source = Array.isArray(material) ? material[0] : material;
    return source?.side ?? FrontSide;
}
function isCachedMaterial(value) {
