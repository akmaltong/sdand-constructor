import { getEffectiveNode, getSegmentSlopeFrame, hasSegmentMaterialOverride, nodeRegistry, sceneRegistry, useLiveNodeOverrides, useScene, } from '@pascal-app/core';
import { useFrame } from '@react-three/fiber';
import * as THREE from 'three';
import { mergeVertices } from 'three/examples/jsm/utils/BufferGeometryUtils.js';
import { ADDITION, Brush, Evaluator, SUBTRACTION } from 'three-bvh-csg';
import { computeBoundsTree } from 'three-mesh-bvh';
function csgGeometry(brush) {
    return brush.geometry;
}
function csgMaterials(brush) {
...
function computeGeometryBoundsTree(geometry) {
    ;
...
const warnedMergedRoofNaNIds = new Set();
    ...
    useFrame(() => {
        ...
        dirtyNodes.forEach((id) => {
            ...
            if (def?.capabilities?.roofAccessory) {
                const segId = node.roofSegmentId;
                const seg = segId ? nodes[segId] : undefined;
                if (seg?.parentId) {
                    pendingRoofUpdates.add(seg.parentId);
                }
                clearDirty(id);
                return;
            }
            if (node.type === 'roof-segment') {
                ...
                const effectiveSegment = getEffectiveNode(node);
                if (mesh) {
                    ...
                    if (isVisible && segmentsProcessed < MAX_SEGMENTS_PER_FRAME) {
                        ...
                        mesh.position.set(effectiveSegment.position[0], effectiveSegment.position[1], effectiveSegment.position[2]);
                ...
                if (effectiveSegment.parentId) {
                    pendingRoofUpdates.add(effectiveSegment.parentId);
                }
            }
...
function updateMergedRoofGeometry(roofNode, group, nodes) {
    ...
    const children = (roofNode.children ?? [])
        ...
        .filter((n) => n !== undefined && !hasSegmentMaterialOverride(n));
    ...
    for (const child of children) {
        ...
        for (const childElemId of child.children ?? []) {
            const childElem = nodes[childElemId];
            if (!childElem)
                continue;
            const meta = typeof childElem.metadata === 'object' && childElem.metadata !== null
                ? childElem.metadata
                : undefined;
            if (meta?.isTransient)
                continue;
            ...
            try {
                const nextShin = csgEvaluator.evaluate(workingShin, cut, SUBTRACTION);
                workingShin.geometry.dispose();
                prepareBrushForCSG(nextShin);
                workingShin = nextShin;
                const nextDeck = csgEvaluator.evaluate(workingDeck, cut, SUBTRACTION);
                workingDeck.geometry.dispose();
                prepareBrushForCSG(nextDeck);
                workingDeck = nextDeck;
                const nextWall = csgEvaluator.evaluate(workingWall, cut, SUBTRACTION);
...
function geometryHasNaNPositions(geometry) {
...
export function mapRoofGroupMaterialIndex(groupMaterialIndex, csgMaterials, matToIndex) {
...
export function getRoofSegmentBrushes(node) {
    const { roofType, width, depth, wallHeight, wallThickness, deckThickness, overhang, shingleThickness, } = node;
    ...
    const getVol = (wExt, vOffset, baseY, matIndex, isVoid) => {
        ...
        const faces = getModuleFaces(roofType, wV, dV, whV, rhV, safeBaseY, { dutchI: structuralI }, width, depth, tanTheta, shapeRatios);
    ...
    const botFaces = getModuleFaces(roofType, shinBotW, shinBotD, shinBotWh, shinBotRh, botBaseY, insetsBot, width, depth, tanTheta, shapeRatios);
    const topFaces = getModuleFaces(roofType, shinTopW, shinTopD, shinTopWh, shinTopRh, topBaseY, insetsTop, width, depth, tanTheta, shapeRatios);
...
export function remapRoofShellFaces(geometry, node) {
    const position = geometry.getAttribute('position');
 ...
 */
function getModuleFaces(type, w, d, wh, rh, baseY, insets, baseW, baseD, tanTheta, shapeRatios) {
    ...
    if (type === 'flat' || rh === 0) {
        ...
        const i = insets.dutchI !== undefined
            ? insets.dutchI
...
function createGeometryFromFaces(faces, matRule = null) {
    ...
    for (const face of faces) {
        ...
        let slopeAlignedDown = null;
        let slopeAlignedAcross = null;
 ...
 */
export function getRoofOuterSurfaceFrameAtPoint(segment, lx, lz) {
    const { roofType, width, depth, wallHeight, wallThickness, deckThickness, overhang, shingleThickness, } = segment;
    ...
    const topBaseY = 0;
    const baseI = Math.min(width, depth) * 0.25;
    const getInsets = (_wh, _baseY, isVoid, _wV, _dV) => {
    ...
    const topFaces = getModuleFaces(roofType, shinTopW, shinTopD, shinTopWh, shinTopRh, topBaseY, insetsTop, width, depth, tanTheta, shapeRatios);
    const topGeo = createGeometryFromFaces(topFaces, (normal) => normal.y > SHINGLE_SURFACE_EPSILON ? 3 : 1);
    if (transZ !== 0)
        topGeo.translate(0, 0, transZ);
