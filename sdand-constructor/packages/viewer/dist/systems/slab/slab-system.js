import { getEffectiveNode, getRenderableSlabPolygon, pointInPolygon2D, polygonsIntersect, sceneRegistry, useScene, } from '@pascal-app/core';
...
export const SlabSystem = () => {
    ...
    useFrame(() => {
        ...
        dirtyNodes.forEach((id) => {
            ...
            if (mesh) {
                updateSlabGeometry(getEffectiveNode(node), mesh);
                clearDirty(id);
            }
...
function ensureCounterClockwisePolygon(polygon) {
    let area2 = 0;
...
function isStrictInteriorHole(contour, hole) {
    return (hole.every((point) => pointInPolygon2D(point, contour, { includeBoundary: false })) &&
        !polygonsIntersect(contour, hole));
}
function affectsContour(contour, hole) {
    return (polygonsIntersect(contour, hole) ||
...
function buildSlabRegions(contour, holes) {
    const containedHoles = [];
    const edgeCutouts = [];
    for (const hole of holes) {
        if (hole.length < 3)
            continue;
        if (isStrictInteriorHole(contour, hole))
            containedHoles.push(hole);
        else if (affectsContour(contour, hole))
            edgeCutouts.push(hole);
    }
    const contours = edgeCutouts.length > 0 ? subtractPolygonsFromPolygon(contour, edgeCutouts) : [contour];
...
function generatePositiveSlabGeometry(slabNode) {
    ...
    for (const region of buildSlabRegions(polygon, holePolygons)) {
        const contour2d = ensureCounterClockwisePolygon(region.contour).map(([x, z]) => new THREE.Vector2(x, z));
