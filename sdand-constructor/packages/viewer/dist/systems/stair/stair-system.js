import { getEffectiveNode, getFloorStackedPosition, sceneRegistry, useScene, } from '@pascal-app/core';
...
export const StairSystem = () => {
    ...
    useFrame(() => {
        ...
        for (const stairId of parentsNeedingSegmentSync) {
            ...
            if (!baseStairNode || baseStairNode.type !== 'stair')
                continue;
            ...
            const stairNode = getEffectiveNode(baseStairNode);
...
function syncStairGroupElevation(stairNode, group, nodes) {
    const effectiveNodes = { ...nodes, [stairNode.id]: stairNode };
    for (const childId of stairNode.children ?? []) {
        const segment = nodes[childId];
        if (segment?.type === 'stair-segment') {
            effectiveNodes[segment.id] = getEffectiveNode(segment);
        }
    }
...
function generateStairRailingGeometry(stairNode, segments, transforms) {
    ...
    for (let index = 1; index < segmentRailPaths.length; index++) {
        ...
        if (!(previousPath && currentPath?.connectFromPrevious))
            continue;
...
function buildSegmentRailPath(layout, side, previousSegment, nextSegment, inset, landingInset) {
    ...
    const flightStartX = previousSegment?.segmentType === 'landing'
