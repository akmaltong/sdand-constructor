...
export const WallCutout = () => {
    ...
    useFrame(({ camera, clock }) => {
        ...
        const highlightedWallIds = new Set([...selectedIds, ...previewSelectedIds].filter((id) => useScene.getState().nodes[id]?.type === 'wall'));
        const deleteHoveredWallId = hoverHighlightMode === 'delete' &&
            hoveredId &&
            useScene.getState().nodes[hoveredId]?.type === 'wall'
            ? hoveredId
            : null;
        ...
        if (((distanceMoved > 0.5 || directionChanged > 0.3) && timeSinceUpdate > 0.1) ||
            ...
            walls.forEach((wallId) => {
                ...
                if (hideWall) {
                    ;
                    wallMesh.material = isDeleteHighlighted
    ...
    useEffect(() => {
        ...
        const restoreForCapture = () => {
            sceneRegistry.byType.wall.forEach((wallId) => {
                ...
                if (!wallNode || wallNode.type !== 'wall')
                    return;
                const mats = getMaterialsForWall(wallNode, useViewer.getState().shading, useViewer.getState().textures, useViewer.getState().colorPreset, useViewer.getState().sceneTheme);
