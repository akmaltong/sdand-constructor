import { calculateLevelMiters, DEFAULT_WALL_HEIGHT, getAdjacentWallIds, getEffectiveNode, getWallCurveFrameAt, getWallMiterBoundaryPoints, getWallPlanFootprint, getWallSurfacePolygon, getWallThickness, isCurvedWall, pointToKey, resolveLevelId, sceneRegistry, spatialGridManager, useLiveNodeOverrides, useLiveTransforms, useScene, } from '@pascal-app/core';
 ...
 */
function getEffectiveWall(wall) {
...
function getLevelWalls(levelId) {
    ...
    for (const childId of level.children) {
        ...
        if (child?.type === 'wall') {
            walls.push(getEffectiveWall(child));
        }
    }
    return walls;
}
...
function updateWallGeometry(wallId, miterData) {
    const nodes = useScene.getState().nodes;
    const sceneNode = nodes[wallId];
    if (!sceneNode || sceneNode.type !== 'wall')
        return;
    const node = getEffectiveWall(sceneNode);
    const mesh = sceneRegistry.nodes.get(wallId);
    if (!mesh)
        return;
    const levelId = resolveLevelId(node, nodes);
    const slabElevation = spatialGridManager.getSlabElevationForWall(levelId, node.start, node.end, node.curveOffset ?? 0, node.thickness);
...
function createShapedOpeningCutoutBrush(opening, wallThickness) {
    const shape = createShapedOpeningCutoutShape(opening);
    const depth = wallThickness * 2;
    const bevelSize = opening.openingShape === 'rounded'
        ? Math.min(Math.max(opening.openingRevealRadius ?? 0.025, 0), Math.max(wallThickness * 0.45, 0.001), Math.max((opening.cornerRadius ?? 0.15) * 0.45, 0.001))
        : 0;
...
function createShapedOpeningCutoutShape(opening) {
...
function getRoundedOpeningRadii(opening, width, height) {
    ...
    if (opening.openingRadiusMode === 'individual') {
        const [topLeft = 0, topRight = 0, bottomRight = 0, bottomLeft = 0] = opening.openingCornerRadii ?? [0.15, 0.15, 0.15, 0.15];
        return normalizeCornerRadii({
            topLeft: Math.max(topLeft, 0),
            topRight: Math.max(topRight, 0),
...
function normalizeCornerRadii(radii, width, height) {
    const next = { ...radii };
    const maxScale = Math.min(1, width / Math.max(next.topLeft + next.topRight, 1e-6), width / Math.max(next.bottomLeft + next.bottomRight, 1e-6), height / Math.max(next.topLeft + next.bottomLeft, 1e-6), height / Math.max(next.topRight + next.bottomRight, 1e-6));
    if (maxScale < 1) {
...
function applyRoundedOpeningShape(shape, left, right, bottom, top, radii) {
