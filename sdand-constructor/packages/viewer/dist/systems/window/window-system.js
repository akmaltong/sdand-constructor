import { getEffectiveNode, sceneRegistry, useInteractive, useLiveNodeOverrides, useScene, } from '@pascal-app/core';
import { useFrame } from '@react-three/fiber';
import { useEffect, useRef } from 'react';
import * as THREE from 'three';
import { createSurfaceRoleMaterial, glassMaterial as defaultGlassMaterial, baseMaterial as getBaseMaterial, } from '../../lib/materials';
...
export const WindowSystem = () => {
    ...
    const materialRevisionRef = useRef(null);
    ...
    useEffect(() => {
        ...
        if (materialRevisionRef.current === materialRevision)
            return;
    ...
    useFrame(() => {
        ...
        for (const id of dirtyWindowIds) {
            ...
            if (!mesh)
                continue; // Keep dirty until mesh mounts
            ...
            if (!isTransient && effectiveNode.parentId) {
                useScene.getState().dirtyNodes.add(effectiveNode.parentId);
            }
        }
    }, 3);
    return null;
...
function addShape(parent, material, shape, depth, z = 0) {
...
function createRectShape(left, right, bottom, top) {
...
function normalizeCornerRadii(radii, width, height) {
    const next = { ...radii };
    const scale = Math.min(1, width / Math.max(next.topLeft + next.topRight, 1e-6), width / Math.max(next.bottomLeft + next.bottomRight, 1e-6), height / Math.max(next.topLeft + next.bottomLeft, 1e-6), height / Math.max(next.topRight + next.bottomRight, 1e-6));
    if (scale < 1) {
        next.topLeft *= scale;
        next.topRight *= scale;
...
function getWindowRoundedRadii(node, width, height) {
    if (node.openingRadiusMode === 'individual') {
        const [topLeft = 0, topRight = 0, bottomRight = 0, bottomLeft = 0] = node.openingCornerRadii ?? [0.15, 0.15, 0.15, 0.15];
...
function insetCornerRadii(radii, inset, width, height) {
    return normalizeCornerRadii({
...
function createRoundedShape(left, right, bottom, top, radii) {
    const shape = new THREE.Shape();
...
function createRoundedFrameShape(width, height, frameThickness, outerRadii) {
    ...
    const innerRadii = insetCornerRadii(outerRadii, inset, innerRight - innerLeft, innerTop - innerBottom);
...
function getClampedArchHeight(width, height, archHeight) {
    return Math.min(Math.max(archHeight ?? width / 2, 0.01), Math.max(height, 0.01));
}
function createArchShape(left, right, bottom, top, archHeight) {
...
function createArchedFrameShape(width, height, archHeight, frameThickness) {
    const halfWidth = width / 2;
    const bottom = -height / 2;
    const top = height / 2;
    ...
    const innerBottom = bottom + inset;
    const innerTop = top - inset;
    const innerArchHeight = getClampedArchHeight(innerRight - innerLeft, innerTop - innerBottom, archHeight - inset);
    const hole = new THREE.Path(createArchShape(innerLeft, innerRight, innerBottom, innerTop, innerArchHeight)
        .getPoints(32)
        .reverse());
    outer.holes.push(hole);
    return outer;
}
function getArchBoundaryY(x, halfWidth, springY, archHeight) {
    if (halfWidth <= 1e-6)
        return springY;
    const t = Math.min(Math.abs(x) / halfWidth, 1);
    return springY + archHeight * Math.sqrt(Math.max(1 - t * t, 0));
}
function getArchedOpeningHalfWidthAtY(y, halfWidth, springY, archHeight) {
    if (y <= springY || archHeight <= 1e-6)
        return halfWidth;
...
function getRoundedBoundaryYAtX(x, left, right, top, radii) {
...
function getRoundedBottomBoundaryYAtX(x, left, right, bottom, radii) {
...
function getRoundedHorizontalBoundsAtY(y, left, right, top, radii) {
    let minX = left;
    let maxX = right;
...
function addRoundedWindowVisuals(node, mesh) {
    ...
    addShape(mesh, baseMaterial, createRoundedFrameShape(width, height, inset, outerRadii), frameDepth);
    if (innerW > 0.01 && innerH > 0.01) {
        const glassDepth = Math.max(0.004, frameDepth * 0.08);
        addShape(mesh, glassMaterial, createRoundedShape(innerLeft, innerRight, innerBottom, innerTop, innerRadii), glassDepth);
        ...
        for (let c = 0; c < numCols - 1; c++) {
            ...
            const dividerTop = Math.min(getRoundedBoundaryYAtX(x1, innerLeft, innerRight, innerTop, innerRadii), getRoundedBoundaryYAtX(x2, innerLeft, innerRight, innerTop, innerRadii));
            if (dividerTop > innerBottom + 0.01) {
                addShape(mesh, baseMaterial, createRectShape(x1, x2, innerBottom, dividerTop), frameDepth + 0.001);
            }
        ...
        for (let r = 0; r < numRows - 1; r++) {
            ...
            const { minX, maxX } = getRoundedHorizontalBoundsAtY(yTop, innerLeft, innerRight, innerTop, innerRadii);
            if (maxX - minX > 0.01 && yTop > innerBottom) {
                addShape(mesh, baseMaterial, createRectShape(minX, maxX, Math.max(yBottom, innerBottom), yTop), frameDepth + 0.001);
            }
...
function addArchedWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, columnRatios, rowRatios, columnDividerThickness, rowDividerThickness, sill, sillDepth, sillThickness, } = node;
    const halfWidth = width / 2;
    const bottom = -height / 2;
    const top = height / 2;
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        const glassDepth = Math.max(0.004, frameDepth * 0.08);
        addShape(mesh, glassMaterial, createArchShape(innerLeft, innerRight, innerBottom, innerTop, innerArchHeight), glassDepth);
        const numCols = columnRatios.length;
        const numRows = rowRatios.length;
        const usableW = innerW - (numCols - 1) * columnDividerThickness;
        const usableH = innerH - (numRows - 1) * rowDividerThickness;
        ...
        for (let c = 0; c < numCols - 1; c++) {
            ...
            const x1 = x;
            const x2 = x + columnDividerThickness;
            const dividerTop = Math.min(getArchBoundaryY(x1, innerHalfWidth, innerSpringY, innerArchHeight), getArchBoundaryY(x2, innerHalfWidth, innerSpringY, innerArchHeight));
            if (dividerTop > innerBottom + 0.01) {
                addShape(mesh, baseMaterial, createRectShape(x1, x2, innerBottom, dividerTop), frameDepth + 0.001);
            }
            x += columnDividerThickness;
        ...
        for (let r = 0; r < numRows - 1; r++) {
            ...
            const yTop = y;
            const yBottom = y - rowDividerThickness;
            const halfAtTop = getArchedOpeningHalfWidthAtY(yTop, innerHalfWidth, innerSpringY, innerArchHeight);
            const x1 = -halfAtTop;
            const x2 = halfAtTop;
            if (x2 - x1 > 0.01 && yTop > innerBottom) {
                addShape(mesh, baseMaterial, createRectShape(x1, x2, Math.max(yBottom, innerBottom), yTop), frameDepth + 0.001);
            }
            y -= rowDividerThickness;
        }
    }
...
function getWindowRenderOpenAmount(node) {
...
function getAwningDirection(node) {
    return node.windowType === 'hopper' ? 'down' : (node.awningDirection ?? 'up');
}
function isRectangleOnlyWindowType(node) {
    return (node.windowType === 'sliding' ||
        node.windowType === 'single-hung' ||
        node.windowType === 'double-hung' ||
        node.windowType === 'bay' ||
        node.windowType === 'bow');
}
function addSlidingWindowVisuals(node, mesh) {
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        addBox(mesh, baseMaterial, innerW, trackThickness, frameDepth, 0, innerH / 2 - trackThickness / 2, 0);
        addBox(mesh, baseMaterial, innerW, trackThickness, frameDepth, 0, -innerH / 2 + trackThickness / 2, 0);
        ...
        addBox(activePanel, baseMaterial, railThickness, panelH, frameDepth * 0.72, -panelWidth / 2 + railThickness / 2, 0, 0);
        addBox(mesh, baseMaterial, railThickness, panelH, frameDepth * 0.72, rightPanelX + panelWidth / 2 - railThickness / 2, 0, rightZ);
        addBox(activePanel, baseMaterial, railThickness, panelH, frameDepth * 0.78, panelWidth / 2 - railThickness / 2, 0, 0);
        addBox(mesh, baseMaterial, railThickness, panelH, frameDepth * 0.78, rightPanelX - panelWidth / 2 + railThickness / 2, 0, rightZ);
    }
    if (sill) {
        const sillW = width + sillDepth * 0.4;
        const sillZ = frameDepth / 2 + sillDepth / 2;
...
function addRectCasementSash(parent, name, panelW, panelH, frameThickness, frameDepth, pivotX, sashCenterX, rotationY) {
    ...
    parent.add(sash);
    addBox(sash, baseMaterial, panelW, sashFrameThickness, sashDepth, sashCenterX, panelH / 2 - sashFrameThickness / 2, 0);
    addBox(sash, baseMaterial, panelW, sashFrameThickness, sashDepth, sashCenterX, -panelH / 2 + sashFrameThickness / 2, 0);
    addBox(sash, baseMaterial, sashFrameThickness, panelH, sashDepth, sashCenterX - panelW / 2 + sashFrameThickness / 2, 0, 0);
    addBox(sash, baseMaterial, sashFrameThickness, panelH, sashDepth, sashCenterX + panelW / 2 - sashFrameThickness / 2, 0, 0);
    addBox(sash, glassMaterial, glassW, glassH, glassDepth, sashCenterX, 0, sashDepth * 0.08);
}
function addFrenchCasementHingeMarkers(mesh, innerW, innerH, frameThickness, frameDepth) {
    const markerW = Math.max(frameThickness * 0.38, 0.018);
    const markerH = innerH * 0.24;
    for (const pivotX of [-innerW / 2, innerW / 2]) {
        addBox(mesh, baseMaterial, markerW, markerH, frameDepth * 1.1, pivotX, innerH * 0.25, frameDepth * 0.08);
        addBox(mesh, baseMaterial, markerW, markerH, frameDepth * 1.1, pivotX, -innerH * 0.25, frameDepth * 0.08);
    }
}
function createFrenchArchLeafShape(leafW, leafH, fullW, springY, archHeight, side, inset = 0) {
    ...
    const shape = new THREE.Shape();
    const segments = 32;
    const topAt = (localX) => Math.max(bottom + 0.01, getArchBoundaryY(localX + xOffset, halfFullW, springY, archHeight) - inset);
    shape.moveTo(left, bottom);
    shape.lineTo(right, bottom);
...
function createFrenchArchLeafFrameShape(leafW, leafH, fullW, springY, archHeight, frameThickness, side) {
    ...
    if (inset <= 0.001)
        return outer;
    const hole = new THREE.Path(createFrenchArchLeafShape(leafW, leafH, fullW, springY, archHeight, side, inset)
        .getPoints(32)
        .reverse());
    outer.holes.push(hole);
    return outer;
}
function createFrenchRoundedLeafShape(leafW, leafH, fullW, fullRadii, side, inset = 0) {
    const left = -leafW / 2 + inset;
    const right = leafW / 2 - inset;
    const bottom = -leafH / 2 + inset;
    ...
    const radii = inset > 0 ? insetCornerRadii(fullRadii, inset, fullRight - fullLeft, top - bottom) : fullRadii;
    const shape = new THREE.Shape();
    const segments = 32;
    const topAt = (localX) => getRoundedBoundaryYAtX(localX + globalOffset, fullLeft, fullRight, top, radii);
    const bottomAt = (localX) => getRoundedBottomBoundaryYAtX(localX + globalOffset, fullLeft, fullRight, bottom, radii);
...
function createFrenchRoundedLeafFrameShape(leafW, leafH, fullW, fullRadii, frameThickness, side) {
    const outer = createFrenchRoundedLeafShape(leafW, leafH, fullW, fullRadii, side);
    const inset = Math.min(frameThickness, leafW / 2 - 0.005, leafH / 2 - 0.005);
    if (inset <= 0.001)
        return outer;
    const hole = new THREE.Path(createFrenchRoundedLeafShape(leafW, leafH, fullW, fullRadii, side, inset)
        .getPoints(32)
        .reverse());
    outer.holes.push(hole);
    return outer;
}
function addShapedFrenchCasementSash(parent, node, name, side, fullW, leafW, leafH, frameThickness, frameDepth, pivotX, sashCenterX, rotationY) {
    const sash = new THREE.Group();
    ...
    if (node.openingShape === 'arch') {
        ...
        addShape(sashVisual, baseMaterial, createFrenchArchLeafFrameShape(leafW, leafH, fullW, sashSpringY, sashArchHeight, sashFrameThickness, side), sashDepth);
        const glassInset = Math.min(sashFrameThickness, leafW / 2 - 0.005, leafH / 2 - 0.005);
        if (glassInset > 0.001) {
            addShape(sashVisual, glassMaterial, createFrenchArchLeafShape(leafW, leafH, fullW, sashSpringY, sashArchHeight, side, glassInset), glassDepth, sashDepth * 0.08);
        }
        return;
    }
    const frameRadii = insetCornerRadii(getWindowRoundedRadii(node, node.width, node.height), frameThickness, fullW, leafH);
    addShape(sashVisual, baseMaterial, createFrenchRoundedLeafFrameShape(leafW, leafH, fullW, frameRadii, sashFrameThickness, side), sashDepth);
    const glassInset = Math.min(sashFrameThickness, leafW / 2 - 0.005, leafH / 2 - 0.005);
    if (glassInset > 0.001) {
        addShape(sashVisual, glassMaterial, createFrenchRoundedLeafShape(leafW, leafH, fullW, frameRadii, side, glassInset), glassDepth, sashDepth * 0.08);
    }
}
function addFrenchCasementWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const innerW = width - 2 * frameThickness;
    const innerH = height - 2 * frameThickness;
    // Fixed outer frame.
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        addRectCasementSash(mesh, FRENCH_CASEMENT_LEFT_SASH_NAME, leafW, innerH, frameThickness, frameDepth, -innerW / 2, leafW / 2, -openAngle);
        addRectCasementSash(mesh, FRENCH_CASEMENT_RIGHT_SASH_NAME, leafW, innerH, frameThickness, frameDepth, innerW / 2, -leafW / 2, openAngle);
...
function addShapedCasementWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const innerW = width - 2 * frameThickness;
    const innerH = height - 2 * frameThickness;
    if (node.openingShape === 'arch') {
        addShape(mesh, baseMaterial, createArchedFrameShape(width, height, getClampedArchHeight(width, height, node.archHeight), frameThickness), frameDepth);
    }
    else {
        addShape(mesh, baseMaterial, createRoundedFrameShape(width, height, frameThickness, getWindowRoundedRadii(node, width, height)), frameDepth);
    }
    if ((node.casementStyle ?? 'single') === 'french') {
        if (innerW > 0.01 && innerH > 0.01) {
            const openAngle = getWindowRenderOpenAmount(node) * (Math.PI / 2);
            const leafW = innerW / 2;
            addShapedFrenchCasementSash(mesh, node, FRENCH_CASEMENT_LEFT_SASH_NAME, 'left', innerW, leafW, innerH, frameThickness, frameDepth, -innerW / 2, leafW / 2, -openAngle);
            addShapedFrenchCasementSash(mesh, node, FRENCH_CASEMENT_RIGHT_SASH_NAME, 'right', innerW, leafW, innerH, frameThickness, frameDepth, innerW / 2, -leafW / 2, openAngle);
            addFrenchCasementHingeMarkers(mesh, innerW, innerH, frameThickness, frameDepth);
        }
        if (sill) {
            const sillW = width + sillDepth * 0.4;
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        mesh.add(sash);
        if (node.openingShape === 'arch') {
            const sashArchHeight = getClampedArchHeight(innerW, innerH, (node.archHeight ?? innerW / 2) - frameThickness);
            addShape(sashVisual, baseMaterial, createArchedFrameShape(innerW, innerH, sashArchHeight, sashFrameThickness), sashDepth);
            ...
            if (glassInset > 0.001) {
                ...
                addShape(sashVisual, glassMaterial, createArchShape(-glassW / 2, glassW / 2, -glassH / 2, glassH / 2, getClampedArchHeight(glassW, glassH, sashArchHeight - glassInset)), glassDepth, sashDepth * 0.08);
            }
        }
        else {
            const outerRadii = getWindowRoundedRadii(node, innerW, innerH);
            addShape(sashVisual, baseMaterial, createRoundedFrameShape(innerW, innerH, sashFrameThickness, outerRadii), sashDepth);
            const glassInset = Math.min(sashFrameThickness, innerW / 2 - 0.005, innerH / 2 - 0.005);
            if (glassInset > 0.001) {
                const glassW = innerW - 2 * glassInset;
                const glassH = innerH - 2 * glassInset;
                addShape(sashVisual, glassMaterial, createRoundedShape(-glassW / 2, glassW / 2, -glassH / 2, glassH / 2, insetCornerRadii(outerRadii, glassInset, glassW, glassH)), glassDepth, sashDepth * 0.08);
            }
        }
        addBox(mesh, baseMaterial, Math.max(frameThickness * 0.38, 0.018), innerH * 0.28, frameDepth * 1.1, pivotX, innerH * 0.24, frameDepth * 0.08);
        addBox(mesh, baseMaterial, Math.max(frameThickness * 0.38, 0.018), innerH * 0.28, frameDepth * 1.1, pivotX, -innerH * 0.24, frameDepth * 0.08);
    }
    if (sill) {
        const sillW = width + sillDepth * 0.4;
...
function addCasementWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        sash.rotation.y = hingeSign * openAngle;
        mesh.add(sash);
        addBox(sash, baseMaterial, innerW, sashFrameThickness, sashDepth, sashCenterX, innerH / 2 - sashFrameThickness / 2, 0);
        addBox(sash, baseMaterial, innerW, sashFrameThickness, sashDepth, sashCenterX, -innerH / 2 + sashFrameThickness / 2, 0);
        addBox(sash, baseMaterial, sashFrameThickness, innerH, sashDepth, sashCenterX - innerW / 2 + sashFrameThickness / 2, 0, 0);
        addBox(sash, baseMaterial, sashFrameThickness, innerH, sashDepth, sashCenterX + innerW / 2 - sashFrameThickness / 2, 0, 0);
        addBox(sash, glassMaterial, glassW, glassH, glassDepth, sashCenterX, 0, sashDepth * 0.08);
...
function addAwningWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    if (node.openingShape === 'rounded' || node.openingShape === 'arch') {
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        mesh.add(sash);
        addBox(sash, baseMaterial, innerW, sashFrameThickness, sashDepth, 0, sashCenterY + innerH / 2 - sashFrameThickness / 2, 0);
        addBox(sash, baseMaterial, innerW, sashFrameThickness, sashDepth, 0, sashCenterY - innerH / 2 + sashFrameThickness / 2, 0);
        addBox(sash, baseMaterial, sashFrameThickness, innerH, sashDepth, -innerW / 2 + sashFrameThickness / 2, sashCenterY, 0);
        addBox(sash, baseMaterial, sashFrameThickness, innerH, sashDepth, innerW / 2 - sashFrameThickness / 2, sashCenterY, 0);
        ...
        addBox(mesh, baseMaterial, innerW * 0.42, Math.max(frameThickness * 0.38, 0.018), frameDepth * 1.1, 0, pivotY, frameDepth * 0.08);
    }
    if (sill) {
        const sillW = width + sillDepth * 0.4;
        const sillZ = frameDepth / 2 + sillDepth / 2;
...
function addShapedAwningWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const innerW = width - 2 * frameThickness;
    const innerH = height - 2 * frameThickness;
    if (node.openingShape === 'arch') {
...
function addHopperWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    if (node.openingShape === 'rounded' || node.openingShape === 'arch') {
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        sash.rotation.x = -openAngle;
        mesh.add(sash);
        addBox(sash, baseMaterial, innerW, sashFrameThickness, sashDepth, 0, innerH - sashFrameThickness / 2, 0);
        addBox(sash, baseMaterial, innerW, sashFrameThickness, sashDepth, 0, sashFrameThickness / 2, 0);
        addBox(sash, baseMaterial, sashFrameThickness, innerH, sashDepth, -innerW / 2 + sashFrameThickness / 2, innerH / 2, 0);
        addBox(sash, baseMaterial, sashFrameThickness, innerH, sashDepth, innerW / 2 - sashFrameThickness / 2, innerH / 2, 0);
...
function addShapedHopperWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const innerW = width - 2 * frameThickness;
    const innerH = height - 2 * frameThickness;
    if (node.openingShape === 'arch') {
...
function addHungSash(parent, panelW, panelHeight, sashFrameThickness, frameDepth, glassDepth, glassW, glassH) {
    addBox(parent, baseMaterial, panelW, sashFrameThickness, frameDepth * 0.72, 0, panelHeight / 2 - sashFrameThickness / 2, 0);
    addBox(parent, baseMaterial, panelW, sashFrameThickness, frameDepth * 0.72, 0, -panelHeight / 2 + sashFrameThickness / 2, 0);
    addBox(parent, baseMaterial, sashFrameThickness, panelHeight, frameDepth * 0.72, -panelW / 2 + sashFrameThickness / 2, 0, 0);
    addBox(parent, baseMaterial, sashFrameThickness, panelHeight, frameDepth * 0.72, panelW / 2 - sashFrameThickness / 2, 0, 0);
    addBox(parent, glassMaterial, glassW, glassH, glassDepth, 0, 0, 0);
}
function addSingleHungWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const innerW = width - 2 * frameThickness;
    const innerH = height - 2 * frameThickness;
    // Fixed outer frame.
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        addBox(mesh, baseMaterial, trackThickness, innerH, frameDepth, -innerW / 2 + trackThickness / 2, 0, 0);
        addBox(mesh, baseMaterial, trackThickness, innerH, frameDepth, innerW / 2 - trackThickness / 2, 0, 0);
        const topSash = new THREE.Group();
        topSash.position.set(0, topPanelY, topZ);
        mesh.add(topSash);
        addHungSash(topSash, panelW, panelHeight, sashFrameThickness, frameDepth, glassDepth, glassW, glassH);
        addHungSash(activeSash, panelW, panelHeight, sashFrameThickness, frameDepth, glassDepth, glassW, glassH);
        // Meeting rails: top sash fixed, bottom sash moves upward over it.
        addBox(mesh, baseMaterial, panelW, railThickness, frameDepth * 0.78, 0, topPanelY - panelHeight / 2 + railThickness / 2, topZ);
        addBox(activeSash, baseMaterial, panelW, railThickness, frameDepth * 0.78, 0, panelHeight / 2 - railThickness / 2, 0);
    }
    if (sill) {
        const sillW = width + sillDepth * 0.4;
        const sillZ = frameDepth / 2 + sillDepth / 2;
...
function addDoubleHungWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const innerW = width - 2 * frameThickness;
    const innerH = height - 2 * frameThickness;
    // Fixed outer frame.
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        addBox(mesh, baseMaterial, trackThickness, innerH, frameDepth, innerW / 2 - trackThickness / 2, 0, 0);
        addHungSash(topSash, panelW, panelHeight, sashFrameThickness, frameDepth, glassDepth, glassW, glassH);
        addHungSash(bottomSash, panelW, panelHeight, sashFrameThickness, frameDepth, glassDepth, glassW, glassH);
        // Opposing meeting rails: top sash descends while bottom sash rises.
        addBox(topSash, baseMaterial, panelW, railThickness, frameDepth * 0.78, 0, -panelHeight / 2 + railThickness / 2, 0);
        addBox(bottomSash, baseMaterial, panelW, railThickness, frameDepth * 0.78, 0, panelHeight / 2 - railThickness / 2, 0);
    }
    if (sill) {
        const sillW = width + sillDepth * 0.4;
        const sillZ = frameDepth / 2 + sillDepth / 2;
...
function addBayWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const innerW = width - 2 * frameThickness;
    const innerH = height - 2 * frameThickness;
    addBox(mesh, baseMaterial, width, frameThickness, frameDepth, 0, height / 2 - frameThickness / 2, 0);
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        const addBayPanel = (parent, panelW) => {
            const glassW = Math.max(panelW - 2 * sashFrameThickness, 0.01);
            const glassH = Math.max(innerH - 2 * sashFrameThickness, 0.01);
            addBox(parent, baseMaterial, panelW, sashFrameThickness, panelDepth, 0, innerH / 2 - sashFrameThickness / 2, 0);
            addBox(parent, baseMaterial, panelW, sashFrameThickness, panelDepth, 0, -innerH / 2 + sashFrameThickness / 2, 0);
            addBox(parent, baseMaterial, sashFrameThickness, innerH, panelDepth, -panelW / 2 + sashFrameThickness / 2, 0, 0);
            addBox(parent, baseMaterial, sashFrameThickness, innerH, panelDepth, panelW / 2 - sashFrameThickness / 2, 0, 0);
            addBox(parent, glassMaterial, glassW, glassH, glassDepth, 0, 0, panelDepth * 0.08);
        };
        const addBayCap = (centerY) => {
            ...
            indices.push(0, 1, 2, 0, 2, 3, 4, 6, 5, 4, 7, 6, 0, 4, 5, 0, 5, 1, 1, 5, 6, 1, 6, 2, 2, 6, 7, 2, 7, 3, 3, 7, 4, 3, 4, 0);
...
function addBowWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const innerW = width - 2 * frameThickness;
    const innerH = height - 2 * frameThickness;
    addBox(mesh, baseMaterial, width, frameThickness, frameDepth, 0, height / 2 - frameThickness / 2, 0);
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        const arcZAt = (x) => projectionDepth * (1 - (x / halfSpan) ** 2);
        ...
        const createCurvedVerticalBand = (yBottom, yTop, zOffset = 0) => {
            const positions = [];
            const indices = [];
        ...
        const createCurvedCap = (centerY, thickness) => {
            const positions = [];
            const indices = [];
            ...
            for (let index = 0; index < curveSegments; index += 1) {
                const a = index * 4;
                const b = a + 4;
                indices.push(a, b, a + 2, b, b + 2, a + 2, a + 1, a + 3, b + 1, b + 1, a + 3, b + 3, a + 2, b + 2, a + 3, b + 2, b + 3, a + 3, a, a + 1, b, b, a + 1, b + 1);
            }
            const geometry = new THREE.BufferGeometry();
            geometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
            geometry.setIndex(indices);
        ...
        const addCurvedMesh = (material, geometry) => {
...
function addLouveredWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    if (node.openingShape === 'rounded' || node.openingShape === 'arch') {
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        mesh.add(slats);
        addBox(mesh, baseMaterial, railThickness, innerH, frameDepth * 0.95, -innerW / 2 + railThickness / 2, 0, 0);
        addBox(mesh, baseMaterial, railThickness, innerH, frameDepth * 0.95, innerW / 2 - railThickness / 2, 0, 0);
        for (let index = 0; index < slatCount; index += 1) {
            ...
            slats.add(slat);
            addBox(slat, glassMaterial, Math.max(innerW - 2 * railThickness, 0.01), slatHeight, slatDepth, 0, 0, 0);
        }
    }
    if (sill) {
        const sillW = width + sillDepth * 0.4;
...
function addShapedLouveredWindowVisuals(node, mesh) {
    const { width, height, frameDepth, frameThickness, sill, sillDepth, sillThickness } = node;
    const halfWidth = width / 2;
    const bottom = -height / 2;
    const top = height / 2;
    ...
    if (innerW > 0.01 && innerH > 0.01) {
        ...
        slats.name = LOUVERED_WINDOW_SLATS_NAME;
        mesh.add(slats);
        const getBoundsAtY = node.openingShape === 'arch'
            ? (() => {
            ...
            : (() => {
                const innerRadii = insetCornerRadii(getWindowRoundedRadii(node, width, height), inset, innerW, innerH);
                return (y) => getRoundedHorizontalBoundsAtY(y, innerLeft, innerRight, innerTop, innerRadii);
            })();
        const addVerticalRail = (x) => {
            const railX1 = x;
            const railX2 = x + (x < 0 ? railThickness : -railThickness);
            const sampleX = x < 0 ? Math.max(railX1, railX2) : Math.min(railX1, railX2);
            const railTop = node.openingShape === 'arch'
                ? getArchBoundaryY(sampleX, innerW / 2, top - getClampedArchHeight(width, height, node.archHeight), getClampedArchHeight(innerW, innerH, getClampedArchHeight(width, height, node.archHeight) - inset))
                : getRoundedBoundaryYAtX(sampleX, innerLeft, innerRight, innerTop, insetCornerRadii(getWindowRoundedRadii(node, width, height), inset, innerW, innerH));
            addShape(mesh, baseMaterial, createRectShape(Math.min(railX1, railX2), Math.max(railX1, railX2), innerBottom, railTop), frameDepth * 0.95);
        };
...
function updateWindowMesh(node, mesh) {
    ...
    const { width, height, frameDepth, frameThickness, columnRatios, rowRatios, columnDividerThickness, rowDividerThickness, sill, sillDepth, sillThickness, openingKind, openingShape, windowType, } = node;
...
function syncWindowCutout(node, mesh) {
    ...
    if (isRectangleOnlyWindowType(node)) {
        ...
        cutout.geometry = new THREE.ExtrudeGeometry(createArchShape(-node.width / 2, node.width / 2, -node.height / 2, node.height / 2, getClampedArchHeight(node.width, node.height, node.archHeight)), {
            depth: 1,
            bevelEnabled: false,
            curveSegments: 24,
        });
        ...
        cutout.geometry = new THREE.ExtrudeGeometry(createRoundedShape(-node.width / 2, node.width / 2, -node.height / 2, node.height / 2, getWindowRoundedRadii(node, node.width, node.height)), {
            depth: 1,
            bevelEnabled: false,
            curveSegments: 24,
        });
